...
**EaseScript 是基于ES6语法的编译型语言。期望所有的功能组件开发一次后可以运行在不同的目标语言环境中，从而达到共享复用减少重复的开发成本。**
...
4. 同功能只开发一次，可编译成不同的目标运行语言（比如 java、golang、python、php、nodejs等。目前仅支持php、nodejs敬请期待~）
