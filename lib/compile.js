...
Iteration.prototype.parseFunctionScope=function( stack )
{
    ...
    else if( stack.keyword() === 'statement' && stack.parent().keyword() === 'var' && stack.parent().scope().keyword() !== 'function' )
    {
        ...
        var done = function (e)
        {
            if( items.length > 0 )
            {
                var defaultVar = buildSyntax.declarationDefaultVar(items);
                if( defaultVar )
                {
                    var startIndex = e.content.indexOf('{');
...
function captureRunPlatformMetatype(it, desc,property, classmodule )
{
    ...
    var pid = it.stack.parent().keyword();
    var scope = it.stack.scope();
    if( pid === "condition" || ( pid==="object" && scope["compile_policy_"+classmodule.currentMakeSyntax] !==true ) )
    {
        throw new ReferenceError( '"'+property.info.join(".") + '" has been removed. in compiling the "'+classmodule.currentMakeSyntax+'" syntax.');
    }
...
function compilePolicy(it,desc,classmodule)
{
    ...
    if( method==="Syntax" && express !=="origin" && globalsConfig.syntax_supported[ express ] !== true )
    {
    ...
    if( method==="RunPlatform" ){
        result = express ==="client" && isClient || express ==="server" && !isClient;
    }else if( method==="Syntax" ){
        if( express ==="origin" ){
            express = globalsConfig.originMakeSyntax;
        }
        result = express === syntax;
    }
    if( scope.keyword() !== "if" )
    {
        desc.thisArg=result;
    ...
    var done = function (e) {
        ...
        //将开始的if else 弹出
        if( (id==="if" && result) || (id==="else" && !result) )
        {
    ...
    if( hasElse )
    {
        if( !(content[indexAt+2] instanceof Ruler.STACK) || content[indexAt+2].keyword().toLowerCase() !=="if" )
        {
