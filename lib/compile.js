...
Iteration.prototype.parseArrowFunction=function( stack )
{
    ...
    const done = e => {
        ...
        if( single )
        {
            e.iteration.content.push( buildSyntax.makeArrowFunction( this.module,this.module.config, params,`return ${body.join("")};`) );
        }else
        {
            if( body[0] ==="{" )body.shift();
            if( body[body.length-1] ==="}" )body.pop();
            e.iteration.content.push( buildSyntax.makeArrowFunction( this.module,this.module.config, params, body.join("") ) );
        }
    };
        ...
        if (isReference(it.current))
        {
            ...
            if( constant_property.indexOf(it.current.value) >= 0)
            {
                ...
                switch ( it.current.value ){
                    case "__CLASS__" :
                        property.thisArg= buildSyntax.getQualifiedObjectName(classmodule,  buildSyntax.getVarReference("this") );
                        break;
                    case "__FILE__" :
            ...
            else
            {
                ...
                {
                    const extractIntoVariable = it.stack.parent().previous(1).content();
...
function expression( stack, classmodule ,  acceptType , fromParam, fromReturn, pit )
{
    ...
    if( isReferenceType(acceptType) )
    {
        ...
        } else if ( !checkTypeOf(classmodule, acceptType, valueType, valueDescribe , null, fromParam ) && !( stack.parent().type() ==='(property)' && valueType==='Number' ) )
        {
            //不是数字类型就报错
            if( !(numberType.indexOf(acceptType.toLowerCase())>=0 && numberType.indexOf(valueType.toLowerCase())>=0)  )
            {
...
function toString( stack, module, acceptType , fromParam , fromReturn, pit )
{
    ...
    while ( it.seek() )
    {
        ...
        if (it.current instanceof Ruler.STACK)
        {
            it.content.push( toString(it.current, module, undefined,undefined,undefined, it ) );
        ...
        else if( it.current )
        {
            ...
            if( it.current.id === '(keyword)'  )
            {
                ...
                }else
                {
                    ...
                    stack.getScopeOf().define(it.current.value) )
                    {
                        const pos = stack.content().indexOf( it.current );
                        const nextStack = stack.previous( pos+1 );
                        if( nextStack.value !==":" )
                        {
...
function checkTypeOf(classModule, needType, currentType, describeType, needDescribe, flag )
{
    ...
    if( needType==='Object' )
    {
        ...
        if( describeType && describeType.isWrapReference && ['boolean','number','int',"float","integer","double"].indexOf( currentType.toLowerCase() ) >= 0  )
        {
            return false;
        }
        return true;
...
function getDependencies(config, modules, syntax)
{
    ...
    {
        const modules = getAllModules( module );
