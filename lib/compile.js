...
function checkCallParameter(it, desc, property , isNew )
{
    ...
    {
        var _desc =  Compile.getLoaclAndGlobalModuleDescription( desc.fullclassname )
    ...
    if( param && parameters.length < param.length )
    {
        for(var i in param )
        {
            ...
            if( parameters[i] === undefined )
            {
                ...
                if( isNew ) {
                    error('Missing parameter the "new ' + property.info.join(".") + '('+parameters.join(",")+')"', 'type');
                }else{
                    error('Missing parameter the "' + property.info.join(".") + '('+parameters.join(",")+')"', 'type');
                }
            }
        }
    }
        ...
        if (isReference(it.current))
        {
            ...
            else
            {
                ...
                if (desc)
                {
                    ...
                    {
                        identifierNames = identifierNames.join("_");
...
{
    //全类型名称
    if( type.indexOf('.') > 0 && getmodule(type) )
    {
        return type;
    }
...
function getNamespaceModuleValue( module, name)
{
    ...
    }else if( module )
    {
        ...
        if (nsmodule && nsmodule.namespaces.hasOwnProperty(name))
        {
            ...
            {
                makeModule(_nsMakeModule, nsmodule, globalsConfig);
            }
...
function makeViewMetatype(config, currentModule, metatype, itemStack )
{
    ...
    var done = function (e) {
        ...
        if( description.nonglobal===true )
        {
            ...
            var thisvrg = buildSyntax.buildExpressionNewOf(config, currentModule, className , property, [] , [ buildSyntax.getVarReference("this") ], property);
...
function makeModule(syntax, stack , classModule, config , force )
{
    ...
    if( stack.keyword()==="namespace" )
    {
        ...
        {
            classModule.namespaces[ stack.name() ].value = getNamespaceValue(content[j], classModule);
        }
