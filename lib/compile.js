...
function isConformRunPlatform(desc,syntax)
{
    ...
    //没有明确指定运行平台和语法
    if( !(defineMetaTypeList.RunPlatform || defineMetaTypeList.Syntax || defineMetaTypeList.Env) )
    {
        return true;
    }
...
//只能在客户端运行的类
const canOnlyClientRun=["MouseEvent","TouchEvent","TouchDragEvent","TouchPinchEvent","TouchSwipeEvent"];
...
function compilePolicy(it,desc,classmodule)
{
    ...
    if( !(method==="RunPlatform" || method==="Syntax" || method==="Env") )
    {
        return false;
    }
    //跳到下一个标识符
