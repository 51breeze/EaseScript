        ...
        for (var i in items)
        {
            ...
            if ( desc.type !== '*' )
            {
                var _con = buildSyntax.getCheckMethodParamTypeValue( globalsConfig,
                    ...
                    shortName, nullable, typeClass, this.module.filename, Iteration.lastStack.line );
                if( _con ) {
                    express.push(_con);
                }
            }
        ...
        if( it.current.type() === '(expression)' )
        {
            ...
            if( it.current instanceof Ruler.STACK )
            {
                if( isForceType )
                {
                    ...
                    //在运行时强制转换
                    if( needTypeConvert ){
                        property.thisArg = buildSyntax.toForceTypeConvert( property.thisArg, type, expressionProperty, classmodule );
                    }
...
function checkTypeOf(classModule, needType, currentType, describeType, needDescribe )
{
    ...
    if( numberIndex >=0 && needType !=="Number" )
    {
        ...
        if( valueNumberIndex >=0 && /^[-+]?[\d\.]+$/.test( describeType.thisArg ) )
        {
            ...
            if (needType === "uint" && testValue < 0 )return false;
            if (needType === "int" && (testValue > globals.Integer.static.MAX || testValue < testValue<globals.Integer.static.MIN) )return false;
            return true;
        }
        //当前值的类型与给定的类型一致
