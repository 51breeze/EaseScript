        ...
        if( e.iteration.index === 0 )
        {
            ...
            stack.param().map( (name)=>{
                ...
                if( defaultValue )
                {
                    ...
                    {
                        defaultValue = toString( desc.value, this.module, type );
                        var stackType= getType( desc.value.type() );
                        if( stackType ==="JSON" || stackType ==="Array" )
                        {
                            defaultValue = buildSyntax.newInstanceClassOf("JSON", getmodule("JSON", this.module ), defaultValue);
                        }
...
Iteration.prototype.nextOf=function(stack)
{
    ...
    {
        return;
        //error('keyword the "of" can only in for iterator','syntax');
    }
    if( stack.parent().content().length > 1 )error('Can only statement one','syntax');
...
Iteration.prototype.parseArgumentsOfMethodAndConstructor=function( stack )
{
    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        const createParamType = (name, desc, targetName )=>{
            ...
            return {type:shortName,value:value, check:!!_def,originValue:originValue, nullable:nullable,typeModuleClass:typeClass };
        };
...
function checkCallParameter(it, desc, property , isNew )
{
    ...
    var parentStack = stackCurrent.parent();
    var lastIndex = parentStack ? parentStack.content().indexOf( stackCurrent ) : -1; 
    var hasRest = 0;
        ...
        while (it.seek())
        {
            if (it.current instanceof Ruler.STACK)
            {
                ...
                if( it.current.keyword() ==="expression" && it.prev instanceof Ruler.STACK && it.prev.keyword() ==="expression")
                {
                    ...
                    paramValue= buildSyntax.getClosureFunctionCall( pa, toString(it.current, it.module , acceptType, true ) );

                }else
                {
                    paramValue= toString(it.current, it.module , acceptType, true );
                }

                if( lastIndex > 0 )
                {
                    if( buildSyntax.isNeedReferenceParamOf(it.module, globalsConfig, property.ownerThisType, parentStack.previous( lastIndex-1 ).value, parameters.length ) )
                    {
                        //console.log( paramValue, lastProperty.descriptor )
                       // process.exit();
                    }
                }
...
function isRemoveExpression(desc,syntax)
{
    ...
    {
        var env = defineMetaTypeList.Env.param.value;
        result = env !== globalsConfig.env_name;
    }
...
function compilePolicy(it,desc,classmodule)
{
    ...
    {
        result = globalsConfig.env_name === condition;
...
var block_statck = ['function','for','if','else','while','try','catch','finally','switch']
...
function createSourceMap(module, original, generated,name)
{
    if( !module.sourceMap )
    {
        module.sourceMap = [];
    }
    module.sourceMap.push([original,generated,name]);
}
    ...
    while ( it.seek() )
    {
        ...
        else if( it.current )
        {
                ...
                if( stack.keyword()==="object" && stack.type()==="(JSON)" && it.current.value !=="{" && it.current.value !=="}" && it.current.value !=="," )
                {
                    ...
                    }else if( !(it.prev && it.prev.value ===":") && !(it.next && it.next.value ===":") )
                    {
                        if( it.current.id==="(identifier)" || it.current.type === '(string)' || it.current.type === "(number)" )
                        {
                            ...
                            if( it.current.type =='(identifier)' && stack.scope().define( it.current.value ) )
                            {
                                currentValue = buildSyntax.getVarReference( it.current.value )
                            }
            ...
            }else if( (it.current.value==='{' || it.current.value==='}') && !(it.next && (it.next.value===';' || it.next.value==='\n'))  )
            {
                ...
                if(  ['if','else','function','do','switch','while','for','try','catch','finally'].indexOf(kid)>=0 )
                {
...
function getAllModules( module , results, hash, syntax )
{
    ...
    {
         var metaTypeModule = module;
         while( metaTypeModule && metaTypeModule.nonglobal && ( !metaTypeModule.defineMetaTypeList || 
            !(metaTypeModule.defineMetaTypeList.RunPlatform || metaTypeModule.defineMetaTypeList.Syntax) ) )
         {
             if( metaTypeModule.inherit && metaTypeModule.inherit !=="Object" )
             {
                metaTypeModule = Compile.getLoaclAndGlobalModuleDescription( getImportClassByType( metaTypeModule, metaTypeModule.inherit ) )
             }else{
                metaTypeModule = null;
             }
         }
        ...
        //与当前指定的语法不一致的依赖不获取
        if( metaTypeModule && !isConformRunPlatform(metaTypeModule,syntax||"javascript") )
        {
            return results;
        }
