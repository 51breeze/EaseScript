...
Iteration.prototype.checkStatement=function(stack)
{
    ...
    if( stack.name()==="@extractIntoVariable" )
    {
        ...
        var extracts = [];
        var isrefer = valueDescriptor.id==="var" || valueDescriptor.id==="let" || valueDescriptor.id==="const";
        var flag =  valueDescriptor.accessor || valueDescriptor.expression || valueDescriptor.isFunAccessor || !isrefer;
        if( flag )
        {
            var uid = null;
            do{
                uid = "__var"+Utils.uid()+"__";
                if( scope.define(uid) ){
                    uid = null;
                }
            }while( !uid );
        ...
        //不能从函数中提出属性
        if( valueDescriptor.type ==="Function" && valueDescriptor.descriptor.id==="function" ||  
            valueDescriptor.type ==="Class" && valueDescriptor.descriptor.id==="class" )
        {   
        ...
        Utils.forEach(extracts, function(item,index){
            if( item.hasValue ){
                extractContent.push( item.name+"="+item.express+"||"+item.value+"," );
            }else{
                extractContent.push( item.name+"="+item.express+"," );
            }
        });
        ...
        case "ServiceProvider" :
            ...
            if( !express.ServiceProviderValue )
            {
                ...
                if ( !serviceProviderList[key] )
                {
                    ...
                    Compile.global_route_unique_hash[ express.ServiceProviderValue ] = param[0]+'@'+param[1];
...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    }else
    {
        ...
        if (isReference(it.current))
        {
            ...
            {
                //super 只能在类成员函数中
                var classScope = it.stack.getScopeOf();
...
function checkInterfaceParamAndType(desc,name,classModule,ns)
{
    ...
    for( var i in interfaceDesc )
    {
        ...
        for( var name in interProto )
        {
            ...
            if( ns )
            {
                ...
                {
                    checkInterfaceParamAndType(interProto[name][p],p,classModule,ns);
                }

            }else
            {
                checkInterfaceParamAndType(interProto[name],name,classModule);
            }
        }
    }
}
