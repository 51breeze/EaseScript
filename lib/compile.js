...
Iteration.prototype.parseArgumentsOfMethodAndConstructor=function( stack )
{
    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        for (var i in items)
        {
            ...
            {
                typeClass = getmodule(getImportClassByType(this.module, desc.type));
            ...
            if ( desc.type !== '*' )
            {
                var _con = buildSyntax.getCheckMethodParamTypeValue( globalsConfig,
                    ...
                    shortName, typeClass, this.module.filename, Iteration.lastStack.line );
                if( _con ) {
                    express.push(_con);
                }
            }
        ...
        if( it.current.type() === '(expression)' )
        {
            ...
            if( it.current instanceof Ruler.STACK )
            {
                if( isForceType )
                {
                    ...
                    var valueType = lastProperty.descriptor ? lastProperty.descriptor.type : '';
                    var needTypeConvert = true;
                    if( isReferenceType(valueType) )
                    {
                        needTypeConvert = false;
                        desc = getmodule( getImportClassByType(classmodule, type) );
                        if( !checkTypeOf(classmodule, type , valueType, lastProperty, desc) )
                        {
                            throw new TypeError(property.thisArg + " can not be converted for " + type);
                        }
                    }

                    if( needTypeConvert )
                    {
                        property.thisArg = buildSyntax.toForceTypeConvert( property.thisArg,  type, lastProperty, classmodule );
                    }
            ...
            {
                desc = getmodule( getImportClassByType(classmodule, property.type) );
            }
        ...
        if (isReference(it.current))
        {
            ...
            else
            {
                ...
                if (desc)
                {
                    ...
                    {
                        var __classModule = getmodule( getImportClassByType(classmodule, getType(desc.type) ) );
...
function doPrototype(classModule, callback, notObject )
{
    ...
    while ( classModule )
    {
        ...
        {
            return doPrototype( getmodule("Object") , callback, notObject );
        }
    }
    return null;
}

var numberType = ['float',"double","integer",'uint','int','number'];
...
function checkTypeOf(classModule, needType, currentType, describeType, needDescribe )
{
    ...
    var numberIndex = ['float',"double","integer",'uint','number'].indexOf( currentType.toLowerCase() );
    if( needType ==="Number" )
    {
        return numberIndex >= 0;
    }

    if( numberType.indexOf( needType.toLowerCase() ) === numberIndex )
    {
        return true;
    }
...
function checkUseNamespace(item, classModule , nameNs )
{
    ...
    setClassModuleUsed( getmodule( getImportClassByType( classModule, nameNs ) ) ,classModule );
}
...
function findDescInPrototype(name, classModule, proto ){
    ...
    proto = proto || 'proto';
    return doPrototype( getmodule( getImportClassByType(classModule,  classModule.inherit || "Object" ) ),function (module){
