...
Iteration.prototype.parseArgumentsOfMethodAndConstructor=function( stack )
{
        ...
        if( stack.name() === this.module.classname && !stack.static() && stack.parent().keyword() === 'class')
        {
            var _done = function (e)
            {
                ...
                if (e.iteration.module.inherit && !stack.called)
                {
                    ...
                    {
                        //if( inheritClass && !inheritClass.isAbstract )
                       // {
    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        for (var i in items)
        {
            ...
            if ( desc.type !== '*' )
            {
                var _con = buildSyntax.getCheckMethodParamTypeValue( this.module, globalsConfig,
...
function Compile(syntax, stack, description, config , flag )
{
    ...
    if( !description.nonglobal && buildSyntax.loadSystemModuleContentByName )
    {
        if( !description.makeContent[ syntax ] )
        {
            const systemModule = buildSyntax.loadSystemModuleContentByName(config, name);
            if( systemModule )
            {
                description.makeContent[ syntax ] = systemModule.content;
                description.path = systemModule.path;
                description.deps=systemModule.deps;
            }
        }
