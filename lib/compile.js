...
Iteration.prototype.nextIn=function(stack)
{
    ...
    if( infor )
    {
        ...
        if( !property.hasSyntaxRemove )
        {
            ...
            _value = parse( this.module, property);
        }
    ...
    if( it.current.content().length > 0 )
    {
        ...
        while (it.seek())
        {
            if (it.current instanceof Ruler.STACK)
            {
                ...
                //如果是跨模块的引用
                /*if( desc && desc.nonglobal && desc.owner !== it.module.fullclassname )
                {
                    var refmodule = getmodule( desc.owner , it.module );
                    if( refmodule.import && refmodule.import.hasOwnProperty( acceptType ) )
                    {
                        acceptType = refmodule.import[ acceptType ];
                    }
                }*/
...
function getClassTypeByDefinedFullName(it, targetProps )
{
    if( it.current.type==='(identifier)' )
    {
        ...
        if( classType && !getmodule( classType,  it.module ) && !getDefinedClassName( it.module , classType ) )
        {
            try
            {
                var loadModule = Compile.loadModuleDescription(globalsConfig.syntax, classType, globalsConfig, classType, null, false , it.module );
                if ( !loadModule )
                {
...
function isConformRunPlatform(desc,syntax)
{
    ...
    {
        var runPlatform =defineMetaTypeList.RunPlatform.param.source;
...
function captureRunPlatformMetatype(it, desc, classmodule )
{
    ...
    var conform = isConformRunPlatform(desc, classmodule.currentMakeSyntax );
    if( conform ){
        return false;
    }
...
function compilePolicy(it,desc,classmodule)
{
    ...
    it.seek();
    it.seek();
    var sid = it.stack.scope().keyword();
    if( sid !== "if" )
    {
        throw new SyntaxError("the "+method+" directive can only appear in if expression");
    }
    var express =  it.stack.content();
    try{
        if( express[1].keyword() !=="object" )throw new SyntaxError("Invalid directive");
        if( !express[1].content()[1].content()[0]  )throw new SyntaxError("Missing expression");
    }catch (e){
        throw new SyntaxError("Invalid directive the "+method );
    }
    express = express[1].content()[1].content()[0].value.replace(/^[\'\"]|[\'\"]$/g,'');
    express = express.toLowerCase();
    if( method==="RunPlatform" && !(express==="server" || express==="client") )
    {
        throw new SyntaxError("The RunPlatform optional expression is server or client");
    }

    if( method==="Syntax" && globalsConfig.syntax_supported[ express ] !== true )
    {
    ...
    var parentScope = it.stack.scope().parent();
    var content = parentScope.content();
    var indexAt = content.indexOf( scope );
    var hasElse = content[indexAt+1] instanceof Ruler.STACK && content[indexAt+1].keyword().toLowerCase() === "else";
    var ifTrue =  false;
    if( method==="RunPlatform" ){
        ifTrue = express ==="client" && isClient || express ==="server" && !isClient;
    }else if( method==="Syntax" ){
        ifTrue = express === syntax;
    }
    ...
    var done = function (e) {
        ...
        if( id==="if" )
        {
            if( ifTrue )
            {
                var len = this.endIdentifer ? 4 : 5;
                e.iteration.content.splice(0, len);
    ...
    if( hasElse && ( !(content[indexAt+2] instanceof Ruler.STACK) || content[indexAt+2].keyword().toLowerCase() !=="if" ) )
    {
...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    ////捕获指定运行在客户端平台的元数据
    var hasSyntaxRemove = captureRunPlatformMetatype( it, desc , classmodule );
    if( hasSyntaxRemove )
    {
        property.hasSyntaxRemove = hasSyntaxRemove;
    }
    ...
    while ( it.next )
    {
        ...
        } else if( it.next.id === '(identifier)' || (it.next.id === '(keyword)' && it.current.value==='.') )
        {
            ...
            hasSyntaxRemove = captureRunPlatformMetatype( it, desc , classmodule );
            if( hasSyntaxRemove )
            {
                property.hasSyntaxRemove = hasSyntaxRemove;
            }
...
function makeModule(syntax, stack , classModule, config , force )
{
    ...
    //是否一致的编译语法
    //如果不一致则不需要编译
    if( !isConformRunPlatform(classModule, syntax ) )
    {
