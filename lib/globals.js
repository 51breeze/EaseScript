var descriptor = {
    'Class':{'id':'class', 'type':'Class', 'inherit':'Object'},
    'Namespace':{
        'id':'class', 'type':'Namespace', 'inherit':'Object',
        'static': {
            'valueOf': {'id': 'function', 'type': 'String'},
        }
    },
    'Interface':{'id':'class', 'type':'Interface', 'inherit':'Object'},
    'Number':{
        'id':'class', 'type':'Number', 'inherit':'Object',
        'static':{
            'EPSILON':{'id':'const','type':'Number'},
            'MAX_SAFE_INTEGER':{'id':'const','type':'Number'},
            'MAX_VALUE':{'id':'const','type':'Number'},
            'MIN_SAFE_INTEGER':{'id':'const','type':'Number'},
            'MIN_VALUE':{'id':'const','type':'Number'},
            'NEGATIVE_INFINITY':{'id':'const','type':'Number'},
            'NaN':{'id':'const','type':'Number'},
            'POSITIVE_INFINITY':{'id':'const','type':'Number'},
            'isFinite':{'id':'function','type':'Number','param':[] },
            'isInteger':{'id':'function','type':'Number','param':[] },
            'isNaN':{'id':'function','type':'Number','param':[] },
            'isSafeInteger':{'id':'function','type':'Number','param':[] },
            'parseFloat':{'id':'function','type':'Number','param':[] },
            'parseInt':{'id':'function','type':'Number','param':[] },
        },
        'proto':{
            'toExponential':{'id':'function','type':'Number','param':[] },
            'toFixed':{'id':'function','type':'Number','param':[] },
            'toLocaleString':{'id':'function','type':'Number','param':[] },
            'toPrecision':{'id':'function','type':'Number','param':[] },
            'toString':{'id':'function','type':'Number','param':[] },
            'valueOf':{'id':'function','type':'Number','param':[] },
        }
    },
    'String':{
        'id':'class', 'type':'String', 'inherit':'Object',
        'static': {
            'fromCharCode': {'type': 'int', 'id': 'function','param':['...']},
        },
        'proto': {
            'length': {'type': 'int', 'id': 'const'},
            'replace':{ 'id':'function','type':'String','param':['*','*']},
            'match': {'type': 'Array', 'id': 'function','param':['*']},
            'matchAll': {'type': 'Array', 'id': 'function','param':['*']},
            'search': {'type': 'int', 'id': 'function','param':['*']},
            'charAt': {'type': 'String', 'id': 'function','param':['int']},
            'charCodeAt': {'type': 'int', 'id': 'function','param':['int']},
            'concat': {'type': 'String', 'id': 'function','param':['...']},
            'indexOf': {'type': 'int', 'id': 'function','param':['String']},
            'lastIndexOf': {'type': 'int', 'id': 'function','param':['String','int'], required:[true,false]},
            'localeCompare': {'type': 'int', 'id': 'function','param':['String','String']},
            'repeat': {'type': 'String', 'id': 'function','param':['int']},
            'slice': {'type': 'String', 'id': 'function','param':['int']},
            'substr': {'type': 'String', 'id': 'function','param':['int']},
            'substring': {'type': 'String', 'id': 'function','param':['int']},
            'toLocaleLowerCase': {'type': 'String', 'id': 'function','param':[]},
            'toLowerCase': {'type': 'String', 'id': 'function','param':[]},
            'toLocaleUpperCase': {'type': 'String', 'id': 'function','param':[]},
            'toUpperCase': {'type': 'String', 'id': 'function','param':[]},
            'split': {'type': 'Array', 'id': 'function','param':['*']},
            'trim': {'type': 'String', 'id': 'function','param':[]},
            'trimRight': {'type': 'String', 'id': 'function','param':[]},
            'trimLeft': {'type': 'String', 'id': 'function','param':[]},
            'includes': {'type': 'Boolean', 'id': 'function','param':['String']},
            'padEnd': {'type': 'Boolean', 'id': 'function','param':['int','String'],required:[true,false]},
            'padStart': {'type': 'Boolean', 'id': 'function','param':['int','String'],required:[true,false]},
        }
    },
    'JSON':{
        'id':'class', 'type':'JSON','inherit':'Object',
        'static': {
            'parse': {'type': 'Object', 'id': 'function', param: ['String']},
            'stringify': {'type': 'String', 'id': 'function', param: ['Object']},
        },
    },
    'Object':{
        'id':'class',
        'type':'Object',
        'static': {
            'create': {'type': 'Object', 'id': 'function', param: []},
            'defineProperty': {'type': 'Object', 'id': 'function', param: []},
            'defineProperties': {'type': 'Object', 'id': 'function', param: []},
            'getPrototypeOf': {'type': 'Object', 'id': 'function', param: []},
            'setPrototypeOf': {'type': 'Object', 'id': 'function', param: []},
            'merge': {'type': 'JSON', 'id': 'function', param: ["*","Object"]},
            'forEach': {'type': 'void', 'id': 'function', param: ["Object","Function","Object"],'required':[true,true,false],"owner":"Object.forEach" },
            'toString': {'type': 'String', 'id': 'function', param: []},
            'valueOf': {'type': '*', 'id': 'function', param: []},
            'keys': {'type': 'Array', 'id': 'function', param: ["Object"]},
            'values': {'type': 'Array', 'id': 'function', param: ["Object"]},
            'entries': {'type': 'Array', 'id': 'function', param: ["Object"]}
        },
        'proto':{
            'constructor':{'type': 'Function', 'id': 'const'},
            'hasOwnProperty': {'type': 'Boolean', 'id': 'function', param: []},
            'isPrototypeOf': {'type': 'Boolean', 'id': 'function', param: []},
            'propertyIsEnumerable': {'type': 'Boolean', 'id': 'function', param: []},
            'setPropertyIsEnumerable': {'type': 'void', 'id': 'function', param: []},
            'toSource': {'type': 'String', 'id': 'function', param: []},
            'toLocaleString': {'type': 'String', 'id': 'function', param: []},
            'toString': {'type': 'String', 'id': 'function', param: []},
            'valueOf': {'type': '*', 'id': 'function', param: []},
       }
    },
    'RegExp':{
        'id':'class', 'type':'RegExp','inherit':'Object',
        'static':{
            'lastIndex': {'type': 'int', 'id': 'const'},
            '$1': {'type': 'String', 'id': 'const'},
            '$2': {'type': 'String', 'id': 'const'},
            '$3': {'type': 'String', 'id': 'const'},
            '$4': {'type': 'String', 'id': 'const'},
            '$5': {'type': 'String', 'id': 'const'},
            '$6': {'type': 'String', 'id': 'const'},
            '$7': {'type': 'String', 'id': 'const'},
            '$8': {'type': 'String', 'id': 'const'},
            '$9': {'type': 'String', 'id': 'const'},
        },
        'proto':{
            'test': {'type': 'Boolean', 'id': 'function', 'param': ['String']},
            'exec': {'type': 'Array', 'id': 'function', 'param': ['String']},
        }
    },
    'Error':{
        'id':'class', 'type':'Error','inherit':'Object',
        'proto':{
            'message': {'type': 'String', 'id': 'const'},
            'name': {'type': 'String', 'id': 'const'},
            'filename': {'type': 'String', 'id': 'const'},
            'line': {'type': 'String', 'id': 'const'},
        }
    },
    'EvalError':{'id':'class', 'type':'EvalError','inherit':'Error'},
    'RangeError':{'id':'class', 'type':'RangeError','inherit':'Error'},
    'ReferenceError':{'id':'class', 'type':'ReferenceError','inherit':'Error'},
    'SyntaxError':{'id':'class', 'type':'SyntaxError','inherit':'Error'},
    'TypeError':{'id':'class', 'type':'TypeError','inherit':'Error'},
    'URIError':{'id':'class', 'type':'URIError','inherit':'Error'},
    'Function':{
        'id':'class', 'type':'Function','inherit':'Object',
        'proto':{
            'call':{'id':'function','type':'*','param':['Object','...'],'required':[true] },
            'apply':{'id':'function','type':'*','param':['Object','Array'],'required':[true,true]},
            'bind':{'id':'function','type':'Function','param':['Object','...'],'required':[true]},
        }
    },
    'Date':{
        'id':'class', 'type':'Date','inherit':'Object',
        'static':{
            'UTC':{'id':'function','type':'Number','param':['uint','uint','uint','uint','uint','uint','uint'],'required':[true,true,false,false,false,false,false]},
            'now':{'id':'function','type':'Number','param':[] },
            'parse':{'id':'function','type':'Number','param':['String'] },
        },
        'proto':{
            'getDate':{'id':'function','type':'Number','param':[] },
            'getDay':{'id':'function','type':'Number','param':[]},
            'getYear':{'id':'function','type':'Number','param':[]},
            'getFullYear':{'id':'function','type':'Number','param':[]},
            'getHours':{'id':'function','type':'Number','param':[]},
            'getMilliseconds':{'id':'function','type':'Number','param':[]},
            'getMinutes':{'id':'function','type':'Number','param':[]},
            'getMonth':{'id':'function','type':'Number','param':[]},
            'getSeconds':{'id':'function','type':'Number','param':[]},
            'getTime':{'id':'function','type':'Number','param':[]},
            'getTimezoneOffset':{'id':'function','type':'Number','param':[]},
            'getUTCDate':{'id':'function','type':'Number','param':[]},
            'getUTCDay':{'id':'function','type':'Number','param':[]},
            'getUTCFullYear':{'id':'function','type':'Number','param':[]},
            'getUTCHours':{'id':'function','type':'Number','param':[]},
            'getUTCMilliseconds':{'id':'function','type':'Number','param':[]},
            'getUTCMinutes':{'id':'function','type':'Number','param':[]},
            'getUTCMonth':{'id':'function','type':'Number','param':[]},
            'getUTCSeconds':{'id':'function','type':'Number','param':[]},
            'setDate':{'id':'function','type':'void','param':['uint'] },
            'setDay':{'id':'function','type':'void','param':['uint']},
            'setYear':{'id':'function','type':'void','param':['uint']},
            'setFullYear':{'id':'function','type':'void','param':['uint']},
            'setHours':{'id':'function','type':'void','param':['uint']},
            'setMilliseconds':{'id':'function','type':'void','param':['uint']},
            'setMinutes':{'id':'function','type':'void','param':['uint']},
            'setMonth':{'id':'function','type':'void','param':['uint']},
            'setSeconds':{'id':'function','type':'void','param':['uint']},
            'setTime':{'id':'function','type':'void','param':['uint']},
            'setTimezoneOffset':{'id':'function','type':'void','param':['uint']},
            'setUTCDate':{'id':'function','type':'void','param':['uint']},
            'setUTCDay':{'id':'function','type':'void','param':['uint']},
            'setUTCFullYear':{'id':'function','type':'void','param':['uint']},
            'setUTCHours':{'id':'function','type':'void','param':['uint']},
            'setUTCMilliseconds':{'id':'function','type':'void','param':['uint']},
            'setUTCMinutes':{'id':'function','type':'void','param':['uint']},
            'setUTCMonth':{'id':'function','type':'void','param':['uint']},
            'setUTCSeconds':{'id':'function','type':'void','param':['uint']},
            'toDateString':{'id':'function','type':'String','param':[]},
            'toGMTString':{'id':'function','type':'String','param':[]},
            'toISOString':{'id':'function','type':'String','param':[]},
            'toJSON':{'id':'function','type':'String','param':[]},
            'toLocaleDateString':{'id':'function','type':'String','param':[]},
            'toLocalString':{'id':'function','type':'String','param':[]},
            'toLocalTimeString':{'id':'function','type':'String','param':[]},
            'toString':{'id':'function','type':'String','param':[]},
            'toUTCString':{'id':'function','type':'String','param':[]},
            'valueOf':{'id':'function','type':'Number','param':[]},
            'toTimeString':{'id':'function','type':'String','param':[]},
        }
    },
    'boolean':{
        'id':'class',
        'type':'Boolean',
        'inherit':'Object'
    },
    'Boolean':{
        'id':'class',
        'type':'Boolean',
        'inherit':'Object'
    },
    /*'Symbol':{'id':'class', 'type':'Symbol','inherit':'Object'},*/
    'ListIterator':{
        'id':'class', 'type':'ListIterator','inherit':'Object',
        'proto':{
            'current':{'id':'function','type':'*'},
            'key':{'id':'function','type':'*'},
            'next':{'id':'function','type':'void'},
            'rewind':{'id':'function','type':'void'},
            'valid':{'id':'function','type':'Boolean'},
        }
    },
    'Reflect':{
        'id':'class', 'type':'Reflect','inherit':'Object',
        'static':{
            'apply':{'id':'function','type':'*','param':['Function','...']},
            'call':{'id':'function','type':'*','param':['Function','...']},
            'construct':{'id':'function','type':'*','param':['Class','...']},
            'deleteProperty':{'id':'function','type':'Boolean','param':['Object','String']},
            'has':{'id':'function','type':'Boolean','param':[]},
            'get':{'id':'function','type':'*','param':['Class','Object','String'],'required':[false,true,true] },
            'set':{'id':'function','type':'*','param':['Class','Object','String','*'],'required':[false,true,true,true]},
            'type':{'id':'function','type':'*','param':['Object','Class']},
        }
    }, 
    'Event':{
        'id':'class', 'type':'Event','inherit':'Object',
        'static':{
            "SUBMIT":{'id':'const','type':'String'},
            "RESIZE":{'id':'const','type':'String'},
            "FETCH":{'id':'const','type':'String'},
            "UNLOAD":{'id':'const','type':'String'},
            "LOAD":{'id':'const','type':'String'},
            "RESET":{'id':'const','type':'String'},
            "FOCUS":{'id':'const','type':'String'},
            "BLUR":{'id':'const','type':'String'},
            "ERROR":{'id':'const','type':'String'},
            "COPY":{'id':'const','type':'String'},
            "BEFORECOPY":{'id':'const','type':'String'},
            "CUT":{'id':'const','type':'String'},
            "BEFORECUT":{'id':'const','type':'String'},
            "PASTE":{'id':'const','type':'String'},
            "BEFOREPASTE":{'id':'const','type':'String'},
            "SELECTSTART":{'id':'const','type':'String'},
            "READY":{'id':'const','type':'String'},
            "SCROLL":{'id':'const','type':'String'},
            "ANIMATION_START":{'id':'const','type':'String'},
            "ANIMATION_END":{'id':'const','type':'String'},
            "ANIMATION_ITERATION":{'id':'const','type':'String'},
            "TRANSITION_END":{'id':'const','type':'String'},
            "INITIALIZE_COMPLETED":{'id':'const','type':'String'}
        },
        'proto':{
            'preventDefault':{'id':'function','type':'void','param':[]},
            'stopPropagation':{'id':'function','type':'void','param':[]},
            'stopImmediatePropagation':{'id':'function','type':'void','param':[]},
            "bubbles":{'id':'const','type':'Boolean'},
            "cancelable":{'id':'const','type':'Boolean'},
            "currentTarget":{'id':'const','type':'Object'},
            "target":{'id':'const','type':'Object'},
            "defaultPrevented":{'id':'const','type':'Boolean'},
            "originalEvent":{'id':'var','type':'Object'},
            "type":{'id':'const','type':'String'},
            "propagationStopped":{'id':'const','type':'Boolean'},
            "immediatePropagationStopped":{'id':'const','type':'Boolean'},
            "altkey":{'id':'const','type':'Boolean'},
            "button":{'id':'const','type':'Boolean'},
            "ctrlKey":{'id':'const','type':'Boolean'},
            "shiftKey":{'id':'const','type':'Boolean'},
            "metaKey":{'id':'const','type':'Boolean'},
        }
    },
    'EventDispatcher':{
        'id':'class', 'type':'EventDispatcher','inherit':'Object',
        'proto':{
            'dispatchEvent':{'id':'function','type':'Boolean','param':['Event']},
            'removeEventListener':{'id':'function','type':'Boolean','param':['String','Function'], 'required':[true,false] },
            'addEventListener':{'id':'function','type':'EventDispatcher','param':['String','Function','Boolean','int','Object'],
                                'required':[true,true,false,false,false],"owner":"EventDispatcher.proto.addEventListener"},
            'hasEventListener':{'id':'function','type':'Boolean','param':['String','Function'], 'required':[true,false]},
        }
    },
    'Array':{
        'id':'class',
        'type':'Array',
        'inherit':'Object',
        'static':{
            'from':{'id':'function','type':'Array','param':['*','Function','Object'],'required':[true,false,false]},
            'of':{'id':'function','type':'Array','param':['...']},
            'isArray':{'id':'function','type':'Boolean','param':['*'],'required':[true]},
        },
        'proto':{
            'length':{'type':'int','id':'const'},
            'concat':{'type':'Array','id':'function','param':['...']},
            'fill':{'type':'Array','id':'function','param':['Object','int','int']},
            'filter':{'type':'Array','id':'function','param':['Function']},
            'find':{'type':'*','id':'function','param':['Function','*']},
            'forEach':{'type':'Array','id':'function','param':['Function','Object'],'required':[true,false], "owner":"Array.proto.forEach" },
            'indexOf':{'type':'int','id':'function','param':['*']},
            'join':{'type':'String','id':'function','param':['String']},
            'lastIndexOf':{'type':'int','id':'function','param':['Object']},
            'map':{'type':'Array','id':'function','param':['Function']},
            'pop':{'type':'*','id':'function','param':[]},
            'push':{'type':'int','id':'function','param':['*']},
            'shift':{'type':'*','id':'function','param':[]},
            'slice':{'type':'Array','id':'function','param':['...']},
            'some':{'type':'Boolean','id':'function','param':['Function']} ,
            'sort':{'type':'Array','id':'function','param':['Function']},
            'splice':{'type':'Array','id':'function','param':['int','...']},
            'toLocaleString':{'type':'String','id':'function','param':[]},
            'unshift':{'type':'*','id':'function','param':['*']},
            'toString':{'type':'String','id':'function','inherit':'Object','param':[]},
            'unique':{ 'id':'function','type':'Array'}
        },
    },
    'Math':{
         'id':'class',
         'type':'Math',
         'static': {
            'E': {type: 'Number', id: 'const'},
            'LN10': {type: 'Number', id: 'const'},
            'LN2': {type: 'Number', id: 'const'},
            'LOG10E': {type: 'Number', id: 'const'},
            'LOG2E': {type: 'Number', id: 'const'},
            'PI': {type: 'Number', id: 'const'},
            'SQRT1_2': {type: 'Number', id: 'const'},
            'SQRT2': {type: 'Number', id: 'const'},
            'abs': {type: 'Number', id: 'function', param: ['Number']},
            'acos': {type: 'Number', id: 'function', param: ['Number']},
            'asin': {type: 'Number', id: 'function', param: ['Number']},
            'atan2': {type: 'Number', id: 'function', param: ['Number']},
            'ceil': {type: 'Number', id: 'function', param: ['Number']},
            'cos': {type: 'Number', id: 'function', param: ['Number']},
            'exp': {type: 'Number', id: 'function', param: ['Number']},
            'floor': {type: 'Number', id: 'function', param: ['Number']},
            'log': {type: 'Number', id: 'function', param: ['Number']},
            'max': {type: 'Number', id: 'function', param: ['Number']},
            'min': {type: 'Number', id: 'function', param: ['Number']},
            'pow': {type: 'Number', id: 'function', param: ['Number']},
            'random': {type: 'Number', id: 'function', param: []},
            'round': {type: 'Number', id: 'function', param: ['Number']},
            'sin': {type: 'Number', id: 'function', param: ['Number']},
            'sqrt': {type: 'Number', id: 'function', param: ['Number']},
            'tan': {type: 'Number', id: 'function', param: ['Number']},
         }
    },
    'arguments':{
        'id':'object',
        'type':'arguments',
        'notLoadFile':true,
        'static': {
            'callee': {type: '*', id: 'function'},
            'caller': {type: '*', id: 'function', 'state':'delete'},
            'length': {type: 'int', id: 'const'},
        }
    },
    'Float':{
        'id':'class',
        'type':'Float',
        'inherit':'Number',
    },
    'float':{
        'id':'object',
        'type':'Float',
    },
    'Double':{
        'id':'class',
        'type':'Double',
        'inherit':'Number',
    },
    'double':{
        'id':'object',
        'type':'Double',
    },
    'Integer':{
        'id':'object',
        'type':'Integer',
        'inherit':'Number',
        'static': {
            'MAX': {type: 'Number', id: 'const',value:2147483647},
            'MIN': {type: 'Number', id: 'const',value:-2147483648},
        }
    },
    'int':{
        'id':'object',
        'type':'Integer'
    },
   'uint':{
        'id':'object',
        'type':'Integer',
    },
    'console':{
        'id':'class',
        'type':'console',
        'static': {
            'log': {type: 'void', id: 'function', param: ['...']},
            'error': {type: 'void', id: 'function', param: ['...']},
            'info': {type: 'void', id: 'function', param: ['...']},
            'warn': {type: 'void', id: 'function', param: ['...']},
            'dir': {type: 'void', id: 'function', param: ['*']},
            'trace': {type: 'void', id: 'function', param: ['*']},
            'time': {type: 'void', id: 'function', param: ['String']},
            'timeEnd': {type: 'void', id: 'function', param: ['String']},
            'assert': {type: 'void', id: 'function', param: ['*']},
        }
    },
    'System':{
        'id':'class',
        'type':'System',
        'static': {
            'env':{
                'id':'object',
                'type':'Object',
                'notCheckType':true,
                'static': {
                    'BROWSER_IE': {type: 'String', id: 'const'},
                    'BROWSER_FIREFOX': {type: 'String', id: 'const'},
                    'BROWSER_CHROME': {type: 'String', id: 'const'},
                    'BROWSER_OPERA': {type: 'String', id: 'const'},
                    'BROWSER_SAFARI': {type: 'String', id: 'const'},
                    'BROWSER_MOZILLA': {type: 'String', id: 'const'},
                    'NODE_JS': {type: 'String', id: 'const'},
                    'platform': {type: 'String', id: 'function', param: ['...']},
                    'version': {type: 'Number', id: 'function', param: ['...']}
                }
            },
            'getDefinitionByName': {type: 'Class', id: 'function', param: ['String']},
            'getQualifiedClassName': {type: 'String', id: 'function', param: ['Object']},
            'getQualifiedObjectName': {type: 'String', id: 'function', param: ['Object']},
            'getQualifiedSuperclassName': {type: 'String', id: 'function', param: ['Object']},
            'isSubClassOf': {type: 'Boolean', id: 'function', param: ['Class','Class']},
            'isObject': {type: 'Boolean', id: 'function', param: ['*']},
            'isArray': {type: 'Boolean', id: 'function', param: ['*']},
            'isString': {type: 'Boolean', id: 'function', param: ['*']},
            'isBoolean': {type: 'Boolean', id: 'function', param: ['*']},
            'isDefined': {type: 'Boolean', id: 'function', param: ['...']},
            'isFunction': {type: 'Boolean', id: 'function', param: ['*']},
            'isScalar': {type: 'Boolean', id: 'function', param: ['*']},
            'isNumber': {type: 'Boolean', id: 'function', param: ['*']},
            'isEmpty': {type: 'Boolean', id: 'function', param: ['*']},
            'trim': {type: 'String', id: 'function', param: ['String']},
            'uid': {type: 'String', id: 'function', param: []},
            'range': {type: 'Array', id: 'function', param: ['int','int']},
            'serialize':{type: 'String', id: 'function', param: ['*']},
            'getGlobalEvent':{type: 'EventDispatcher', id: 'function', param: []},
            'getContext':{type: 'Application', id: 'function', param: []},
            'environments':{type: 'Object', id: 'function', param: ["String"], required:[false]},

            //系统全局方法
            'decodeURI':{type:'String',id:'function',param:['String']},
            'decodeURIComponent':{type:'String',id:'function',param:['String']},
            'encodeURI':{type:'String',id:'function',param:['String']},
            'encodeURIComponent':{type:'String',id:'function',param:['String']},
            'escape':{type:'String',id:'function',param:['String']},
            'eval':{type:'*',id:'function',param:['String']},
            'isFinite':{type:'Boolean',id:'function',param:['*']},
            'isNaN':{type:'Boolean',id:'function',param:['*']},
            'parseFloat':{type:'float',id:'function',param:['*']},
            'parseInt':{type:'int',id:'function',param:['*']},
            'unescape':{type:'String',id:'function',param:['String']},
            'setTimeout':{type:'int',id:'function',param:['Function','Number']},
            'setInterval':{type:'int',id:'function',param:['Function','Number']},
            'clearTimeout':{type:'int',id:'function',param:['int']},
            'clearInterval':{type:'int',id:'function',param:['int']},
            'hotUpdate':{type:'void',id:'function',param:['Object','Function']},
        }
    },
    "Map":{
        'id':'class',
        'type':'Map',
        'static': {
        },
        'proto':{
            "get":{type:'Object',id:'function',param:['Object']},
            "set":{type:'*',id:'function',param:['Object','*']},
            "entries":{type:'ListIterator',id:'function',param:[]},
            "keys":{type:'Array',id:'function',param:[]},
            "values":{type:'Array',id:'function',param:[]},
            "delete":{type:'Object',id:'function',param:['Object']},
            "forEach":{type:'void',id:'function',param:['Function','Object'], required:[true,false]},
            "size":{type:'int',id:'function',param:[]},
            "has":{type:'Boolean',id:'function',param:[]},
        }
    }
};

module.exports  = descriptor;