...
const SyntaxBuilder={
    ...
    'javascript':function(config, descriptions, default_bootstrap_class, onlyClient, resovle, reject )
    {
       ...
       //加载css文件的路径
        ...
        var buildEach=function(index, callback, resovle, reject)
        {
            ...
            buildStyle(skinModules, lessPath, options, config, function(err, output)
            {
                ...
                } else if(output)
                {
                    ...
                    config.__buildContentMap__[ filename ]={
                        content:output.css,
                        file:filename,
                        name:outputname,
                        suffix:".css",
                ...
                config.__buildContentMap__[ filepath ]={
        ...
        var buildSystem = function(resovle, reject)
        {
            ...
            if( doPart === true )
            {
                ...
                config.__buildContentMap__[ baseScriptFile ]={
                    content:scriptContent,
                    file:baseScriptFile,
                    name:"easescript",
                    suffix:".js",
    ...
    try
    {
        ...
        if( config.watching === true )
        {
            if( !watching )
            {
                ...
                watching = true;
                Watch.start({files:[config.workspace, PATH.join(config.project_path,'.esconfig')],
                match:/\.(es|css|less|html|esconfig)$/i},
                function(filename, stat){
                    remake(filename, config, invoke );
                });
            }
        ...
        //构建的内容
        var buildContent = {};
        config.__buildContentMap__=buildContent;
        ...
        for(;i<len;i++)
        {
            ...
            var classname = PATH.relative(project_path, _filepath ).replace(/\\/g,'/').replace(/\//g,'.');
        ...
        var default_bootstrap_class = config.default_bootstrap_class;

        //是否有找到指定的入口类
        if( default_bootstrap_class && !bootstrap_class[0] )
        {
            throw new ReferenceError("Not found assign bootstrap class. for '"+default_bootstrap_class+"'");
        }

        //默认入口类
        if( !default_bootstrap_class )
        {
            default_bootstrap_class = bootstrap_class.filter(function (a) {
                return !!a;
            })[0];
        }

        //如果指定默认的入口类
        if( default_bootstrap_class && Maker.localDescriptions[  default_bootstrap_class ] )
        {
            var bootstrapClassModule = Maker.localDescriptions[  default_bootstrap_class ];
            ...
            if( Maker.checkInstanceOf(bootstrapClassModule,"es.core.Application") )
            {
                ...
                {
                    throw new ReferenceError("Default bootstrap router method is not define '"+ default_bootstrap_class+"'");
                }
                config.default_bootstrap_router_provider=default_bootstrap_class+"@"+bootstrapClassModule.defineMetaTypeList.Router.param.default;
            }
        }
        ...
        if( descriptions.length > 0 )
        {
            ...
            {
                try{
                   SyntaxBuilder[ config.syntax ]( config , descriptions, default_bootstrap_class, false, resovle, reject);
                }catch(e){
                    reject(e);
                }
