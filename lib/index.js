...
const SyntaxBuilder={
    ...
    'php':function(config, descriptions, default_bootstrap_class, onlyServer, resovle, reject , doneCallback, clientDependencies )
    {
        ...
        const viewRoutes = Compile.getServiceRoutes( Compile.getUsedModules( 
            Object.entries(Maker.localDescriptions).map( item=>item[1] ).filter( item=>item.isApplication ),
            "javascript" 
        ));
    ...
    'node':function(config, descriptions, default_bootstrap_class, onlyServer, resovle, reject , doneCallback )
    {
        ...
        config.SERVER_PROVIDER_MODULES = Compile.getModulesWithPolicy( usedModules, Compile.MODULE_POLICY_LOCAL | Compile.MODULE_POLICY_CORE );
...
function getApplicationModules(config, modules, inherits, bootstrap_class)
{
     ...
     return modules.filter(function (a)
     {
         ...
         {
             //默认指定第一个为入口类
             if( bootstrap_class[2] === null && modules.length===1 )
             {
