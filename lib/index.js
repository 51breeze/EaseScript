...
const SyntaxBuilder={
    ...
    'javascript':function(config, descriptions, default_bootstrap_class, onlyClient, resovle, reject, doneCallback )
    {
        ...
        const completed = function(resovle, reject)
        {
            ...
            const makeService = ()=>{
                ...
                if( onlyClient !== true )
                {
                    const providerModuleList = config.server_render ? descriptions : [];
                    if( !config.server_render )
                    {
                        const providers = {};
    ...
    'php':function(config, descriptions, default_bootstrap_class, onlyServer, resovle, reject , doneCallback )
    {
        ...
        const callback=(results, error, clientBuild )=>{
            ...
            {
                const loadScripts = config.server_render_load_scripts || {};
                ...
                if( clientBuild )
                {
                    ...
                    scripts.forEach( (item)=>{
                        const scripts = (loadScripts[ item.name ]||[]).map( value=> value.replace(/\\/g,"/") );
                        loadScripts[ item.name ] = Array.from( ( new Set( scripts.concat(item.scripts) ) ).values() );
                    });
                }
...
function single(config, filename, syntax, output, invoke)
{
...
function rebuild(config ,filename, invoke)
{
    try{
        ...
        if( desc )
        {
            ...
            {
                try{
                    SyntaxBuilder[ config.syntax ]( config , [desc], null, !!config.only_current_syntax , resovle, reject, invoke);
                }catch(e){
                    reject(e);
                }
...
function outputDoneInfo( config )
{
    var output = `** Make path: ${config.build_path} **`;
    var title = `=====Make completed=====`;
    if( output.length < title.length)
    {
        output = output.padStart( output.length + (title.length - output.length) / 2 ,"*").padEnd( title.length , "*");
    }
    title = title.padStart( title.length + (output.length - title.length) / 2 ,"=").padEnd( output.length , "=");
    Utils.info(`\n${title}`);
    Utils.info(output);
    Utils.info( '='.repeat( output.length ) );
}
...
function make( config , doneCallback)
{
    ...
    const completed = function(results,error)
    {
        ...
        {
            outputDoneInfo(config);
        }
    };

    config = createConfigure( config );

    if( !config.originMakeSyntax )
    {
       config.originMakeSyntax = config.syntax;
    }
    ...
    if( descriptions.length > 0 )
    {
        ...
        {
            try{
                SyntaxBuilder[ config.syntax ]( config , descriptions, default_bootstrap_class[0], config.only_current_syntax , resovle, reject, completed);
            }catch(e){
                reject(e);
            }
