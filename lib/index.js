...
const SyntaxBuilder={
    ...
    'javascript':function(config, descriptions, default_bootstrap_class, onlyClient, resovle, reject, doneCallback )
    {
        ...
        const completed = function(resovle, reject)
        {
            ...
            const callback=function(resovle, reject)
            {
                resovle();
            ...
            const makeService = ()=>{
                ...
                if( onlyClient !== true )
                {
                    ...
                    {
                        SyntaxBuilder[config.service_provider_syntax](config,providerModuleList, default_bootstrap_class, true, resovle, reject, callback );
                    }else{
                        resovle( callback );
                    }
    ...
    'php':function(config, descriptions, default_bootstrap_class, onlyServer, resovle, reject , doneCallback)
    {
        ...
        //获取所有使用的模块
        usedModules = usedModules.concat( Compile.getUsedModules( allModules, "php" ) );
    ...
    'node':function(config, descriptions, default_bootstrap_class, onlyServer, resovle, reject , doneCallback)
    {
        ...
        //获取所有使用的模块
        usedModules = usedModules.concat( Compile.getUsedModules( allModules, "node" ) );
...
function createConfigure(config)
{
    ...
    project_path = typeof config.project ==="object" && config.project.path ? config.project.path : project_path;
    ...
    {
        config = mergeConfig ? globalConfig : JSON.parse( Utils.getContents( makefile ) );
    }
...
function make( config , doneCallback)
{
    ...
    const descriptions = getBootstrap(config);
