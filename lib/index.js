    ...
    'javascript':function(config, descriptions, default_bootstrap_class, onlyClient, resovle, reject, doneCallback )
    {
       ...
       /* less.functions.functionRegistry.add("superposition", superposition);
        ...
        const buildEach=function(index, callback, resovle, reject)
        {
            ...
            if( !onlyClient )
            {
                Object.assign(serviceProviders, getServiceProvider(config,usedModules).providers );
            }
            ...
            if( isCallback )
            {
                buildModule = buildContentModules[bootstrap.fullclassname]={
                    bootstrap:bootstrap,
                    context:[config.workspace],
                    output:[filepath],
                    dependencies:usedModules.filter(function(item){
                        return item.fullclassname !== bootstrap.fullclassname;
                    }),
            ...
            Less.render(styleContent, options, function(err, output){
                ...
                } else if(output)
                {
                    ...
                    if( buildModule )
                    {
                        buildModule.output.push( filename );
                        Utils.forEach(styles.assets,function(value,name)
                        {
                            buildModule.assets.push( value );
                        });
                    }
                }
                callback(resovle, reject);
        ...
        {
            resovle();
            isCallback && doneCallback( buildContentModules );
        }
...
function make( config , doneCallback, flag )
{
    ...
    //完成后回调
    const completed = typeof doneCallback === 'function' ? function(results){
        making = false;
