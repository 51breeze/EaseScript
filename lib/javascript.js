...
Iteration.prototype.parseArgumentsOfMethodAndConstructor=function( stack )
{
    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        var _items = items.map(function (a) {
            return buildSyntax.getVarReference( a );
        });

        this.content.push( _items.join(',') );
        for (var i in items) {
            ...
            if (desc.value instanceof Ruler.STACK)
            {
                ...
                express.unshift('if('+buildSyntax.getVarReference( items[i] )+' == null ){'+value+';}\n');
            }
            if ( desc.type !== '*' /*&& desc.type !=='Object'*/ )
            {
                ...
                if( globalsConfig.mode !== 3 )
                {
                    if (shortName.indexOf(".") > 0) {
                        express.push('if( !'+buildSyntax.isOf( buildSyntax.getVarReference(items[i]), buildSyntax.getDefinitionByName( desc.type ) )+')throw new TypeError("type does not match. must be ' + desc.type + '","' + this.module.filename + '",' + Iteration.lastStack.line + ');\n');
                    } else {
                        express.push('if( !' +buildSyntax.isOf( buildSyntax.getVarReference(items[i]), shortName) + ')throw new TypeError("type does not match. must be ' + shortName + '","' + this.module.filename + '",' + Iteration.lastStack.line + ');\n');
                    }
                }
...
function getClassTypeByDefinedFullName(it)
{
    if( it.current.type==='(identifier)' )
    {
        ...
        {
            try {
                globalsConfig.$loadModuleDescription(globalsConfig.syntax, classType, globalsConfig, globalsConfig.project, classType);
                if (!getmodule(classType)) {
            ...
            makeModule(stackModules[classType], stackModules[classType].description, globalsConfig);
    ...
    for ( ; i< len ; i++ )
    {
        ...
        if( item instanceof Ruler.STACK && item.keyword() !=='metatype' )
        {
            ...
            else if( item.keyword() === 'var' || item.keyword() === 'const' )
            {
                ...
                classProperties[ item.name() ]={ id:item.keyword(),isStatic:!!item.static(),ns:item.qualifier() ,value:ret};
...
function makeClassModule( o , config)
{
    ...
    if( o.inherit && o.id !== "namespace" )
    {
        ...
        {
            var iCode =  makeClassModule( inheritClass , config );
...
function start(config, makeModules, descriptions , bootstrap )
{
    ...
    globalsConfig = Utils.merge({},config);
    makeNamespaces = {};
    ...
    try {
        ...
        for (var m in localClass)
        {
            ...
            if ( isUsed === true)
            {
                ...
                {
                    var file = Utils.getResolvePath(config.build_path + '/src', fullclassname );
                    Utils.mkdir(file.substr(0, file.lastIndexOf("/")));
                    Utils.setContents(file + '.js', _loadModule.makeContent );
                }
