    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        for (var i in items) {
            ...
            if (desc.type !== '*' && desc.type !=='Object')
            {
                ...
                {
                    express.push('if(!System.is(' + items[i] + ', System.getDefinitionByName("'+desc.type+'")))throw new TypeError("type does not match. must be ' + desc.type + '");\n');
                }else{
                    express.push('if(!System.is(' + items[i] + ', '+shortName+'))throw new TypeError("type does not match. must be ' + shortName + '");\n');
                }
            }
        }
...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    while ( it.next )
    {
        if ( it.next instanceof Ruler.STACK )
        {
            ...
            }else if( it.current.type() === '(expression)' )
            {
                ...
                if( property.before[0] !=="new" && (desc && desc.id==="class") && property.thisArg !=="super" )
                {
                    classType = getImportClassByType(classmodule, desc.type);
                    refClassModule = getmodule(classType);
                ...
                }else if( property.before[0] === "new" && (desc.type ==="Class" || desc.type==="Function") )
                {
                    property.type = 'Object';
                    property.referenceType = 'Object';
                }
        ...
        } else if( it.next.id === '(identifier)' || (it.next.id === '(keyword)' && it.current.value==='.') )
        {
            ...
            if ( desc && desc.notCheckType !== true && !(desctype ==='*' || desctype ==='JSON' || desctype ==='Object') )
            {
                ...
                //如果只是对一个函数的引用
                if( desc.id==='function' && !property.accessor && !desc.isAccessor ) property.type = 'Function';
    ...
    if( it.next && it.next.value==='as' )
    {
          ...
          }else
          {
              ...
              {
                 if( !checkTypeOf(classmodule, typename, property.type, property ) )
                 {
                     error('type does not match. must be "'+typename+'"', 'type');
                 }
...
function operateMethodOfGet(classmodule, thisvrg, desc, props ,info, before)
{
    ...
    var method = globalsConfig.mode==1 ? classmodule.classname+'.__get__' : 'Reflect.get';
    ...
    if( before || desc.after )
    {
        ...
        if( before==="++" || desc.after==="++" )
        {
            method = globalsConfig.mode==1 ? classmodule.classname+'.__incre__' : 'Reflect.incre';

        }else if(  before==="--" || desc.after==="--" )
        {
            method = globalsConfig.mode==1 ? classmodule.classname+'.__decre__' : 'Reflect.decre';
        }
    }
    ...
    if( ns && ns.length>0 )
    {
        ...
        ns = ns.map(function (n) {
            return getNamespaceUriByModule(classmodule,n);
        });
...
function operateMethodOfSet(classmodule, thisvrg, desc, props ,info, value, operator )
{
    ...
    if( !checkRunning && desc.descriptor )
    {
        ...
        //成员私有属性
        if( desc.descriptor.id !== 'function' && desc.descriptor.privilege === "private" )
        {
            //静态属性
            if( desc.descriptor.isStatic )
            {
    ...
    var method = globalsConfig.mode==1 ? classmodule.classname+'.__set__' : 'Reflect.set';
...
function operateMethodOfApply(classmodule, thisvrg, desc, props ,info)
{
    ...
    //运行时检查
    var method = globalsConfig.mode==1 ? classmodule.classname+'.__call__' : 'Reflect.call';
    ...
    {
        method = globalsConfig.mode==1 ? classmodule.classname+'.__call__' : 'Reflect.apply';
    }
...
function operateMethodOfNew( classmodule, express, desc, info )
{
    ...
    var method = globalsConfig.mode==1 ? classmodule.classname+'.__construct__' : 'Reflect.construct';
    ...
    pushInfo(param, info, desc, express );
    return method+'('+param.join(',')+')';
}
...
function pushInfo(refArr, info, desc, express, classmodule )
{
    if( globalsConfig.mode==1 )
    {
        ...
        refArr.unshift( classmodule.classname );
    }
}
...
{
    var checkRunning = desc.runningCheck || globalsConfig.mode!=3;
    if( props.length === 0 )
    {
    ...
    if( !checkRunning && desc.descriptor )
    {
        ...
        return "Reflect.deleteProperty("+thisvrg+",'"+ props[0]+"')";
    }
    if( desc.super )
    {
        throw new SyntaxError("Unexpected super");
    }
    var method = globalsConfig.mode==1 ? classmodule.classname + '.__unset__' : 'Reflect.deleteProperty';
    var param = [thisvrg,props[0]];
    pushInfo(param, info, desc);
    return method+'('+param.join(',')+')';
}
...
function toString( stack, module, acceptType , fromParam )
{
    ...
    {
        return  formatCode( stack, expression( stack , module , acceptType, fromParam ) );
    }
...
function getNamespaceModuleValue( module, name)
{
    ...
    if( nsmodule && nsmodule.namespaces.hasOwnProperty(name) && nsmodule.namespaces[name].value  )
    {
...
function makeModule( stack , classModule, config )
{
    ...
    for (var j in classModule.import)
    {
        ...
        {
            var _stackModule = stackModules[ classModule.import[j] ];
...
function setClassModuleUsed(classModule, currentModule )
{
...
function getUseNamespace( desc )
{
    ...
    {
        return [desc.use];
    }else if( desc.descriptor && desc.descriptor.privilege )
    {
        return [desc.descriptor.privilege];
    }else if( desc.funScope && desc.funScope.define("use") )
    {
    ...
    for( var p in describe )
    {
        ...
        if (typeof desc.value === "object")
        {
            if (desc.value.get)
            {
                ...
                code.push(prefix + '.Get_' + nsuri+p + '={' + item.join(',') + '};\n');
            }

            if (desc.value.set)
            {
                item=['"value":'+(desc.varToBindable ? desc.value.set : desc.value.set.value) ];
                code.push(prefix + '.Set_' + nsuri+p + '={' + item.join(',') + '};\n');
            }
        ...
        }else
        {
            ...
            code.push(prefix + '.' + nsuri+p + '={' + item.join(',') + '};\n');
        }
    }
}
...
function toValue( describe, config , prefix, inherit, classModule )
{
    ...
    for( var p in describe )
    {
        ...
        {
            if( desc.id !== "function" && !desc.isStatic && desc.id !== "namespace" )
            {
                //如果是类成可见不用生成函数来代替
                if( desc.privilege==="private" )
                {
                    continue;
                }
            ...
            {
                if( desc.isStatic && desc.id !=="const" && desc.id !== "namespace" )
                {
...
function makeClassModule( o , config , codeModules )
{
    ...
    codeModules[ o.fullclassname ] =  config.context.defineModuleMethod+"("+JSON.stringify(import_values)+","+str+");\n";
...
function start(config, makeModules, descriptions , project )
{
    ...
    try {
        ...
        for (var i in makeModules)
        {
            ...
            Utils.info('  Making ' + description.filename);
