...
Iteration.prototype.parseArgumentsOfMethodAndConstructor=function( stack )
{
    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        for (var i in items)
        {
            ...
            if ( desc.type !== '*' /*&& desc.type !=='Object'*/ )
            {
                var _con = buildSyntax.getCheckMethodParamTypeValue( globalsConfig, items[i], shortName, this.module.filename, Iteration.lastStack.line );
                if( _con ) {
                    express.push(_con+ ";\n");
                }
            }
            paramTypeMap[ items[i] ]={type:shortName,value:value, check:!!_def };
        }
...
function getClassTypeByDefinedFullName(it)
{
    if( it.current.type==='(identifier)' )
    {
        ...
        if( classType && !getmodule( classType ) && !getDefinedClassName( it.module , classType ) )
        {
            try
            {
                var loadModule = globalsConfig.$loadModuleDescription(globalsConfig.syntax, classType, globalsConfig, globalsConfig.project, classType);
                if ( !loadModule )
                {
        ...
        if (isReference(it.current))
        {
            ...
            if( !desc && it.current.type==='(identifier)' )
            {
                ...
                if( type )
                {
                    ...
                    {
                        console.log( globalsConfig.syntax )
                    ...
                    if( type.indexOf(".") > 0 ){
                        property.thisArg = buildSyntax.getDefinitionByName( type );
                    }else{
    ...
    if( it.next && it.next.value==='as' )
    {
        ...
        }else
        {
            ...
            if( !isReferenceType( _reftype ) || !checkTypeOf(classmodule, typename, _reftype , origin ) )
            {
                ...
                property.thisArg = operateMethodOfCheckType(classmodule, typename.indexOf('.') >= 0 ?  buildSyntax.getDefinitionByName( typename ) : typename, property.thisArg, info);
                property.name = [];
            }
        }
        property.isAs = true;
...
function operateMethodOfGet(classmodule, thisvrg, desc, props ,info, before)
{
    ...
    return buildSyntax.buildExpressionGetOf(globalsConfig,classmodule, thisvrg, desc, props ,info, before,ns,useNs );
}
...
{
    var superModule = desc.super ?  getmodule( getImportClassByType(classmodule, desc.super) ) : null;
    var useNs = props.length > 0 ? getUseNamespace( desc ) : null;
    var ns = desc.use ?  desc.use : (desc.descriptor ? desc.descriptor.privilege : null);
    ns = getNamespaceUriByModule(classmodule, ns );
...
function operateMethodOfNew( classmodule, express, desc, info )
{
    ...
    if ( isReferenceType(referenceType) )
    {
        ...
        {
            instanceType = getmodule( getImportClassByType(classmodule, referenceType) );
        }
    }
...
function bunch(it, classmodule)
{
    ...
    while( it.next && Utils.isLeftAndRightOperator(it.next.value) && !Utils.isMathAssignOperator(it.next.value)  )
    {
        ...
        }else
        {
            ...
            if( operator==="+" )
            {
                operator = buildSyntax.getJointOperator(operator, leftType, value.type );
            }
...
function expression( stack, classmodule ,  acceptType , fromParam, fromReturn )
{
            ...
            if ( isReferenceType(_acceptType) && operator === '=' )
            {
                ...
                if ( valueType=== '*' )
                {
                    ...
                    if( fromParam !==true )
                    {
                        ...
                        value = operateMethodOfCheckType( classmodule,_acceptType.indexOf('.') >= 0 ? buildSyntax.getDefinitionByName(_acceptType) : _acceptType,value, info );
                    }
    ...
    if( isReferenceType(acceptType) )
    {
        ...
        if ( valueType === '*' )
        {
            ...
            if( fromParam !== true )
            {
                value = operateMethodOfCheckType( classmodule,acceptType.indexOf('.') >= 0 ? buildSyntax.getDefinitionByName(acceptType) : acceptType,value, info );
            }
        ...
        } else if ( !checkTypeOf(classmodule, acceptType, valueType, valueDescribe ) && !( stack.parent().type() ==='(property)' && valueType==='Number' ) )
        {
            ...
            {
                error( '"'+Utils.joinProperty( describe.info )+'" type does not match. must is "' + acceptType + '"', 'type');
            }
        }
    }
    return value;
...
function makeViewMetatype(config, currentModule, metatype, itemStack )
{
    ...
    config.$loadSkinModuleDescription( viewClass, config, config.project, config.syntax );
    var moduleObject = stackModules[ viewClass ];
    var description = moduleObject.description;
    var className = description.classname;
    if(  currentModule.import[ description.classname ] ){
    ...
    var done = function (e) {
        ...
        {
            code.push("var "+id+" = new "+className+".constructor(this)");
        }else{
 ...
 * @returns {string}
 */
function makeModule( stack , classModule, config , force )
{
    if( stack.__making__ === true && !force )
    {
        return classModule;
    }
    ...
    if( !(id==="class" || id==="interface") )
    {
        ...
        for(;j<len;j++)
        {
            if( content[j] instanceof Ruler.STACK )
            {
                ...
                {
                    makeModule( content[j] , classModule, config , true);
    ...
    if( useNamespace ) {
        ...
        {
            var _useNs = getmodule(getImportClassByType(classModule, dns));
    ...
    for (var j in _import )
    {
        ...
        } else if( classModule.useExternalClassModules.indexOf(_import[j]) >= 0 )
        {
            ...
            {
                makeModule(_stackModule, _stackModule.description, config, force);
            }
        }
    }
    return classModule;
    ...
    for(var p in declared )
    {
        if( declared[p].id==="class" )
        {
            ...
            if( codeModule )
            {
                ...
                {
                    str += syntax.defineProperty(o.classname, codeModule);
                }else{
                    str+=codeModule;
                }
            }
 ...
 * @returns {string}
 */
function makeClassModule( o , config, force )
{
    if( o.makeDone===true && !force )return '';
    o.makeDone = true;
    var internalCode = "";
    if( o.inherit && o.id !== "namespace" )
    {
        ...
        {
            var iCode =  makeClassModule( inheritClass , config, force );
    ...
    for (var i in o.import)
    {
        ...
        importMap[ i ] = buildSyntax.formatImportClassPath( o.import[i], o );
    }
    ...
    if( o.id==="namespace" )
    {
        ...
        for( var n in o.namespaces )
        {
            ...
            o.makeContent +=  buildSyntax.bulidNamespaceClassValueOf( n, fullname, o, importMap, o.namespaces[n].value , config );
        }
        return o.makeContent;
    }
    ...
    if( !o.isInternal )
    {
        ...
        {
            internalCode += o.rootContent.join("")+"\n";
        }
        internalCode += makePrivateModuleClass(o, config);
    }

    var inheritModule = o.inherit ?  getmodule( getImportClassByType(o, o.inherit) ) : null;
    propertiesAndMethod += buildSyntax.buildClassPropertiesAndMethod(o, inheritModule, config);
    ...
    return o.makeContent = buildSyntax.buildClassStructure( o.classname, o, inheritModule, importMap, propertiesAndMethod, internalCode, config, uri );
}

function fetchSystemAllRequirementsClass(seeMap, requirements )
{
    var len = seeMap.length;
    var i = 0;
    var results=[];
    var hash = {};
    for(;i<len;i++)
    {
        var module = getmodule( seeMap[i] );
        var externalClass = module.useExternalClassModules || [];
        var b = 0;
        var num = externalClass.length;
        for(;b<num;b++)
        {
            if( requirements[ externalClass[b] ] === true && !hash[ externalClass[b] ] )
            {
                hash[ externalClass[b] ] = true;
                results.push( externalClass[b] )
            }
        }
    }
    return results;
...
function fetchAllRequirementsClass( bootstrapModule , map )
{
    map = map || {};
    var imports = bootstrapModule.import;
    var items = [];
    var externalClass = bootstrapModule.useExternalClassModules || [];
    for( var i in imports )
    {
        if( map[ imports[i] ] !==true )
        {
            var importModule = getmodule(imports[i]);
            var isNs = importModule.id==="namespace";
            var hasUsed = isNs ? importModule.namespaces[ importModule.classname ].hasUsed : importModule.hasUsed;
            if( hasUsed && (isNs || externalClass.indexOf( imports[i] )>=0) )
            {
                map[ imports[i] ] = true;
                items = items.concat( fetchAllRequirementsClass(importModule, map) );
                items.push( imports[i] );
            }
        }
    }
    return items;
 ...
 * 开始生成代码片段
 */
function start(config, makeModules, descriptions , bootstrap )
{
    ...
    globalsConfig = Utils.merge(false,{},config);
    makeNamespaces = {};
    buildSyntax = require('./syntax/'+config.syntax+'.js');

    for ( var p in makeModules )
    {
        makeModules[p].__making__=false;
    }

    for ( var p in descriptions )
    {
        descriptions[p].makeDone=false;
        descriptions[p].__making__=false;
    }
    ...
    try {
        ...
        for (var srmc in config.system_require_main_class )
        {
            ...
            if( moduleObject )
            {
                ...
                systemRequireMainClass.push(config.system_require_main_class[srmc]);
                makeModule(moduleObject, description, config);
            }else{

                console.log( config.syntax )
                console.log( config.system_require_main_class[srmc] )
            }
        }

        moduleObject = makeModules[ bootstrap ];
        if( moduleObject )
        {
            description = moduleObject.description;
            makeModule(moduleObject, description, config);
        ...
        for (var p in descriptions)
        {
            if( descriptions[p].__making__ )
            {
                makeClassModule(descriptions[p] , config);
            }
        }

        var localClass = fetchAllRequirementsClass(description);
        localClass = systemRequireMainClass.concat( localClass );
        localClass.push( description.fullclassname );
        requirements = fetchSystemAllRequirementsClass(localClass,  requirements);
        localClass = localClass.filter(function (a) {
            var m = getmodule(a);
            return m.nonglobal;
        });

        var fileSuffix = config.syntax ==="javascript" ? ".js" : ".php";
        var code = [];
        for (var m in localClass)
        {
            var fullclassname = localClass[m];
            var _loadModule = getmodule( fullclassname );
            var isUsed = _loadModule.hasUsed;
            if ( isUsed === true)
            {
                if( config.source_file ==="enable" || config.syntax !=="javascript" )
                {
                    var bulid_path = config.syntax ==="javascript" ? config.build.child.js.path+"/src" : config.build.child.application.path
                    var file = Utils.getResolvePath( bulid_path , fullclassname );
                    Utils.mkdir(file.substr(0, file.lastIndexOf("/")));
                    Utils.setContents( file + fileSuffix, _loadModule.makeContent );
                }
                code.push( _loadModule.makeContent );
            }
        }
        ...
        return builder(bootstrap, config, code.join(""), requirements, {namespace:function(content) {
...
start.buildFragmentScript = function(config, makeModules, descriptions , bootstrapModule )
{
    defineModules = descriptions;
    stackModules  = makeModules;
    requirements = config.requirements || (config.requirements={});
    ...
    try {
        ...
        var description = bootstrapModule.description;
        makeModule(bootstrapModule, description, config);
        makeClassModule(description , config);
        return description.makeContent;
