...
Iteration.prototype.nextIn=function(stack)
{
    ...
    if( !infor )
    {
        ...
        if( desc && isstring )
        {
            ...
            if( isReferenceType(desctype) )
            {
                ...
                if( refObj.nonglobal ===true )
                {
                    ...
                    {
                        var prop = desc.thisArg === desctype ? "static" : "proto";
                        desc = getClassPropertyDescNS('', name.replace(/^[\'\"]+|[\'\"]+$/g, ""), prop, refObj, this.module, this, false, property);
                        desc = desc.length == 1 ? desc[0] : null;
                    }
                }else{
                    desc = refObj;
                }
        ...
        property = createDescription(this, this.module, property);
        property.descriptor = desc;
        property.name = [];
        property.runningCheck = !isstring;

        this.state=false;
        this.content.push({
            name:[],
            descriptor:null,
            thisArg: checkRunning(this.module, property, name , "in"),
            expression:false,
            before:[],
            after:'',
            "super":null,
...
function operateMethodOfDel(classmodule, thisvrg, desc, props ,info)
{
    ...
    if( !checkRunning && desc.descriptor )
    {
        props = createReferenceProps( props,  true );
        if( desc.funScope )
        {
