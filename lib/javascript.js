    ...
    if( stack.keyword() ==='statement' && stack.parent().keyword() === 'function' )
    {
        ...
        for (var i in items) {
            ...
            if (desc.type !== '*' && desc.type !=='Object')
            {
                ...
                {
                    express.push('if( '+items[i]+' && !System.is(' + items[i] + ', System.getDefinitionByName("'+desc.type+'")))throw new TypeError("type does not match. must be ' + desc.type + '");\n');
                }else{
                    express.push('if( '+items[i]+' && !System.is(' + items[i] + ', '+shortName+'))throw new TypeError("type does not match. must be ' + shortName + '");\n');
                }
            }
        }
    ...
    while ( it.next )
    {
        if ( it.next instanceof Ruler.STACK )
        {
            ...
            }else if( it.current.type() === '(expression)' )
            {
                ...
                //全局函数的访问器
                if( desc && !desc.owner && desc.id==="function" && desc.param instanceof Array && desc.param.length===1 )
                {
...
function expression( stack, classmodule ,  acceptType , fromParam, fromReturn )
{
    ...
    {
        var valueType = getDescribeType(valueDescribe);
        if( fromReturn && valueType ==="void" )
        {
            error( joinProperty( describe.info )+' cannot has returned', 'type');
        }
...
function checkTypeOf(classModule, needType, currentType, describeType, needDescribe )
{
    ...
    {
        //如当前值的类型是包裹容器验证不通过
        return ['String','Boolean','Number'].indexOf( currentType )<0;
    }
