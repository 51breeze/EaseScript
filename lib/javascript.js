...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    //抽象类不能实例化
    if( property.before[0] ==="new" && desc.id==="class" )
    {
...
function expression( stack, classmodule ,  acceptType , fromParam )
{
    ...
    }else
    {
        ...
        while (express.length > 0)
        {
            ...
            {
                if( describe.accessor !=='set' && describe.descriptor.fromVarToAccessor !==true && !describe.descriptor.value.set)
                {
...
function getClassPropertyDesc(it, refObj, name , classmodule , flag, propertyDesc )
{
    if( refObj )
    {
        ...
        if( desc )
        {
            if( isset && (desc.id ==="const" || ( desc.id==="function" && !desc.value.set ) ) )
            {
