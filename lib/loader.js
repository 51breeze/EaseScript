...
function invalid(filename)
{
    ...
    complier.single(config, ( module )=>{
        server_modules[ classname ] = module;
    }, filename, config.service_provider_syntax, true );
}
    ...
    if( hotUpdated[ classname ]===true )
    {
        ...
        complier.rebuild(config, resource ,function(results, error)
        {
            ...
            }else
            {
                ...
                if( config.SERVER_PROVIDER_MODULES )
                {
                    config.SERVER_PROVIDER_MODULES.map( module=>{
                        if( module.filename )
                        {
                            server_modules[ module.fullclassname ] = module;
                            //context.addDependency( path.resolve( module.filename ) );
                            //watch.start( module.filename, invalid );
                        }
                    });
                }
