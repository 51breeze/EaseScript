...
module.exports=function(content, map, meta)
{
    try
    {
        ...
        if( isSystem )
        {
            var data = builder.buildSystemModule(config, classname,  hotUpdated[ classname ] );
...
function getModuleContent(context, module)
{
    ...
    if( module.nonglobal )
    {
        ...
        //将模块中需要的样式文件打包在一起
        if( builder.makeModuleStyleContentExcludeFile(config, module) )
        {
    ...
    }else
    {
        ...
        var data = builder.buildSystemModule(config, identifier);
        if( context.hot && data.deps )
        {
        ...
        content = data && data.makeContent["javascript"] ? data.makeContent["javascript"] : '';
    }
...
module.exports.pitch = function(remainingRequest, precedingRequest, data) 
{
    ...
    if( this.resourceQuery )
    {
        ...
        if( query[0] ==='less')
        {
            ...
            return style ? lessLoader.call(this, style) : '';
        }
    }
