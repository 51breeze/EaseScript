    ...
    switch ( metatype.type )
    {
        ...
        case "Router" :
            ...
            }else if( currentStack.keyword() === "class" )
            {
                ...
                action = classModule.proto[ metatype.param.default ];
            ...
            if( !action || action.id !=="function" )
            {
                throw ReferenceError('Not found the specified method. for "'+metatype.param.default+'"');
            }

            //方法必须为公开的
            if( action.privilege !=="public" )
            {
                throw ReferenceError('the "'+metatype.param.default+'" method is not public of qualifier.');
            }
            ...
            if( requireBound.length > 0 )
            {
                ...
                {
                    router.alias = router.alias.replace(/\/$/,'')+"/{"+requireBound.join("}/{")+"}";
                }
            }
            metatype.param.alias = '/'+metatype.param.alias.replace(/^\/|\/$/g,'');
            ...
            Utils.forEach(methods,function (method) {
                ...
                //加入到全局哈希值中
                if( Compile.global_route_unique_hash[ key ] )
                {
                    throw TypeError( 'The "'+metatype.param.alias+'" already bound to '+Compile.global_unique_hash[ key ] );
                }
...
{
    var moduleClass = {'static':{},'proto':{},'import':{},'constructor':{},'attachContent':{},"rootContent":[],'defineMetaTypeList':{},'hasPackage':false,
    ...
    for( ;i<len ;i++ )
    {
        ...
        }else if( root_block_declared.indexOf(id) >= 0 )
        {
            ...
            }else
            {
                ...
                {
                    var subClass = getDeclareClassDescription(item, moduleClass.hasPackage , config, syntax );
