 ...
 * @returns {string}
 */
function parseMetaType( describe, currentStack, metaTypeStack , config,  syntax  )
{
    ...
    for (; i < len; i++)
    {
        ...
        if (item instanceof Ruler.STACK)
        {
            ...
            if ( metatypelist.length > 0 && item.keyword() !== 'metatype')
            {
                ...
                for(;mI<mLen;mI++)
                {
                    parseMetaType(desc, item, metatypelist[mI], config, syntax);
                }
                metatypelist = [];
            }
        }
 ...
 * @returns
 */
function loadSkinModuleDescription(syntax, skinClassName , config )
{
    var module = makeSkin( skinClassName , config , syntax );
    if( module )
    {
 ...
 * @returns
 */
function loadModuleDescription(syntax, file , config , resource , suffix , nonSkin )
{
    ...
    if( !fs.existsSync(sourcefile+suffix) )
    {
        ...
        //加载皮肤
        return loadSkinModuleDescription( syntax, fullclassname , config );
    }
    sourcefile+=suffix;
    ...
    for (var i in description.import)
    {
        ...
        loadModuleDescription( syntax, file, config,  description.filename );
    }
    return description;
}
...
function loadFragmentModuleDescription( syntax, fragmentModule, config, isTemp )
{
    ...
    for (var i in description.import)
    {
        loadModuleDescription( syntax, description.import[i], config, scope.filename );
    }
