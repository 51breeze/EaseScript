...
function operateMethodOfGet(classmodule, thisvrg, desc, props ,info, before)
{
    ...
    if( ns.length > 0 )
    {
        ...
        {
            map.splice(index,1,'['+classmodule.classname+'::$'+ns.join(","+classmodule.classname+"::$")+']');
        }else{
...
function operateMethodOfApply(classmodule, thisvrg, desc, props ,info)
{
    ...
    if( !checkRunning )
    {
        if( thisvrg==="Math" && props.length===1 )
        {
            return props[0]+'('+param.join(',') +')';
        }
...
function toString( stack, module, acceptType , fromParam )
{
    ...
    if( stack.keyword() === 'object' )
    {
        ...
        if( type ==="Array")
        {
            requirements["ArrayList"]=true;
            module.requirements["ArrayList"]=true;
            return 'new ArrayList('+it.content.slice(1,-1).join('')+')';
        }
...
function makeModule( stack , classModule, config )
{
        ...
        if( item instanceof Ruler.STACK && item.keyword() !=='metatype' )
        {
            ...
            else if( item.keyword() === 'var' || item.keyword() === 'const' )
            {
                ...
                if( !( /^([\"\']).*?\1$/.test(ret) || /^\w+$/.test(ret) ) && !Utils.isConstant( ret ) )
                {
                    ...
                    if (item.static()) {
                        props.push( classModule.classname+"::" + _pn + '=' + ret);
                    } else {
    ...
    for( var u in classModule.use )
    {
        ...
        {
            props.unshift( classModule.classname+"::$"+u+"="+"new Namespaces('','"+classModule.namespaces[u].value+"')");
        }else
        {
            ...
            props.unshift( classModule.classname+"::$"+u+"="+"new "+u+"()");
        }
    }
