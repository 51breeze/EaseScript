...
function extractIntoVariable()
{
    ...
    this.scope().name("@extractIntoVariable")
    this.loop(function(){
        if( this.next.value==='}' || this.current.value==='}')return false;
        this.check();
        return true;
    });
    ...
    if( this.next.id===':' )
    {
        ...
        if( type !=='void' && stack.parent().keyword() !=='interface' )
        {
            stack.addListener('(switch)', function (e) {
                ...
                if( !this.isReturn )error('Must be return in the "'+name+'" function','', currentType);
            });
        }
    }
...
syntax['(operator)']=function( e )
{
    ...
    if( id===this.next.value || ( this.next.type==='(operator)' && this.next.value !==';' && !(id==="*" || this.next.value==="*")  &&
     !Utils.isLeftOperator(this.next.value) && !Utils.isIncreaseAndDecreaseOperator(id) ) )
    {
        this.error('Unexpected token ' + this.next.value, this.next);
    }
 ...
 */
function describe(type,value,id, cursor)
{
    return {type:type ,value:value, id:id || value , line:0, cursor:cursor||0};
}
...
Ruler.prototype.identifier=function(s)
{
    ...
    switch( v )
    {
        ...
        case "'":
            ...
            return describe( v==='`' ? '(template)' : '(string)', s.substr(0,i+1), v , hasNewline ? cursor+1 : 0);
