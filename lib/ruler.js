...
syntax['function']= function(event){
    ...
    if( name && !isassign )
    {
        ...
        if( content.length > 2 )
        {
            ...
            {
                isSyntaxMetatype = metatype.keyword() === "metatype" && metatype.name() === "Syntax";
            }
        }
    ...
    if( balance[id] )
    {
       ...
       }else
       {
           ...
           if( s.type()==='(Array)' )
           {
               ...
               if( this.prev.id==='(identifier)' || this.prev.value==='this' || this.prev.value==='::' || (prev instanceof Stack && prev.type()==='(expression)') )
               {
                   is=true;
                   s.type('(property)');
               }
        ...
        if( ( this.scope().keyword() ==='object' && Utils.isLeftDelimiter(this.next.value) ) ||
            ...
            //bb() as Object or (bb) as Object or (Object)refObject
            ( s.type() ==='(expression)' && (this.next.value==='as' || this.next.id==="(identifier)" || this.next.value==="this" || this.next.value==="new") )  ||
...
function statement()
{
    ...
    }else if( this.config("strictType") && id !=="namespace" )
    {
...
syntax['(string),(number),(regexp),(double),(float)']=syntax['(identifier)'];
 ...
 * @constructor
 */
function Ruler( content, config , rootblock)
{
...
Ruler.prototype.seek=function( flag )
{
    ...
    }else
    {
        if (this.input.length === this.cursor)
        {
            ...
            if (!o)this.error('Unexpected identifier ' + s);
...
Ruler.prototype.end=function( stack )
{
    ...
    {
        var pid = stack.parent().keyword();
        if( pid==='function' || pid==='if' || pid==='for' || pid==='switch' || pid==='else' || pid==='while' || pid==='do' || pid==='try' )
        {
...
Ruler.prototype.number=function(s)
{
    ...
    {
        var type = "(number)";
        if( match[1].slice(-1).toUpperCase() === "F")type= "(float)";
