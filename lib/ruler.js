...
function statement()
{
    ...
    //常量必须指定默认值
    if( id==='const' && this.next.value !=='=' && stack.parent().__spread__ !== true )
    {
        this.error('Missing default value in const',this.next)
    };
    ...
    if( this.next.value ==='=' )
    {
        ...
        if( stack.parent().parent().keyword() === 'interface' ){
            ...
            {
                 this.error('Can not assign default value',this.next);
            }
        }
    }
    return desc;
