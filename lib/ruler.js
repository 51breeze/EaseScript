...
const reserved = [
    ...
    'extends','import','class','function','new','typeof','const','var','let','interface','abstract','implements','namespace',
    ...
    if ( this.line < this.lines.length )
    {
        ...
        if( !this.skip  )
        {
            ...
            if ( (n=/\/\*/.exec( this.input )) )
            {
                ...
                this.skip = true;
                this.multiLineAnnotation = this.input.substr(n.index);
            }
            //单行注释
            else
            {
                ...
                if( index >= 0 && this.input.charCodeAt( index-1 ) !== 92 )
                {
                    ...
                    if( !(left < index && index < right) )
                    {
                        ...
                        this.lineAnnotation = this.input.substr(index);
                    }
                }
            }
        }
        ...
        if( this.skip )
        {
            ...
            if( e )
            {
                ...
                this.multiLineAnnotation += this.input.substr(0, e.index+2);
            }
            this.input=s;
        }
