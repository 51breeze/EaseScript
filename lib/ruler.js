       ...
       }else
       {
        ...
        if( s.type()==='(JSON)' )
        {
            this.loop(function () {
                ...
                {
                    if( this.scope().getScopeOf().define( this.current.value )  )
                    {
                        if( this.next.value ==="," )
                        {
                            this.add( this.seek() );
...
syntax['(operator)']=function( e )
{
    ...
    if( id==="=" && this.next.id ===">" && this.scope().keyword() ==="expression" )
    {
        var lastStack = this.scope().previous( -1 );
        if( lastStack.keyword() === "object" && lastStack.type() ==="(expression)" )
        {
            Stack.__current__ = lastStack;
            lastStack.__close__ = false;
            this.add( this.current );
            this.seek();
            this.add(this.current);
            this.scope().name("@arrowFunction");

            if( this.next.value ==="{" )
            {
                var block = new Scope('function', '(*)');
                this.add( block );
                this.seek();
                this.add(this.current);
                this.loop(function(){
...
Ruler.prototype.parser=function parser()
{
    ...
    o.line= this.line;
    o.cursor = this.cursor;
    o.raw = this.raw( this.line-1 );
    this.lastStack = o;
    return o;
}

Ruler.prototype.test=function()
{
    var stack = this.parser();
...
Ruler.prototype.end=function( stack )
{
    ...
    {
        var pid = stack.parent().keyword();
        if( pid==='function' || pid==='if' || pid==='else' || pid==='when' || pid==='then' || pid==='for' || pid==='switch'  || pid==='while' || pid==='do' || pid==='try' )
        {
