...
var template_syntax={
    ...
    'php': {
        'foreach': function (attr, content) {
            return '<?php foreach($' + attr.name + ' as $' + (attr.key || 'key') + ' => $' + (attr.value || 'item') + '){ ?>' + content + '<?php}?>';
        },
        'for': function (attr, content) {
            return '<?php for(' + replace_condition(attr.condition) +'){ ?>' + content + '<?php}?>';
        },
        'if': function (attr, content) {
            return '<?php if(' + replace_condition(attr.condition) + '){ ?>'+content+'<?php}?>';
        },
        'elseif': function (attr, content) {
            return '<?php elseif(' +  replace_condition(attr.condition) + '){ ?>'+content+'<?php}?>';
        },
        'else': function (attr, content) {
            return '<?php }else{ ?>'+content+'<?php}?>';
        },
        'do': function (attr, content) {
            return '<?php do{ ?>'+content+'<?}?>';
        },
        'switch': function (attr, content) {
            return '<?php switch(' +  replace_condition(attr.condition) + '){ ?>'+content+'<?php}?>';
        },
        'case': function (attr, content) {
            content = '<?php case "' +  replace_condition(attr.condition) + '": ?>'+content;
            if( attr["break"]==='true' )content+='<?php break; ?>';
            return content;
        },
        'default': function (attr, content) {
        ...
        'break': function (attr, content) {
            return '<?php break; ?>'+content;
        },
        'while': function (attr, content) {
            return '<?php while(' +  replace_condition(attr.condition) + '){ ?>'+content+'<?php}?>';
        },
        'code': function (attr, content) {
            return '<?php '+content+' ?>';
        },'script': function (attr, content) {
...
function makeViewComponentOfPHP(skin, context, level, callback )
{
    ...
    var str = [];
    str.push( depth1+"var "+skin.id+"= new "+skin.fullclassname+"("+(skin.param||"")+");" );
...
function parseTextBinding(context,parent,text, childIndex )
{
    ...
    if( text.slice(0,2) === '{{' && text.slice(-2) === '}}' )
    {
        ...
        {
            var bindname = text.replace(/\s/g,'');
            bindname = bindname.split('.');
            var refObject = "this.assign('"+text+"')";
            if( bindname[0] === 'this' )
            {
                refObject = bindname.join(".");
            }
            if( context.isMakeView )
            {
                parent.attributeId || (parent.attributeId='Q'+uid()+'_'+(new Date()).getTime());
                context.initProperties.push("Element('#"+parent.attributeId+"').addChildAt( Element.createElement( "+refObject+" ), "+childIndex+" )");
            }else
            {
                if( !parent.id )
                {
    ...
    else if( elem.namespace() )
    {
        ...
        if( !skinObject.isMetadata && parent.isProperty && parent.description && (parent.description.isAccessor || parent.description.isFunAccessor) )
        {
            ...
            if( !(refType ==="void" || refType==="*") )
            {
                ...
                var refTypeModule = makeSkin.getLoaclAndGlobalModuleDescription( fullname );
        ...
        else if (desc)
        {
            ...
            {
                console.log( module.fullclassname )
...
function makeScript( classModule , skinObject )
{
    ...
    //组件初始化属性
    var initComponentProperties = getInitComponentProperties(components, imports, property, attach, loaded, classModule );
    ...
    if( skinObject.metadataList && skinObject.metadataList.HostComponent )
    {
        ...
        body.push('this._hostComponent=hostComponent;\n');
    }
    ...
    if( bindableContent )
    {
        if( classModule.isMakeView===true )
        {
            body.push( bindableContent );
        }else{
            body.push('this.addEventListener( SkinEvent.CREATE_CHILDREN_COMPLETED ,function(event){\n' + bindableContent + '});\n');
        }
    }
    ...
    if (classModule.isMakeView === true)
    {
        if( classModule.syntax === "javascript")
        {
            ...
            content.push('function ' + classModule.classname + '(context:Application)' + '{super(context);}');
        }
...
function getInitComponentProperties(components, imports, classProperty, attach, loaded, classModule )
{
    //组件初始化属性
    ...
    for (var classname in components )
    {
        ...
        if( component.isDisplayElement )
        {
            ...
            {
                initComponentProperties.push(component.viewport + ".addChildAt( " + component.id + ".display() , "+component.childIndex+")");
