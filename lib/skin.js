...
function __toString(skin, notContainer , parent, separation, makeSyntax )
{
    ...
    //如果组件在子级中
    if( skin.isComponent === true  &&  skin.fullclassname )
    {
        tag = skin.fullclassname;
...
* retrun object
*/
function parseValueString(value, refer, isString)
{
    var is = false;
    //引用属性值
    if( value.charAt(0)==="@" )
    ...
    {
        var bind = parseTextBinding(value);
        if( bind.result )
        {
            value = bind.value;
            is = true;
    ...
    if( !is && ( isString || !/^([\-\+]?[\d\.]+|true|false)$/i.test( value ) ) )
    {
        okValue = '"'+escapeValueString(value)+'"';
    }
    
    if( /^([\-\+]?[\d\.]+)$/i.test( okValue ) )
    {
        if( parseFloat( okValue ).toString() !== okValue )
        {
            okValue = '"'+okValue+'"';
        }
    }
    return {value:value,has:is,okValue:okValue};
}
...
function willNodeElementConvertToObject(children, context, topScope )
{
    ...
    if( typeof children ==="string" )
    {
        ...
        var bind = parseTextBinding( children );
        if( bind.result )
        {
            return bind.value;
        }

        return "new String("+parseValueString( children ).okValue+")";
    }
    ...
    for (;index<len;index++)
    {
        ...
        if( typeof child ==="string" )
        {
            throw new Error('Object children item cannot be strings or references');
        }

        var name  = child.attrdata && child.attrdata.name || child.name;
        var value = null;
        var type  = child.attrdata && child.attrdata.type;
        if( type==="*" )type = null;

        //组件或者节点元素都转成对象
        if( child.isComponent || child.isCommand || (child.module && child.module.isDisplayElement) )
        {
        ...
        }else if( child.children && child.children.length > 0  )
        {
            ...
            if( child.children.length === 1 && typeof child.children[0] ==="string" )
            {
                ...
                value = getValueByType( child.children , type||"Array", context, topScope ); 
            }

        }else
        {
            value = child.attrdata ? child.attrdata.value || child.attrdata.content || child.attrdata.innerHTML : null;
            if( value ){
...
function willNodeElementConvertToArray(children, context, topScope, isParam )
{
    ...
    for (;index<len;index++)
    {
        ...
        if( typeof child === "string" )
        {
            value = parseValueString(child).okValue;
        ...
        }else if(child)
        {
            ...
            }else
            {
                ...
                value = attr.value || attr.content || attr.innerHTML;
...
var make_skin_factory={
    ...
    'switch': function (attr, content) {
        return 'switch(' +  replace_condition(attr.name) + '){\n'+content+'\n}';
    },
...
function __toMakeSkinFactory(skinChildren, context, topScope, parentObject, innerHTML )
{
    ...
    var _topScope ={
        ...
        topStack:topScope.topStack||declaration,
        children:children,
...
function checkInstanceOf(context, classModule, classType, isInterfaceType )
{
    ...
    {
        if( (classModule.fullclassname || classModule.type) === classType ){
            return true;
        }
        if( classModule.implements && classModule.implements.length > 0 && isInterfaceType )for( var i in classModule.implements )
        {
            ...
            if(doPrototype( _m , descriptions ,function (interfaceModule) {
                if( interfaceModule.fullclassname === classType ){
                    return true;
                }
            })){
                return true;
...
function getClassPropertyDesc(name, currentModule, context , isset, isget, isStatic )
{
    if( currentModule.description )
    {
        ...
        var last = parent;
        var parentClass;
        var protoName = isStatic ? "static" : "proto";
        do{
            var proto = $has.call(parent,protoName) ? parent[ protoName ] : null;
            if( proto )
            {
                if ( $has.call(proto,name) )
                {
                    ...
                    if( !isset && ( desc.isAccessor || desc.isFunAccessor || desc.id==="const" ) )
                    {
                        if( !isget )return desc;
                        if( (desc.value && desc.value.get) || desc.id==="const" )return desc;
                    }
                    if( isset && ( (desc.value && desc.value.set) || desc.isFunAccessor) ) return desc;
                }
            }
            var xml = parent['xml'];
        ...
        }while ( parent && parent.id==='class' && last !== parent && !isStatic );
    }
    return null;
}
...
function parseTextBinding(text,context)
{
    ...
    if( text.charAt(0) ==="@" )
    {
        return {value:text.substr(1),refs:refs, binding:binding, result:true,isTwoWayBind:isTwoWayBind,twoWayBindProps:twoWayBindProps};
    }
...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    for(var p in attrs)
    {
        ...
        if( hash !== true )
        {
            ...
            var isEventState = false;
            if( attrName.indexOf(".") > 1 )
            {
                var attrNameArr = attrName.split(".");
                if( attrNameArr.length > 1 )
                {
                    var eventType  = attrNameArr.pop().split(":")[0];
                    var eventClass = attrNameArr.join(".");
                    if( makeSkin.isClassFileExists( eventClass , context.config ) )
                    {
                        var typeModule = makeSkin.loadModuleDescription( context.syntax, eventClass, context.config, eventClass, undefined, undefined, context.ownerModule );
                        isEventProperty = checkInstanceOf(context, typeModule, "Event");
                        var eventDesc = getClassPropertyDesc(eventType, {description: typeModule}, context, false, true, true );

                        if( isEventProperty && !eventDesc )
                        {
                            throw new ReferenceError(`"${eventClass}::${eventType}" is not exists.`);
                        }

                        if( isEventProperty )
                        {
                            isEventState = attrName.indexOf(":") > 0;
            ...
            if( !skinObject.isCommand )
            {
                ...
                {
                    bind.isAttr=true;
                    bind.isEventProperty =isEventProperty;
                    bind.isEventState    =isEventState;
    ...
    else if( elem.namespace() )
    {
        ...
        if( !skinObject.isMetadata )
        {
            ...
            if( isClass )
            {
                ...
                //是否为一个状态组件
                if( checkInstanceOf(context, skinObject.description, context.stateClass) )
                {
            ...
            {
                var desc = null;
                var assignType = attrdata.as || null;
                delete attrdata.as;

                //是否对一个父级属性的引用
                if( parent && parent.isProperty )
                {
                    if( !parent.propertyGetAccessor )
                    {
                        throw new ReferenceError("Property is not getter accessor for '"+parent.name+"'");
                    }

                    var type = parent.propertyGetAccessor.type;
                    if( parent.propertyGetAccessorType )
                    {
                        type = parent.propertyGetAccessorType;
                    }

                    if ( !(type === "void" || type === "*") )
                    {
                        var typeClass = getImportClassName(type, module.description.import) || type;
                        var typeModule = makeSkin.loadModuleDescription( context.syntax, typeClass, context.config, typeClass, undefined, undefined, context.ownerModule );
                        desc = getClassPropertyDesc(name, {description: typeModule}, context );
                        skinObject.propertyGetAccessor = getClassPropertyDesc(name, {description: typeModule}, context,false,true);
                        skinObject.propertyGetAccessorType = assignType || null;
                ...
                else if( module === parent )
                {
                    ...
                    skinObject.propertyGetAccessor = getClassPropertyDesc(name, module, context,false,true);
                    skinObject.propertyGetAccessorType = assignType || null;
                }
                ...
                //必须是一个存储器 
                if( !(desc.isAccessor || desc.isFunAccessor || desc.id==="var" || desc.id==="const") )
                {
        ...
        (parent && parent.isDisplayElement && skinObject.isProperty) ) )
    {
        ...
        var bindState = bindAttrs.includeIn || bindAttrs.excludeFrom;
        ...
        skinObject.definedIncludeState={
            ...
            expre:bindState? "includeIn("+bindState.value+")" : "includeIn("+parseValue.okValue+")"
        };
    ...
    skinObject.ownerIncludeStateObject = skinObject.definedIncludeState ? skinObject : parent ? parent.ownerIncludeStateObject : null;
    ...
    if (nodes.length > 0)
    {
        for (var i in nodes) 
        {
           ...
           if( typeof child  ==="string" && child.substr(0,2) === "{{" )
           {
              var twoBind = parseTextBinding( child , context);
              if( twoBind.result && twoBind.isTwoWayBind )
              {
                  twoBind.isAttr = true;
                  skinObject.bindAttrs["content"] = twoBind;
 ...
 * @returns {string}
 */
function makeScript( classModule , skinObject , isMakeView )
{
    ...
    if( skinObject.metadataList && skinObject.metadataList.HostComponent )
    {
        ...
        property.push( `override protected function get hostComponent():${hostComponent}{\nreturn this._hostComponent;\n}\n`);
    }
    ...
    if( render.topScope.staticProperties )
    {
        ...
        if( staticProperties.length > 0 )
        {
             ...
             staticProperties=["const attrs:Object={"+staticProperties.join(",")+"};\n"];
             staticProperties.push("this.properties=attrs;\n");
        }
    }
    ...
    var name = (classModule.name === "Skin" ? '"div"' : '"'+classModule.name+'"');
    var initContent = body.join("");
    var findElem = `
        var elem:Node = null;
        if( !(hostComponent as SkinComponent).async )
        {
            elem = this.findElement( this.getUniqueKey() );
        }
    `;
    var superValue = `super(${name});\n`;
    var superClientValue = `super(elem ? elem : ${name});`;

    if ( isMakeView === true)
    {
        superValue='super(hostComponent);\n';
        superClientValue = superValue;
        findElem = "";
    }

    body = [
        staticProperties.join(""),
        'this._hostComponent=hostComponent;\n',
        `when(RunPlatform(server)){
            ${superValue}
            ${render.topScope.topCommonStack.join("\n")}
            ${initContent}
        }then{
            ${findElem}
            ${superClientValue}
            ...
            if(System.env.HOT_UPDATA){
                System.env.HOT_UPDATA( this, function(obj:${classModule.classname}, newClass:Class ){
                    if( obj instanceof es.core.View ){
                        (obj.hostComponent as es.core.Application).hotUpdate( newClass, obj as es.core.View );
                        return true;
    ...
    if( render.client && classModule.syntax !=="javascript" )
    {
        // content.push('\n[Syntax(javascript)]');
        // content.push('\noverride protected function __$initClient():Boolean' + '{\n' + render.client + '\n return true;\n}');

        // content.push('\n[Syntax(javascript)]');
        // content.push(`\nprivate function __$initOriginProps():void{ 
        //    const attrs:Object = this.properties;
        //    ${initContent}
        // }`);

        // content.push('\n[Syntax(javascript)]');
        // content.push(`\nprivate function __$initClientProps():void{ 
        //     const attrs:Object = this.properties;
        //     ${render.topScope.clientScope.topDeclaration.join("\n")}
        //     ${render.topScope.clientScope.topStack.join("\n")}
        // }`);
    ...
    if( classModule.fullclassname==="view.Viewport")
    {
...
function makeSkin(view , config, syntax, ownerModule)
{
    ...
    //源文件修改过的时间
    var statId = new Date(stat.mtime).getTime();

    //如果是已经加载过
    if( loadedSkins[ filepath ] && loadedSkins[ filepath ].uid === statId )
    {
        return null;
    }
    ...
    classModule.isModify = !!loadedSkins[ filepath ];
    loadedSkins[ filepath ] = classModule;
    ...
    try{
        ...
        classModule.script = makeScript( context, skinObject, isMakeView );
        //是一个视图
...
function getComponentPropertyDescriptor( propertyObject, value, component, context, topScope )
{
    ...
    {
        while( value.length > 0 && value[0].isProperty )
        {
            property.push( value.shift() );
        }
    }
    ...
    if( target.items.length > 0 )
    {
        ...
        var desc = prop.desc;
        if( !( (desc.isAccessor && desc.value.set) || desc.isFunAccessor || desc.id==="var" ) )
        {
            throw new ReferenceError("Property is not setter. for '"+prop.name+"'")
        }
        desc = desc.isFunAccessor ||  desc.id ==="var" ? desc : desc.value.set;
        target.desc = desc;
        if( value )
        {
            var type = (desc.paramType && desc.paramType[0] ? desc.paramType[0] : desc.param && desc.param[0] ? desc.param[0] : null) ;
            if( type === "*" || !type )
            {
    ...
    if( component.isComponent )
    {
        ...
        Utils.forEach(component.attrdata,function(value,name)
        {
            ...
            if( !( ( isSkin || (component.isDisplayElement && !component.isSkinComponent) ) && name ==="name") )
            {
                ...
                var isEventProperty = false;
                if( bind && bind.isEventProperty && !bind.isEventState )
                {
                ...
                var isBindEvent = isEventProperty || !!Utils.getEventType( name );
                ...
                else if( (isSkin || component.isDisplayElement) && !(component.isSkinComponent && !isBindEvent && name!=="content" ) )
                {
                    if( stateName )
                    {
                        var states = defineAttributeStates[ name ] || (defineAttributeStates[ name ]={items:[],value:null,isEventProperty:!!(bind && bind.isEventProperty)});
        ...
        (component.children||[]).forEach(function(child){
            ...
            if( child.isProperty )
            {
               const propGroups = mergePropertyGroups( child );
               propGroups.forEach( (props)=>{
                    var target = getComponentPropertyDescriptor( props.shift(), props ,component, context, topScope );
...
function mergePropertyGroups(child, parent )
{
    var items = [];
    if( child.isProperty )
    {
        if( child.children instanceof Array && child.children.length > 0 )
        {
            ...
            {
                var childItems = mergePropertyGroups( child.children[i], child );
                if( childItems.length > 0 )
                {
                    childItems.forEach( (item)=>{
                        if( parent ){
                            items.push( [parent].concat(item) );
                        }else{
                            items.push( item );
                        }
                    });
            ...
            if( items.length === 0 )
            {
                items.push(  (parent ? [ parent, child ] : [child]).concat( child.children ) );

            }else if( items.length < child.children.length )
            {
                var childs = child.children.filter( (item)=>!item.isProperty ).map( __toString );
                throw new SyntaxError(`Invalid node for "${childs.join(",")}" `)
            }

        }else if( items.length === 0 )
        {
            items.push( parent ? [ parent, child ] : [child] );
        }
    }
    return items;
};
...
function getDefinedStateProperties(skin, stateObject, stateHandle, topScope )
{
    ...
    Utils.forEach(stateObject,function(state,name)
    {
        ...
        var defaultValue = eventType || state.isEventProperty ? "null" : '""';
        if( state.value )
        {
        ...
        if( expression.length > 0 )
        {
            ...
            if( eventType ||  state.isEventProperty ){
                events[ eventType || name ]= expression.join(" : ");
            }else{
    ...
    for( var p in target )
    {
        ...
        }else
        {
            if( flag )
            {
                ...
                if( target[p].bind && target[p].bind.isEventProperty )
                {
                    eventType = p;
                }
...
function getRenderScope(component, contextType, context, topScope)
{
    if( component===true || ( component.isComponent && (component.isRenderComponent || (component.isSkin && !component.hasDefinedRenderProperty) ) ) )
    {
        ...
        return {
            ...
            clientScope:topScope.clientScope,
            topCommonStack:topScope.topCommonStack
        }
    }
    return topScope;
}

var global_total_count = 0;

//生成动态渲染皮肤的脚本
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    const DOMRender = topScope.DOMRender;
    const Context = topScope.Context;
    if( !skin.isCommand )
    {
    ...
    //const uniqueKey = context.totalChildCount;
    const uniqueKey = Utils.uid();
    if( typeof skin === "string" )
    {
       if( parent && parent.hasChildCommand )
       {
           return createUniqueArrayRef(parent)+".push("+parseValueString(skin).okValue+");";
       }

       if( parent.iscdata && !parent.isCommand )
       {
            var okValue = '"'+escapeValueString(skin)+'"';
            container.push( okValue );
            return;
       }
    
       container.push( parseValueString(skin).okValue );
       return;
    }

    const isClientRunning = true;
    const tag = skin.name;
    const children = skin.children || [];
    const attr = skin.attrdata || {};
    const childContent = [];
    const isCommand = skin.isCommand;
    const len = children.length;
    const childrenReference = isCommand ? container : [];
    ...
    {
        topScope.clientScope.children = clientChildren;
    }
    ...
    if( skin.definedIncludeState && clientChildren !== topScope.clientScope.children )
    {
       if( topScope.clientScope.children.length > 0 )
       {
            clientChildren.push( topScope.clientScope.children.join("\n") );
       } 
       topScope.clientScope.children = clientChildren;
    }
    ...
    if( isCommand )
    {
        const bindCommand = null;
        const tagName = tag.substr(1).toLowerCase();
        const code =  [];
    ...
    //一个组件或者是元素
        ...
        //是否为公开的元素
        const isPublic = skin.id && skin.isPrivate !==true;
        //元素定义的属性
        const properties = getComponentDefinedProperties( skin, context , topScope);
        //双向绑定的属性
        const propertyTwoWayBinding = getAttributesTwoWayBinding( skin, context, topScope , isPublic );
        //生成元素静态属性上的状态机
        const stateProperties = getDefinedStateProperties(skin,skin.defineAttributeStates, topScope.StateHandle, topScope );
        //获取静态属性
        const elementAttributes = getElementAttributes( skin ,topScope);
        const staticAttr = elementAttributes.attrs;
        //获取动态属性
        const bindAttributes = getElementAttributes(skin,topScope,true);
        const bindAttr = bindAttributes.attrs;
        const eventAttr = bindAttributes.events;
        const dynamicAttr = [];
        const attributes=[];
        var bindEvents = [];
        ...
        if( !isPublic && !(bindAttributes.innerHTML || elementAttributes.innerHTML) && childrenReference.length === 1 && skin.children.length ===1 && typeof skin.children[0] ==="string" )
        {
            innerHTML = childrenReference.pop();
            hasChildren = false;
        }
        ...
        var hasDynamicEventType = false;
        var eventReferenceName = null;

        //生成事件属性
        for( var eventType in eventAttr )
        {
            if( eventType.charAt(0) ==="@" ){
                hasDynamicEventType = true;
                bindEvents.push( [ eventType.slice(1), eventAttr[eventType] ] );
            }else{
        ...
        {
            bindEvents = '['+bindEvents.map( value=> `${value[0]},${value[1]}`).join(",")+"]";
            ...
            switch( skin.name.toLowerCase() )
            {
                case "title" :
                    if( innerHTML )
                    {
                        if( bindAttributes.innerHTML )
                        {
                            ...
                            topScope.topContentStack.push("hostComponent.title="+(innerHTML)+" as String;"); 
                        }
                    }
                ...
                case "meta" :
                    if( innerHTML )
                    {
                        if( bindAttributes.innerHTML )
                        {
                            dynamicAttr.push('"content":'+innerHTML)
                        }else{
                            attributes.push('"content":'+innerHTML)
                        }
                    }
                    if( dynamicAttr.length > 0 )
                    {
                        childContent.push(`if(hostComponent instanceof es.core.Application){(hostComponent as es.core.Application).meta(${uniqueKey},{${dynamicAttr.join(",")}});}`);
                    }
                    if( attributes.length > 0 )
                    {
                        topScope.topContentStack.push(`if(hostComponent instanceof es.core.Application){(hostComponent as es.core.Application).meta(${uniqueKey},{${attributes.join(",")}});}`);
                    }
                    context.gloableElementList.head.children.push( skin );
                    return childContent.join("\n");
                case "script" :
        ...
        if( attributes.length > 0 )
        {
            ...
            attrRefName="attrs."+attrRefName +" as Object";
        }

         //此元素是否在其它地方有被引用到
        var hasReference = isPublic || properties.length > 0 || skin.hasChildCommand ||
        (propertyTwoWayBinding.content.length > 0) || ( (bindEvents||dynamicAttr.length >0) && skin.isComponent);
        ...
        if( skin === context )
        {
            ...
            if( properties.length > 0 )
            {
                ...
                if( componentProps )
                {
                    ...
                    {
                        topScope.topCommonStack.push( componentProps.init.join("\n") );
                    }
            ...
            if( dynamicAttr.length > 0 )
            {
                childContent.push( DOMRender+".attributes("+Context+".element,{"+dynamicAttr.join(",")+"});");
            }
            //元素对象的动态属性
            if( bindEvents )
            {
                if( isClientRunning ){
                    childContent.push( DOMRender+".bindEvent("+[uukey,uniqueKey,"this",bindEvents].join(",")+");");
                }else{
                    //topScope.topContentStack.push(`this.marker(this,"id",this.getUniqueKey("${uniqueKey}"));`);
                }   
            }
        }
        ...
        else if( skin.isComponent )
        {
            ...
            if( classname==="Object"  )
            {
               newValue =  newComponentParam.join(",");
            ...
            if( skin.isDisplayElement )
            {
                ...
                if( !isPublic )
                {
                    ...
                    {
                        param.splice(4,1, componentChildren);
                        indexAt = Math.max(4,indexAt);
                    }

                    //元素对象的动态属性
                    if( dynamicAttr.length > 0  )
                    {
                        param.splice(6,1, "{"+dynamicAttr.join(",")+"}");
                        indexAt =Math.max(6,indexAt);
                    }
                    ...
                    {
                        if( isClientRunning ){
                           param.splice(7,1, bindEvents);
                        }else{
                           param.splice(7,1, "true");
                        }
                        indexAt = Math.max(7,indexAt);
                    }
                }
                ...
                newClientValue = DOMRender+".findComponent("+param.slice(1,2).join(",")+")";
                ...
                {
                    newClientValue =  DOMRender+".findComponent("+param.slice(1,2).join(",")+",true)";
                }

                 //有定义状态机
                if( skin.definedIncludeState && !(isPublic && !skin.ownerForeachCommand) )
                {
                    if( skin.definedIncludeState.bind )
                    {
                        newValue = skin.definedIncludeState.logic+skin.definedIncludeState.bind.value+"?"+newValue+":null";
                    }else{
                    ...
                    if( skin.definedIncludeState.bind )
                    {
                        newClientValue = skin.definedIncludeState.logic+skin.definedIncludeState.bind.value+"?"+newValue+":null";
                    }else{
                        newClientValue =  skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+newClientValue+":null";
                    }
                }
                ...
                if( hasReference )
                {
                    ...
                    if( isPublic )
                    {
                        ...
                        if( skin.ownerForeachCommand )
                        {
                            topScope.topStack.push( "var "+createUniquePublicArrayRef(skin,context)+":Array="+topScope.Context+"."+createUniqueRefId(skin)+";" );
                            //先清空数组中的元素
                            topScope.topStack.push( createUniquePublicArrayRef(skin,context)+".splice(0,"+createUniquePublicArrayRef(skin,context)+".length);" );
                        ...
                        }else
                        {
                            ...
                            if( skin.definedIncludeState )
                            {
                                if( skin.definedIncludeState.bind )
                                {
                                    _expression = "("+skin.definedIncludeState.logic+skin.definedIncludeState.bind.value+"?"+_expression+":null) as "+classType;
                            ...
                            topScope.topStack.push( "var "+createUniqueRefId(skin)+":"+classType+"="+_expression+";" );

                            if( skin.isSkinComponent || eventAttr.length > 0 || propertyTwoWayBinding.content.length > 0 )
                            {
                                topScope.clientScope.declaration.push( "var "+createUniqueRefId(skin)+":"+classType+"="+_expression+";" );
                            } 
                        }
                    }
                    ...
                    if( !isPublic || !skin.ownerForeachCommand )
                    {
                        pushDeclareExpressionToContent( 
                            ...
                            topScope,
                            isPublic,
                            "var "+createUniqueRefId(skin)+":"+classType+"=("+newClientValue+") as "+classType+";",
                            skin.isSkinComponent || eventAttr.length > 0 || propertyTwoWayBinding.content.length > 0
                        );
                    }
                ...
                if( propertyTwoWayBinding.content.length > 0 )
                {
                    ...
                    //componentContent.push(`this.marker(${createUniqueRefId(skin)},"id",this.getUniqueKey("${uniqueKey}"));`);
                }
                ...
                if( isPublic )
                {
                    ...
                    if( bindEvents )
                    {
                        componentContent.push( DOMRender+".bindEvent("+[uukey,uniqueKey,createUniqueRefId(skin),bindEvents].join(",")+");");
                    }
                ...
                if( componentContent.length > 0 )
                {
                    ...
                    {
                        topScope.declaration.push( ["if(",createUniqueRefId(skin),"){\n",componentContent.join("\n"),"\n}"].join("") );
                    }else{
                        topScope.declaration.push( componentContent.join("\n") );
                    }
                }
            ...
            else
            {
                ...
                if( hasReference )
                {
                    if( skin.isStateComponent )
                    {
                        ...
                        topScope.topCommonStack.push( "var "+createUniqueRefId(skin)+":"+classType+"="+newValue+";" );
        ...
        else if( tag ==="text" && bindAttributes.innerHTML )
        {
             ...
             }else if( bindAttributes.innerHTML )
             { 
                 //父级元素可以对当前元素的直接引用
        ...
        //创建一个元素
            ...
            if( !isPublic || skin.ownerForeachCommand )
            {
                ...
                if( bindEvents )
                {
                    indexAt = 6;
                    if( isClientRunning ){
                       param.splice(indexAt,1,bindEvents);
                    }else{
                       param.splice(indexAt,1,"true"); 
                    }
                }
            }
            ...
            var newClientExpression = DOMRender+".findElement("+param.slice(1,2).join(",")+")";
            if( isPublic ){
                newClientExpression+=" || "+expression;
            }
            if( skin.ownerForeachCommand ){
                newClientExpression = DOMRender+".findElement("+param.slice(1,2).join(",")+",true)";
            }
            if( skin.definedIncludeState && !(isPublic && !skin.ownerForeachCommand) )
            {
                if( skin.definedIncludeState.bind )
                {
                   expression = skin.definedIncludeState.logic+skin.definedIncludeState.bind.value+"?"+expression+":null";
                   newClientExpression =  skin.definedIncludeState.logic+skin.definedIncludeState.bind.value+"?"+newClientExpression+":null";
                }else{
                   expression = skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+expression+":null";
                   newClientExpression =  skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+newClientExpression+":null";
                }
            }
            ...
            if( hasReference )
            {
                ...
                if( isPublic )
                {
                    ...
                    if( skin.ownerForeachCommand )
                    {
                        ...
                        topScope.topStack.push("var "+refNodeTarget+":Object = "+_expression+";" );
                    }

                    if( bindEvents || propertyTwoWayBinding.content.length > 0 )
                    {
                       topScope.clientScope.declaration.push( "var "+refNodeTarget+":Object = "+_expression+";" );
                    }
                }
                ...
                pushDeclareExpressionToContent( 
                    ...
                    topScope,
                    isPublic,
                    "var "+createUniqueRefId(skin)+":Object="+newClientExpression+";",
                    bindEvents || propertyTwoWayBinding.content.length > 0
                );
                ...
                if( isPublic && !skin.ownerForeachCommand )
                {
                    ...
                    if( bindEvents )
                    {
                        childContent.push( DOMRender+".bindEvent("+[uukey,uniqueKey,refNodeTarget,bindEvents].join(",")+");");
                    }
                ...
                {
                    childContent.push( propertyTwoWayBinding.content.join("\n") );
                    //childContent.push(`this.marker(${createUniqueRefId(skin)},"id",this.getUniqueKey("${uniqueKey}"));`);
                }
            }
            //不需要有引用的元素
            else
        ...
        if( isPublic && skin !== context )
        {
             ...
             if( skin.ownerForeachCommand )
             {
                 ...
                 topScope.clientScope.topStack.push(  topScope.Context+"."+skinId+"="+skinId+" as Array;");

             }else
             {
                topScope.topContentStack.push(  topScope.Context+"."+skinId+"="+skinId+";");
                topScope.clientScope.topStack.push(  topScope.Context+"."+skinId+"="+skinId+";");
             }
        ...
        {
             makeClientContent(skin, context, topScope, uukey, uniqueKey, propertyTwoWayBinding, elementAttributes, bindAttributes, stateProperties, componentProps );
        }
    }
    return childContent.join("\n");
}
...
//把当前的表达式声明在指定的代码块中
function pushDeclareExpressionToContent(expression, skin, contentStack, topScope, isPublic, clientExpression, hasReference )
{
    //如果是在一个循环命令下就添加到当前位置
    if( skin.ownerForeachCommand )
    {
         ...
         if( isPublic )
         {
            if( clientExpression )
            {
               topScope.clientScope.topDeclaration.push( clientExpression  );
            }

         }else{
            
            if( clientExpression && (skin.isSkinComponent || hasReference) )
            {
                if( skin.ownerIncludeStateObject && !skin.definedIncludeState )
                {
                     topScope.clientScope.children.push( clientExpression );
                }else
                {
                     topScope.clientScope.declaration.push( clientExpression );
                }
            }
        }
...
function makeComponentPropertiesMothod(component,properties,topScope)
{
    ...
    properties.forEach(function(property)
    {
        ...
        while( property.items.length > 0 )
        {
            ...
            if( target.desc.isAccessor || target.desc.id==="var" || target.desc.id==="const" )
            {
                if( (!target.desc.value || !target.desc.value.get) && !target.target.propertyGetAccessor )
                {
                    throw new ReferenceError("Reference property is not getter. for '"+target.name+"'");
                }

                ref.push( target.name );
                if( target.target.propertyGetAccessorType )
                {
                    ref = ["("+ ref.join(".")+" as "+target.target.propertyGetAccessorType +")"]
                }
        ...
        if( !(prop.desc.isFunAccessor || prop.desc.isAccessor || prop.desc.id==="var" || prop.desc.id==="const" ) )
        {
           throw new ReferenceError("Reference property is not accessor. for '"+refsName+"'"); 
        }
        ...
        {
            if( (prop.desc.isAccessor && prop.desc.value && prop.desc.value.set ) || prop.desc.id==="var" )
            {
...
function makeClientContent(skin, context, topScope, uukey, uniqueKey, propertyTwoWayBinding, elementAttributes, bindAttributes, stateProperties, componentProps )
{
    var events = {};
    if( bindAttributes && bindAttributes.events )
    {
        Object.assign(events, bindAttributes.events);
    }

    if( stateProperties && stateProperties.events )
    {
        Object.assign(events, stateProperties.events);
    }
   
    events = Object.entries(events).map( (value)=>{
        return value[0]+":"+value[1];
    });

    var isPublic = skin.id && skin.isPrivate !==true;
    var find = skin.ownerForeachCommand ? `this.findElement(${uniqueKey},true)` : `this.findElement(${uniqueKey})`;
    var target =isPublic || skin.isSkinComponent ? createUniqueRefId( skin ) : find;
    var expression = "";
    ...
    if( skin.isSkinComponent )
    {
        if( !skin.definedIncludeState )
        {
            expression+= `if( ${target} ){
               ${target}.skinClass = ${skin.attrdata.skinClass};\n
               ${target}.display();\n
            }`;

        }else{
            expression+= `${target}.skinClass = ${skin.attrdata.skinClass};\n`;
            expression+= target+".display();\n";
        }

    }else if( isPublic )
    {
        //topScope.topContentStack.push(`this.marker(${target},"id", this.getUniqueKey("${uniqueKey}") );` );
        if( skin.isComponent )
        {
            var classname = skin.fullclassname || skin.classname;
            var classType = (skin.classType ? skin.classType :classname);
            //topScope.topContentStack.push(`this.marker(${target},this.getAttrName("component"),"${classType}");` );
        }
    }

    if( events.length > 0 )
    {
        expression+="this.bindEvent("+[0,uniqueKey,target,"{"+events.join(",")+"}"].join(",")+");\n";
    }

    if( propertyTwoWayBinding.content.length > 0  )
    {
        expression+= propertyTwoWayBinding.content.join("\n");
    }

    if( skin.ownerIncludeStateObject && expression )
    {
        topScope.clientScope.children.push( expression ); 
        expression = null;
    }

    if( skin.definedIncludeState && topScope.clientScope.children.length > 0 )
    {
        expression = topScope.clientScope.children.splice(0, topScope.clientScope.children.length ).join("\n");
        if( skin.isSkinComponent )
        {
            expression = "if("+createUniqueRefId(skin)+"){"+expression+"}";
        }else{
            expression = "if("+skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"){"+expression+"}";
        }

        if( skin.parent && skin.parent.ownerIncludeStateObject )
        {
            topScope.clientScope.children.push( expression );
            expression = null;
        }
    ...
    if( expression )
    {
        topScope.clientScope.contentStack.push( expression ); 
    } 
}
...
function makeRenderFactoryScript(skinObject, context, topStack, DOMRender,Context )
{
    ...
    if( context.__hasUseState__ )
    {
        ...
        topScope.clientScope.declaration.unshift( "if(!"+topScope.StateHandle+"){\nthrow new TypeError(\"State group is not defined for '"+topScope.StateHandle+"'\");\n}" );
        topScope.clientScope.declaration.unshift( "const "+topScope.StateHandle+":State="+topScope.Context+".getCurrentStateGroup();" );
    }
    ...
    if( topScope.topStack.length > 0 )
    {
       body.push( topScope.topStack.join("\n") );
    } 
    ...
    var client = "const dataset:Object=this.dataset;\n";
    client+=context.declares.join("\n");
    client+=topScope.clientScope.declaration.join("\n");
    client += topScope.clientScope.contentStack.join("\n");
