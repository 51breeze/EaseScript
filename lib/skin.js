...
function parseBinable(attrValue, attrName, skinObject, context )
{
    if(attrValue && attrValue.slice(0,2) === '{{' && attrValue.slice(-2) === '}}' )
    {
        ...
        /*if( context.isMakeView )
        {
            ...
            setViewportElementFlag( skinObject, context );
        }*/
...
function parseAttributes(attrs, skinObject, module, context , async )
{
    ...
    for(var p in attrs)
    {
        ...
        if( attrName.indexOf(".")>0 )
        {
           /* if( context.isMakeView )
            {
                skinObject.attributeId || (skinObject.attributeId=getViewportId());
            ...
            context.stateProperties.push({"name":attrName,"value":attrValue, "target":skinObject});
            continue;
        }
    ...
    if( nodes.length > 0 )
    {
        ...
        for (var i in nodes)
        {
            ...
            if( child )
            {
                ...
                {
                    //子级设置为了组件父级就必须得为组件
                    if( child.id && child.isProperty !== true && !skinObject.id /*&& !skinObject.attributeId && !child.attributeId && !context.isMakeView*/ )
                    {
                        setViewportComponent( skinObject );
                    }
...
function getServerComponentProperties(classModule, isMakeView, component, imports, loaded, body, property, newInstances, invokeContents , clientInitComponents )
{
    ...
    } else
    {
        ...
        {
            var newClass = 'var ' + component.id + ':' + value + '= new ' + value + '(' + makeNewInstanceId(component) + ');\n';
            newInstances.push( newClass );
            if( clientInitComponents )
            {
    ...
    {
        if( component.async && !component.viewport && component.parent && component.parent.attributeId )
        {
            component.viewport = "Element('#" + component.parent.attributeId + "')";
        }
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    //客户端要拉取的数据对象
    var clientPullDataHandle=getUniqueVar(classModule);

    //客户端要拉取的数据
    var clientPullData=[];

    //客户端要拉取的数据的代码块
   var clientPullDataGetInstance = "var "+clientPullDataHandle+":Object=Interaction.pull("+paramName+".getUniqueId());\n";
   if( isMakeView === true ){
       clientPullDataGetInstance = "var "+clientPullDataHandle+":Object=Interaction.pull('"+classModule.fullclassname+"');\n";
   }else{
       serverPushData.push( "'"+classModule.classname+"':this");
   }

    //嵌套的组件
    Utils.forEach(components, function ( component )
    {
        if( component.isPrivate !==true && component.isSkinComponent !== true && isClientRunning(true,originSyntax) )
        {
            serverPushData.push( "'"+component.id+"':"+component.id+"" );
            clientPullData.push("this." + component.id + "=new Skin("+clientPullDataHandle+"."+component.id+");\n");
        }
        //标记为异步的组件都运行在客户端
       if( component.async )
       {
           ...
           getServerComponentProperties(classModule, false, component, imports, loaded,
    ...
    //导入SkinEvent类
    var skinEvent = "es.events.InteractionEvent";
    ...
    if( classModule.syntax !=="javascript" )
    {
    ...
    //判断运行环境
    var runPlatform = Compile.isConformRunPlatform(hostComponentModule,  originSyntax );
    ...
    var serverPushContent='';

    //推送属性到客户端
    if( serverPushData.length > 0 && isClientSupport)
    {
        if ( isMakeView=== true)
        {
            serverPushContent="Interaction.push('"+classModule.fullclassname+"',{" + serverPushData.join(",") + "});\n";
        }else
        {
            serverPushContent="Interaction.push("+paramName+".getUniqueId(),{"+serverPushData.join(",")+"});\n";
        }
    }
    ...
    if ( isMakeView=== true)
    {
         content.push( makeConstruct(classModule,paramName, ["super("+paramName+");\n"].concat(body,serverPushContent),hostComponent, applicationClass, isClientSupport ? classModule.syntax : null ) );

         //如果需要生成一个前端的构造函数
         if( isClientSupport )
         {
             var clientBody = ["super("+paramName+");\n", clientScript.newInstances.join("") , clientScript.body.join(""), clientInitComponents.join("") ];
             if( clientPullData.length > 0 )
             {
                 clientBody = clientBody.concat( clientPullDataGetInstance,clientPullData );
             }
    ...
    } else
    {
        ...
        content.push( makeConstruct(classModule,paramName,["super("+superParam+");\n"].concat(body,serverPushContent),hostComponent, ownerType, isClientSupport ? classModule.syntax : null ) );
        if( isClientSupport )
        {
            var clientBody = [ clientInitComponents.join("") ];
            clientBody = clientBody.concat(clientPullDataGetInstance, "super("+clientPullDataHandle+"."+classModule.classname+");\n" ,clientPullData );

            var wrap= ["if( "+paramName+".async !==true ){\n"];
            wrap.push( clientBody.join("") );
            wrap.push( "}else{\n" );
            wrap.push("super("+superParam+");\n")
            wrap.push( body.join("") );
            wrap.push( "}\n" );
            content.push( makeConstruct(classModule, paramName,wrap,hostComponent, ownerType, "javascript" ) );
        }
    }
    ...
    content.push( '\n}\n}' );
    if( classModule.fullclassname==="es.skins.NavigateSkin") {
...
function makeConstruct( classModule, paramName, body, hostComponent, ownerClass , syntax) {
    ...
    wrap.push('public function ' + classModule.classname + '('+paramType+')' + '{\n' + code.join("") + '}\n');
    return wrap.join("");
}
