...
function parseTextBinding(text,context)
{
    ...
    while ( match=regexp.exec(text) )
    {
        ...
        else if( name.indexOf(".") < 0 && !twoWayBind )
        {
           refs.push( name );
        }
        ...
        if( hash !== true )
        {
            ...
            if( !skinObject.isCommand )
            {
                ...
                if( bind.result===true )
                {
                    ...
                    skinObject.bindAttrs[ attrName ]=bind;
                ...
                }else
                {
                    ...
                    if( bind.result===true )
                    {
                       ...
                       /* var _bindContext = skinObject.isProperty ? skinObject.module : child;
                        ...
                        {
                            (_bindContext.oneWayBinding || (_bindContext.oneWayBinding=[])).push({
                                name:"content",value:child.attr.content,bind:bind,target:child,isAttr:true
                            });
                        }
            ...
            if( !(isSkin && name ==="name") )
            {
                ...
                if( desc && desc.isAccessor && desc.value && desc.value.set )
                {
                    properties.push({
                        ...
                        desc:desc,
                        bind:bind,
                        value:bind ? value : getValueByType( value, desc.paramType[0] , context , topScope )
                    });
...
* 包含带有状态机的双向绑定属性也一同返回
*/
function getAttributesTwoWayBinding( skin, context, topScope )
{
    var content=[];
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    //一个组件或者是元素
        ...
        var properties = getComponentDefinedProperties( skin, context , _topScope );
         //双向绑定的属性
        var propertyTwoWayBinding = getAttributesTwoWayBinding( skin, context, topScope );
         //是否为公开的元素
        var isPublic = skin.id && skin.isPrivate !==true;
        //此元素是否在其它地方有被引用到
        var hasReference = isPublic || properties.length > 0 || skin.hasChildCommand ||
        (skin.isComponent && propertyTwoWayBinding && !skin.isDisplayElement);
        ...
        if( !innerHtml ){
           innerHtml = elementAttributes.innerHTML;
        }
        ...
        else if( skin.isComponent )
        {
            ...
            if( skin.isDisplayElement )
            {
                ...
                else if( innerHtml )
                {
                 ...
                 //如果有双向绑定
                if( propertyTwoWayBinding  )
                {
                    //生成双向绑定代码
                    //componentContent.push( propertyTwoWayBinding( createUniqueRefId(skin) ,true ) );
                }
                ...
                if( bindEvents.length > 0 )
                {
                    componentContent.push( DOMRender+".bindEvent("+[
                        "uniqueKey",createUniqueRefId(skin),"{"+bindEvents.join(",")+"}", topScope.Context
                    ].join(",")+");");
                }
            ...
            }else
            {
                if( hasReference )
                {
                    ...
                    {
                        //生成双向绑定代码
                        //childContent.push( propertyTwoWayBinding( createUniqueRefId(skin) ,true ) );
                    }
        ...
        //创建一个元素
            ...
            //双向绑定属性
            if( propertyTwoWayBinding.length > 0 )
            {
                indexAt=6;
                param.splice(indexAt,1, '['+propertyTwoWayBinding.join(",")+']' );
            }
