...
var contentModuleMapHash = [];
function getCurentContextModules( context )
{
    var i = 0;
    var len = contentModuleMapHash.length;
    for(i<len;i++;)
    {
        if( contentModuleMapHash[i].context === context )
        {
            return contentModuleMapHash[i].module;
        }
    }
}

function setCurentContextModules(context, module)
{
    contentModuleMapHash.push({context:context,module:module});
}

//当前视图上下文模块
var viewCurrrentContextModule;
var moduleContents = [];
