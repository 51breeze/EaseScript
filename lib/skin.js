...
function makeScript( classModule , skinObject , isMakeView )
{
    ...
    var findElem = `
        ...
        {
            elem = this.findElement( this.getAttrName( (hostComponent as SkinComponent).getComponentId() ) );
        }
    `;
    ...
    if( classModule.fullclassname==="es.skins.DataGridSkin")
    {
...
function getComponentPropertyDescriptor( propertyObject, value,component, context, topScope )
{
    ...
    if( target.items.length > 0 )
    {
        ...
        {
            var type = (desc.paramType && desc.paramType[0] ? desc.paramType[0] : desc.param && desc.param[0] ? desc.param[0] : defaultType) || defaultType;
            if( type === "*" || !type )
            {
