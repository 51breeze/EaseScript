...
function makeSkinParam(skin, flag)
{
    ...
    if( flag !== false )
    {
        if( skin.children.length===1 && typeof skin.children[0] === "string" )
        {
           ...
           // childStr = getSkinChildToString(skin);
            for( var index in skin.children )
            {
                var child = skin.children[index];
                if( !(child.nonChild || child.isProperty || child.isComponent || child.isSkin) )
                {
                    (skin.context.initProperties).push((skin.id || "this") + ".addChildAt( new Skin('" + __toString(child) + "')," + child.childIndex + ")");
                }
            }
        }
    }
...
function parseTextBinding(context,parent,text, childIndex )
{
    ...
    if( has  )
    {
        ...
        if( parent.isTitleElement ){
            context.initProperties.push("document.title="+content);
        }else
        {
 ...
 * @param context
 */
function getServerViewHeadAndBodyElement(viewObject , context)
{
    var content = [];
...
var makeNewInstanceIdHash = {};
function makeNewInstanceId( classModule )
{
    var id=  Math.round( Math.random() * 100000 );
    if( makeNewInstanceIdHash[ id ] !==true )
    {
        makeNewInstanceIdHash[ id ] = true;
        if( classModule.context.isMakeView !==true )
        {
            return "hostComponent.getUniqueId()+'"+id+"'";
        }
        return id;
    }
    return makeNewInstanceId(classModule);
}

function getServerComponentProperties(classModule, isMakeView, component, imports, loaded, body, property, newInstances, invokeContents , clientInitComponents)
{
    ...
    } else
    {
        ...
        {
            var newClass = 'var ' + component.id + ':' + value + '= new ' + value + '();\n';
            if( !component.async ) {
                newInstances.push(newClass);
            }
            if( clientInitComponents && component.async )
            {
    ...
    //组件属性
    if (component.initProperties && component.initProperties.length > 0 && !component.async )
    {
        body.push(component.initProperties.join(";\n") + ";\n");
    }

    //组件初始化属性，视图需要单独去生成所以在这里不添加。
    if (component.isDisplayElement && !component.elementAdded && !component.async )
    {
        if( component.viewport )
        {
...
function buildExpressionNewOf(classModule, component, classType, param )
{
   classType = classType.split(".").pop();
   return makeSkin.syntaxBuilder.javascript.buildExpressionNewOf(classModule.config,component,classType,{descriptor:{id:"class",nonglobal:true}},[classType],[param],classType);
}

function buildExpressionSetOf(classModule, component, thisarg, name , value)
{
   return makeSkin.syntaxBuilder.javascript.buildExpressionSetOf(classModule.config,component,thisarg,{descriptor:{id:"function",nonglobal:true,isAccessor:true,value:{set:true}}},
        [name], [thisarg,name],[ value ],'=','_');
}

function buildExpressionCallOf(classModule, component, thisarg, name , value)
{
    return makeSkin.syntaxBuilder.javascript.buildExpressionCallOf(classModule.config,component,null,thisarg,
        {descriptor:{id:"function",nonglobal:true},param:[value||'']},
        [name], [thisarg,name],'_');
}

function buildExpressionSuperOf(classModule, component, superClass, value)
{
    return makeSkin.syntaxBuilder.javascript.buildExpressionCallOf(classModule.config,component,{nonglobal:true},'this',
        {descriptor:{id:"class",nonglobal:true}, "super":superClass,  param:[value||'']},
        [], [],'');
}

function getAllComponentInjectionProperties( component )
{
        ...
        Utils.forEach(desc.proto,function (value,name) {
            if(value.defineMetaTypeList && value.defineMetaTypeList.Injection)
            {
                injections.push({
                    ...
                    "proto":true,
                    "type":value.defineMetaTypeList.Injection.param.value,
                    "name":name,
                    "id":value.id,
...
function getClientInitComponentsFromServerMake(classModule, handle , originSyntax )
{
    var content=[];
    var requirements={};
    var map={};
    ...
    var setRequirements=function (fullclassname)
    {
        var name = fullclassname.split(".").pop();
        if (map[name] && map[name] !== fullclassname) {
            requirements[fullclassname] = fullclassname.replace(/\./g, "_");
        } else {
            requirements[fullclassname] = name;
            map[name] = fullclassname;
        }
    }
    ...
    Utils.forEach(classModule.components, function ( component )
    {
        if( !component.async )
        {
            if ( component.isSkinComponent )
            {
                ...
                setRequirements( classType );
                setRequirements( skinClass );

                var event = getUniqueVar(classModule);
                events.push("var " + event + ":es.events.SkinEvent=new es.events.SkinEvent(\"initSkinComponent\");\n");
                events.push(component.id + ".skin.dispatchEvent(" + event + ");\n");
                content.push("var " + component.id + "=" + buildExpressionNewOf(classModule, component, requirements[classType]) + ";\n");

                var injectionProperties = getAllComponentInjectionProperties( component );
                Utils.forEach( injectionProperties , function (item) {
                    var injection = getUniqueVar(classModule);
                    if( item.type )
                    {
                        injectionContents.push("var " + injection + ":String ="+ component.id+".valueToString("+component.id+"."+item.name+","+item.type+");\n");
                        content.push(buildExpressionSetOf(classModule, component, component.id, item.name,
                            buildExpressionNewOf(classModule, component, item.type, ["{Event{" + injection + "}}"])) + ";\n");
                        setRequirements( item.type );

                    }else
                    {
                        injectionContents.push("var " + injection + ":String ="+ component.id+".valueToString("+component.id+"."+item.name+");\n");
                        content.push(buildExpressionSetOf(classModule, component, component.id, item.name,"{Event{" + injection + "}}")+";\n");
                    }

                });

                content.push(buildExpressionSetOf(classModule, component, component.id, "skin",
                    buildExpressionNewOf(classModule, component, requirements[skinClass], [component.id, "{Event{" + event + ".content}}"])) + ";\n");

                if (component.isPrivate !== true)
                {
                    content.push(buildExpressionSetOf(classModule, classModule, "this", component.id, component.id) + ";\n");
                }
                content.push(buildExpressionCallOf(classModule, classModule, "this", "addChildAt", [component.id, component.childIndex ].join(",") ) + ";\n");

            } else if (component.isPrivate !== true)
            {
                setRequirements( classModule.baseSkinclass );
                content.push(buildExpressionSetOf(classModule, classModule, "this", component.id, buildExpressionNewOf(classModule, component, "Skin", '"#{ID{' + component.id + '}}"')) + ";\n");
            }
        }
    });
    ...
    {
        param.push( requirements[p] );
    }

    if( !classModule.isMakeView )
    {
        self = getUniqueVar(classModule);
        content.unshift( buildExpressionSuperOf(classModule,classModule,"Skin","\"#{ID{"+self+"}}\"")+";\n" );
    }

    content.unshift("function("+param.join(",")+"){\n");
    //content.push("return \"#{ID{"+self+"}}\";\n}");
    content.push("\n}");
    content = content.join("");
    content = content.replace(/\{ID\{(.*?)\}\}/g,"'+$1.generateId()+'");
    content = content.replace(/\{Event\{(.*?)\}\}/g,"'+$1+'");
    ...
    if( classModule.isMakeView )
    {
        ...
        wrap+=events.join("");
        wrap+=injectionContents.join("");
        wrap += id+".content='window[\""+handle+"\"]="+content+"';\n";
        wrap += id+".content="+id+".content.replace(/\\n/,\"\\n\");\n";
        wrap += "document.head.addChild( "+id+" );\n";
        return {content:wrap,requirements:requirements,map:map,requirementsClassName:param };
    }
    ...
    wrap+=events.join("");
    wrap+=injectionContents.join("");
    wrap+="this.addEventListener('initSkinComponent',function(event:SkinEvent){\n event.content='"+content+"';\n});\n";
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    {
        getServerComponentProperties(classModule, isMakeView , component, imports, loaded, body, property, newInstances, invokeContents , clientInitComponents);
    });
   ...
   if( classModule.isMakeView )
   {
    ...
    if( isMakeView===true )
    {
        var headContent = getServerViewHeadAndBodyElement(skinObject, classModule);
    ...
    //从服务端生成皮肤组件到前端
    var skinFactoryComponents = {};
    if( isClientSupport )
    {
        skinFactoryComponents = getClientInitComponentsFromServerMake(classModule, clientViewInitializationHandle, originSyntax);
    }
    ...
    //皮肤生成器需要的组件类
    var skinFactoryParameter=['this'];
    Utils.forEach(skinFactoryComponents.requirements,function (val, key) {
        skinFactoryParameter.push(key);
    });
    ...
    {
         //组装代码
         body = ["super("+paramName+");\n", declarationReference.join(""), newInstances.join("") ].concat(body,invokeContents,skinFactoryComponents.content);

         //生成构造函数
         content.push( makeConstruct(classModule,paramName,body,hostComponent, applicationClass, isClientSupport ? classModule.syntax : null ) );
         ...
         if( isClientSupport )
         {
             var clientBody = [
                 "super("+paramName+");\n",
                 "var initialization:Function =  window['"+clientViewInitializationHandle+"'] as Function;\n",
                 "initialization.call("+skinFactoryParameter.join(",")+");\n",
    ...
    } else
    {
        ...
        var originBody = ["super("+superParam+");\n", declarationReference.join(""), newInstances.join("") ].concat(body);
        if( isClientSupport )
        {
            originBody.push( invokeContents,skinFactoryComponents.content );
        }
        content.push( makeConstruct(classModule,paramName,originBody,hostComponent, ownerType, isClientSupport ? classModule.syntax : null , true) );
        if( isClientSupport )
        {
            var wrap= ["if( skinFactory ){\n"];
            wrap.push("skinFactory.call(" + skinFactoryParameter.join(",") + ");\n");
            wrap.push( "}else{\n" );
            wrap.push("super("+superParam+");\n");
            wrap.push( [ declarationReference.join(""), newInstances.join("") ].concat(body).join("") );
            wrap.push( "}\n" );
            wrap.push( invokeContents );
            wrap.push( stateContent );
            wrap.push( bindableContent );
            content.push( makeConstruct(classModule, paramName,wrap,hostComponent, ownerType, "javascript", true ) );
        }
    }
    ...
    content.push( '\n}\n}' );
    if( classModule.fullclassname==="es.skins.PopUpSkin") {
