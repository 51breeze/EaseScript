...
function __toMakeSkinFactory(skinChildren, context, topScope, parentObject )
{
    ...
    var beforeStack =topScope.beforeStack || [];
    var contentStack =topScope.contentStack || [];
    var children = [];
...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    for(var p in attrs)
    {
        ...
        //状态机属性,两者只能取其一 
        if( lowerAttrName === "excludefrom" || lowerAttrName==="includein" )
        {
            if( skinObject.definedIncludeState )
            {
                throw new ReferenceError("'"+name+"' conflicts with '"+skinObject.definedIncludeState.name+"'' attribute.");
            }
            var parseValue = parseValueString(attrValue);
            skinObject.definedIncludeState={
                name:name,
                value:attrValue,
                logic:lowerAttrName === "excludefrom" ? "!":"",
...
function getComponentDefinedProperties( component , context, topScope )
{
    ...
    component.defineAttributeStates= hasState ? defineAttributeStates : null;
...
* 获取已经定义的状态属性
*/
function getDefinedStateProperties(skin, stateObject, stateHandle )
{
    if( !stateObject )
    {
        return null;
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    //一个组件或者是元素
        ...
        //生成元素的状态机
        var stateProperties = getDefinedStateProperties(skin,skin.defineAttributeStates, topScope.StateHandle);
        if( stateProperties )
        { 
        ...
        else if( skin.isComponent )
        {
            ...
            if( properties.length > 0 )
            {
                componentProps = makeComponentPropertiesMothod(skin, properties);
                if( componentProps && componentProps.length > 0 )
                {
