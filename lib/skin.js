    ...
    if( flag !== false )
    {
        ...
        if (childStr)
        {
            if( skin.children.length===1 && typeof skin.children[0] === "string" ) {
                attrStr.push('"innerHtml":"'+childStr.replace(/"/g,'\\"')+'"')
            }else{
                var childIndexAt = skin.childIndex;
                if( skin.context === skin )
                {
                    for( childIndexAt in skin.children )
                    {
                        var child = skin.children[childIndexAt];
                        if( !(child.nonChild || child.isProperty || child.isComponent || child.isSkin) )
                        {
                            (skin.context.initProperties).push("this.addChildAt( new Skin('" + __toString(child) + "')," + childIndexAt + ")");
                        }
                    }

                }else
                {
                    (skin.context.initProperties).push((skin.id || "this") + ".addChildAt( new Skin('" + childStr + "')," + childIndexAt + ")");
                }
            }
        }
    }
...
function getClientInitComponentsFromServerMake(classModule, handle )
{
    var content=[];
    var requirements={};
    var map={};
    ...
    Utils.forEach(classModule.components, function ( component )
    {
        if( !component.async )
        {
            if (component.isSkinComponent)
            {
                ...
                var classTypeName = classType.split(".").pop();
                var skinClassName = skinClass.split(".").pop();
                if (map[classTypeName] && map[classTypeName] !== classType) {
                    requirements[classType] = classType.replace(/\./g, "_");
                } else {
                    requirements[classType] = classTypeName;
                    map[classTypeName] = classType;
                }

                if (map[skinClassName] && map[skinClassName] !== skinClass) {
                    requirements[skinClass] = skinClass.replace(/\./g, "_");
                } else {
                    requirements[skinClass] = skinClassName;
                    map[skinClassName] = skinClass;
                }
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    //设置组件为异步
    var setComponentsAsync=[];

    //嵌套的组件
    Utils.forEach(components, function ( component )
    {
        if( component.isSkinComponent ===true )
        {
            setComponentsAsync.push( component.id +".async=true;\n" );
        }
   ...
   if( classModule.isMakeView )
   {
    ...
    //从服务端生成皮肤组件到前端
    var skinFactoryComponents = getClientInitComponentsFromServerMake(classModule, clientViewInitializationHandle );
    ...
    var isClientSupport = isClientRunning(true, classModule.syntax) && runPlatform;
    if( !runPlatform )
    {
    ...
    } else
    {
        ...
        if( isClientSupport )
        {
            ...
            wrap.push( [ declarationReference.join(""), newInstances.join("") , setComponentsAsync.join("") ].concat(body).join("") );
            wrap.push( "}\n" );
