...
function makeScript( classModule , skinObject , isMakeView )
{
    ...
    var superValue = 'super(' + (classModule.name === "Skin" ? '"div"' : '"'+classModule.name+'"')+ ');\n';
    var superClientValue = 'super( this.findElement() );\n';
    if ( isMakeView === true)
    {
    ...
    //一个组件或者是元素
        ...
        if( skin === context )
        {
            ...
            if( properties.length > 0 )
            {
                ...
                if( componentProps )
                {
                    if( componentProps.init.length > 0 )
                    {
                        ...
                        topScope.clientScope.topStack.push( componentProps.init.join("\n") );
                    }
            ...
            if( propertyTwoWayBinding.content.length > 0 )
            {
                ...
                topScope.clientScope.topStack.push( propertyTwoWayBinding.content.join("\n") );
            }
        ...
        else if( skin.isComponent )
        {
            ...
            if( skin.isDisplayElement )
            {
                ...
                newClientValue =  skin.ownerForeachCommand ? DOMRender+".findComponent("+param.slice(1,3).join(",")+",true)" : DOMRender+".findComponent("+param.slice(1,3).join(",")+")";
            ...
            else
            {
                ...
                if( hasReference )
                {
                    if( skin.isStateComponent )
                    {
                        ...
                        topScope.clientScope.topDeclaration.push( "var "+createUniqueRefId(skin)+":"+classType+"="+newValue+";" );
        ...
        //创建一个元素
            ...
            var newClientExpression = DOMRender+".findElement("+param.slice(1,3).join(",")+")";
            if( skin.ownerForeachCommand ){
                newClientExpression = DOMRender+".findElement("+param.slice(1,3).join(",")+",true)";
            }
            if( skin.definedIncludeState && !(isPublic && !skin.ownerForeachCommand) )
            {
...
{
    var def = void 0;
    var events = Object.assign({}, bindAttributes ? bindAttributes.events||def : def, stateProperties ? stateProperties.events || def :def);
    events = Object.entries(events).map( (value)=>{
        return value[0]+":"+value[1];
    });

    var isPublic = skin.id && skin.isPrivate !==true;
    var find = skin.ownerForeachCommand ? `this.findElement(${uniqueKey},"${skin.name}",true)` : `this.findElement(${uniqueKey},"${skin.name}")`;
    ...
    if( skin.isSkinComponent )
    {
        if( !skin.definedIncludeState )
        {
            expression+= `${target} && ${target}.display();\n`;
        }else{
