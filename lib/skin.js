...
function __toString(skin, notContainer , parent, separation, makeSyntax )
{
    ...
    if( tag.charAt(0) ===":" )
    {
        ...
        return syntax[tag](attr,content, children.length > 0 );
    }
...
var template_syntax={
    ...
    'php': {
        ...
        'for': function (attr, content) {
            var condition = attr.condition.replace(/^var/,'');
            condition = replace_condition(condition, "php");
            return '<?php for(' + condition +'){ ?>' + content + '<?php}?>';
        },
        'if': function (attr, content) {
            return '<?php if(' + replace_condition(attr.condition, "php") + '){ ?>'+content+'<?php}?>';
        },
        'elseif': function (attr, content) {
            return '<?php elseif(' +  replace_condition(attr.condition, "php") + '){ ?>'+content+'<?php}?>';
        },
        'else': function (attr, content, hasChild ) {
            if( hasChild )
            {
        ...
        'switch': function (attr, content) {
            return '<?php switch(' +  replace_condition(attr.condition, "php") + '){ ?>'+content+'<?php}?>';
        },
        'case': function (attr, content) {
            content = '<?php case "' + attr.condition + '": ?>'+content;
        ...
        'while': function (attr, content) {
            return '<?php while(' +  replace_condition(attr.condition, "php") + '){ ?>'+content+'<?php}?>';
        },
        'code': function (attr, content) {
            return '<?php '+content+' ?>';
        },'script': function (attr, content) {
...
{
    return 'Q'+ uid() + '_' +(new Date()).getTime();
}
...
function makeScript( classModule , skinObject )
{
    ...
    {
        body.push('this.addEventListener("INTERNAL_BEFORE_CHILDREN" ,function(event){\n' + invokeContents.join("") + '});\n');
    }
