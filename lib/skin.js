...
function parseValueString(value, refer)
{
    ...
    return {value:value,has:is,okValue:is ? value : '"'+escapeValueString(value)+'"'};
}
...
function createComponentObjectOfValue(skinObject, context, topScope)
{
    ...
    /*if( skinObject.description && 
        (checkInstanceOf(context, skinObject.description, "es.interfaces.IRender", true) || 
        checkInstanceOf(context, skinObject.module.description, context.baseSkinclass) ) )
    {
         
    }*/

    var content = makeRenderScript(skinObject,context,skinObject,topScope,topScope.forLevel,topScope.keyPrefix,children);
    if( content )
    {
...
//private
function __toMakeSkinFactory(skinChildren, context, topScope, parentObject )
{
    ...
    Utils.forEach(skinChildren,function(skinObject,index){
        ...
        var content = makeRenderScript(skinObject,context,skinObject.parent,_topScope,0,'',children);
        if( content )
        {
...
function makeClientScript( classModule , skinObject , isMakeView )
{
    ...
    if ( isMakeView === true)
    {
         if( hostComponent )
         {
            ...
            body.unshift('var hostComponent:'+hostComponent+'=context as '+hostComponent+';\n');
        }
...
function getComponentPropertyDescriptor( propertyObject, value,component, context, topScope )
{
    ...
    if( lastPropertyObject.isProperty && lastPropertyObject.attrdata.content && value instanceof Array && value.length=== 0 )
    {
        ...
        Utils.forEach(component.attrdata,function(value,name)
        {
            ...
            if( !(isSkin && name ==="name") )
            {
                ...
                }else if( (isSkin || component.isDisplayElement) && !component.isSkinComponent )
                {
                    if( stateName )
                    {
                        var states = defineAttributeStates[ name ] || (defineAttributeStates[ name ]={items:[],value:'""'});
                        if( component.attributes[name] )
                        {
                            states.value = component.attributes[name];
                            delete component.attributes[name];
                        }
                        states.items.push({value:value,name:name,stateName:stateName});

                    }else
                    {
                        //优先使用状态属性
                        if( defineAttributeStates[ name ] ){
                            defineAttributeStates[ name ].value =value;
                        }else{
...
function getComponentConstructParams(component , context, topScope, isSkin , attrString , childScope )
{
    ...
    }else
    {
        ...
        //不是一个皮肤对象
        else if( !(component.isSkin || isSkin) )
        {
            //每一个参数的类型必须对应
            params = paramTypes.map(function(type){
                if( type ==="Element" )
                {
                    ...
                    if( !value || !value.match(/^\[a-zA-A]+\w+$/) )
                    {
                        ...
                        //{
                             //value = elem.attrdata.name || elem.attrdata.value || elem.name;
                        //}else{
    ...
    if( component.isDisplayElement )
    {
        ...
        {
            var content = makeRenderScript(elements.shift(),context,component,topScope,topScope.forLevel,topScope.keyPrefix,childScope.childrenRefs);
            if( content )
            {
...
function getDefinedStateProperties(skin, stateObject, stateHandle, topScope )
{
    ...
    Utils.forEach(stateObject,function(state,name)
    {
        ...
        for(;index<len;index++)
        {
            ...
            expression.push( stateHandle+".includeIn("+parseValueString(item.stateName).okValue+") ? "+item.value.replace(/^\s*this/, topScope.Context) )
        }
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    //一个组件或者是元素
        ...
        if( context.isMakeView )
        {
            switch( skin.name.toLowerCase() )
            {
                case "title" :
                    if( bindProperties )
                    {
                        var content = bindProperties.content||bindProperties.innerHTML;
                        childContent.push("hostComponent.title="+(bindProperties.content||bindProperties.innerHTML||'""')+" as String;");
                    }
                case "meta" :
                case "script" :
        ...
        //是否有为此组件定义属性
         var _topScope ={
            declaration: skin === context ? topScope.topStack : topScope.declaration,
            beforeStack:topScope.beforeStack,
            properties:topScope.properties,
            afterStack:topScope.afterStack,
            childCount:topScope.childCount,
            contentStack:skin === context ? topScope.topStack : childContent,
            DOMRender:topScope.DOMRender||"DOMRender",
            Context:topScope.Context||"Context",
            ContextType:topScope.ContextType,
            StateHandle:"stateGroup",
            keyPrefix:topScope.keyPrefix,
            forLevel:topScope.forLevel
        }

        if( skin.isComponent && skin.isSkin )
        {
            _topScope.ContextType = skin.fullclassname;
        }
        ...
        var staticAttr = {};
        var dynamicAttr = [];
        var dynamicInnerHTML = !!innerHtml;
        var bindEvents = [];
        ...
        if( stateProperties )
        { 
            for( var p in stateProperties )
            {
                var eventType = Utils.getEventType( p );
                if( eventType ){
                    bindEvents.push( '"'+eventType+'":'+stateProperties[p].replace(/^\s*this/, topScope.Context) );
                }else{
                    dynamicAttr.push('"'+p+'":'+stateProperties[p]);
                }
            }
        }
        ...
        { 
            for( var p in bindProperties.attrs )
            {
                ...
                if( !(p === "innerHTML" && innerHtml) )
                {
                    var eventType = Utils.getEventType( p );
                    if( eventType ){
                        bindEvents.push( '"'+eventType+'":'+bindProperties.attrs[p].replace(/^\s*this/, topScope.Context) );
                    }else{
        ...
        if( skin === context )
        {
            ...
            if( properties.length > 0 )
            {
                ...
                {
                    topScope.topStack.push( componentProps.join("\n") );
                }
            }
        ...
        else if( skin.isComponent )
        {
            ...
            {
                var _childScope={
                    childrenRefs:childrenReference,
                    contentStack:childContent,
                    hasChildren:hasChildren,
                }
                newComponentParam = getComponentConstructParams(skin,context,topScope,false,attrRefName, _childScope );
            ...
            if( properties.length > 0 )
            {
                ...
                if( componentProps && componentProps.length > 0 )
                {
                    componentProps=componentProps.join("\n");
                }
            }
            ...
            if( skin.isDisplayElement )
            {
                ...
                componentContent.push( createUniqueRefId(skin) +'= '+ newValue+';' );
                ...
                if( skin.isSkin )
                {
                     if( innerHtml )
                     {
                        componentContent.push( createUniqueRefId(skin)+".render.factory=function(){\n return ["+innerHtml+"];\n}" ); 

                     }else
                     {
                        if( !skin.hasDefinedRenderProperty )
                        {
                            var oldContextType = topScope.ContextType;
                            topScope.ContextType = classType;
                            var childSkinFactory = __toMakeSkinFactory(skin.children, context, topScope, skin );
                            topScope.ContextType = oldContextType;
                            if( childSkinFactory.content )
                            {
                                componentContent.push( createUniqueRefId(skin)+".render.factory="+childSkinFactory.content+";" ); 
                            }
                        }
                        ...
                        {
                            componentContent.push( childContent.join("\n") );
                            //清除已有的子级内容
                            childContent.splice(0, childContent.length);
                        }
                    }
                    innerHtml = null;
                    hasChildren = false
                ...
                {
                    var express = createUniqueRefId(skin)+".element.html("+innerHtml+");";
                    if( skin.isSkin && skin.name )
                    {
                          switch( skin.name )
                          {
                              case "input":
                                express= createUniqueRefId(skin)+".element.value("+innerHtml+");";
                                break;
                              // case "button":
                              // case "textarea":
                              //   express= createUniqueRefId(skin)+".element[0].textContent="+innerHtml+";";
                          }
                    }
                    innerHtml = express;
                    ...
                    if( !dynamicInnerHTML )
                    {
                        componentContent.push( innerHtml );
                        innerHtml = null;
                    }
                }
                ...
                if( componentProps )
                {
                    componentContent.push( componentProps );
                }
                ...
                {
                    componentContent.push( DOMRender+".createChildren("+[ 
                ...
                //元素对象的动态属性
                if( skin.isDisplayElement && dynamicAttr.length > 0 )
                {
                    componentContent.push( createUniqueRefId(skin)+".element.properties({"+dynamicAttr.join(",")+"});");
                }

                //绑定元素事件属性
                if( skin.isDisplayElement && bindEvents.length > 0 )
                {
                    componentContent.push( DOMRender+".bindEvent("+[
                        uukey,uniqueKey,createUniqueRefId(skin),"{"+bindEvents.join(",")+"}", topScope.Context
                        ].join(",")+");");
                }
                ...
                if( hasReference )
                {
                    ...
                    if( skin.definedIncludeState )
                    {
                        childContent.push( 
                            ...
                            DOMRender+".createComponent("+param.join(",")+") : null) as "+classType+";"
                        );
                ...
                {
                    var newComponentValue = DOMRender+".createComponent("+param.join(",")+")";
                    //有定义状态机
                    if( skin.definedIncludeState )
                    {
            ...
            }else
            {
                if( hasReference )
                {
                    childContent.push( "var "+createUniqueRefId(skin)+":"+classType+"="+newValue+";" );
                ...
                if( componentProps )
                {
                    childContent.push( componentProps );
                }
            }
        }
        //创建一个元素
            ...
            if( hasReference )
            {
                ...
                if( skin.definedIncludeState )
                {
                     refNodeTarget = skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+refNodeTarget+":null";
                }
...
function makeComponentPropertiesMothod(component,properties,topScope)
{
    ...
    properties.forEach(function(property)
    {
        ...
        var state = content[ refsName ] || (content[ refsName ]={items:[],defualtValue:null});
        var expre = null;
        ...
        expre = {stateName:stateName,expre:expre,logic:stateLogic,isRef:!!definedIncludeState};
        //如果是包函或者排除需要置空
    ...
    for(var p in content)
    {
        ...
        {
            code.push( state.defualtValue.expre );
        }
    }
    return code;
}
...
function makeElementOneWayBindingContent(skinObject,context, refTarget, topScope )
{
    ...
    Utils.forEach(skinObject.oneWayBinding,function(item)
    {
        ...
        if( item.bind.binding.length < 1 )
        {
            ...
            if( stateName && item.isAttr )
            {
                var states = defineAttributeStates[ name ] || (defineAttributeStates[ name ]={items:[],value:'null'});
                states.items.push({value:item.value,name:name,stateName:stateName});
            }else
            {
                //优先把文本内容当子级 
...
function makeRenderFactoryScript(skinObject, context, topStack, DOMRender,Context)
{
    ...
    if(  (skinObject.defineStates || skinObject.defineStateGroups) && 
        skinObject.description && 
        checkInstanceOf(skinObject, skinObject.description,context.baseSkinclass) )
    {
    ...
    return {topScope:topScope,content:factory};
}
