...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    if (nodes.length > 0)
    {
        ...
        for (var i in nodes) 
        {
           ...
           {
                if( !skinObject.isCommand && nodes.length === 1 )
                {
                    //单向绑定
                    ...
                    if( bind.result===true )
                    {
                        var oneWayBinding = context.oneWayBinding || (skinObject.oneWayBinding=[]);
...
* 获取组件需要的构造参数
*/
function getComponentConstructParams(component , context, isSkin )
{
    var params = [];
    var elements = [];
    ...
    if( isCommand )
    {
        ...
        if( skin.isComponent )
        {
            ...
            if( bindingFlag )
            {
                  ...
                  var bindingContent = ['function('+[bindTarget+":Element",stateName+":Object"].concat(bindingProperties.join(":*,")+":*",bindingRefs.join(":*,")+":*").join(",")+"){\n"];
                  ...
                  var bindingParam = [skin.id,funName].concat( '["'+bindingProperties.join('","')+'"]', bindingRefs );
...
function makeEverySkinElement(skinObject, context, instances, methods)
{
     (skinObject.children||[]).forEach(function(child,index)
     {
         ...
         }else if( child.isComponent )
         {
            ...
            var params = getComponentConstructParams(child , context, true);
