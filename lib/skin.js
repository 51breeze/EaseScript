...
function getServerViewHeadAndBodyElement(viewObject , context, bootstrap )
{
   ...
   if( context.config.script_part_load )
   {
       ...
       baseScript.attr.src= PATH.relative( context.config.build_bootstrap_root_path, PATH.resolve( Utils.getBuildPath(context.config, 'build.js'), "easescript.js" ) ).replace(/\\/g,'/')
   }
