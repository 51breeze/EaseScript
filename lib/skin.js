...
function getServerComponentProperties(classModule, isMakeView, component, imports, loaded, body, property, newInstances,declaration, invokeContents , clientInitComponents, synchSkinProperties )
{
    ...
    if (component.notNewInstance)
    {
        ...
        if (param)
        {
            declaration.push( 'var ' + component.id +_type+';\n' );
           // newInstances.push('var ' + component.id +_type+'=' + param + ";\n");
            newInstances.push( component.id +'=' + param + ";\n");
        }
    ...
    } else
    {
        ...
        {
            var componentId = isMakeView ? "" : "hostComponent.getComponentId('"+component.id+"')";
            var newClass = component.id + '= new ' + value + '('+componentId+');\n';
            if( !component.async )
            {
                newInstances.push(newClass);
                declaration.push( 'var ' + component.id +':'+value+';\n' );
            }
            ...
            //公开的组件属性需要在客户端中实例化
            if (component.isPrivate !== true && !isMakeView)
            {
                 synchSkinProperties[ component.id ] = {value:componentId,type:value,isSkin:false};
            }

        }else
        {
            newInstances.push( component.id + '= new ' + value + '(' + param + ');\n');
            declaration.push( 'var ' + component.id +':'+value+';\n' );
        }
    }
    ...
    if( component.isSkin && !component.isSkinComponent && !isMakeView )
    {
        synchSkinProperties[component.id] = {value:component.id + ".generateId()",type:classModule.baseSkinclass, isSkin:true };
    }
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    Utils.forEach(components, function ( component )
    {
        ...
        {
            assignPropertiesPublicOf.push('this.' + component.id + '=' + component.id + ";\n");
        }
        getServerComponentProperties(classModule, isMakeView , component, imports, loaded, body, property, newInstances,declaration, invokeContents , clientInitComponents, synchSkinProperties);
    });
   ...
   if( classModule.isMakeView )
   {
    ...
    if( isClientSupport )
    {
        skinFactoryComponents = makeClientSkinFactory(classModule, clientViewInitializationHandle, originSyntax);
    }
    ...
    //皮肤生成器需要的组件类
    var skinFactoryParameter=['this'].concat( skinFactoryComponents.requirements );

    //如果是一个视图
    if ( isMakeView=== true)
    {
         //组装代码
         body = ["super("+paramName+");\n", declarationReference.join(""), declaration.join(""), newInstances.join("") ].concat(
             body,
             assignPropertiesPublicOf,
             invokeContents
         );
    ...
    } else
    {
        ...
        var componentIdPrefix = Utils.uid();
        var clientPullContent = 'var '+clientPullHander+':Object=Interaction.pull(hostComponent.getComponentId("'+componentIdPrefix+'"));\n';
        ...
        Utils.forEach( synchSkinProperties , function (item,name) {
              if( item.isSkin )
              {
                  ...
                  if( name === classModule.classname){
                      clientPullSkinProperties.push('super("#"+'+ clientPullHander+'.'+name+')');
                  }else {
                      clientPullSkinProperties.push( name + ' = new ' + item.type + '("#"+' + clientPullHander + '.' + name + ')');
                  }
              }else{
                  clientPullSkinProperties.push( name+' = new '+ item.type+'('+item.value+')');
              }
        });
        ...
        var originBody = ["super("+superParam+");\n", declarationReference.join(""), declaration.join(""), newInstances.join("") ].concat(body, assignPropertiesPublicOf);

        //需要推送皮肤属性到客户端
        if( isClientSupport )
        {
            serverPushSkinProperties  = 'Interaction.push(hostComponent.getComponentId("'+componentIdPrefix+'"),{'+serverPushSkinProperties.join(",\n")+'});\n';
            originBody.push( invokeContents, "if( hostComponent.async !==true ){\n"+serverPushSkinProperties+"}\n" );
        }
        ...
        if( isClientSupport )
        {
            var wrap= [
                declaration.join(""),
                clientPullContent,
                "if("+clientPullHander+" && "+clientPullHander+".initialized !== true ){\n"
            ];
            wrap.push( clientPullHander+ ".initialized=true;\n");
            wrap.push( clientPullSkinProperties.join(";\n") + ";\n");
            wrap.push( "}else{\n" );
            wrap.push("super("+superParam+");\n");
            wrap.push( [ declarationReference.join(""), newInstances.join("") ].concat(body).join("") );
            wrap.push( "}\n" );
            wrap.push( assignPropertiesPublicOf.join("") );
