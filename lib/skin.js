...
var make_skin_factory={
    'foreach': function (attr, content) {
        ...
        var keyname = attr.key ? "var "+attr.key+":*" : '__forKey__';
        var itemname = attr.value ? "var "+attr.value+":*" : "__forItem__";
        return 'for('+keyname+" in "+attr.name+'){\n '+itemname+"="+attr.name+"["+key+"];\n" + content + '\n}';
...
function parseTextBinding(text,context)
{
    ...
    while ( match=regexp.exec(text) )
    {
        ...
        //如是三元表达式，需要增加优先运算符 "()"
        if( match.index>0 && refObject.indexOf("?")>0 && refObject.indexOf(":")>0 && refObject.charAt(0) !=="(" )
        {
           refObject="("+refObject+")";
        }
        ...
        for (var i in nodes) 
        {
           ...
           if( child && typeof child === "string" && !skinObject.isMetadata )
           {
                ...
                }else
                {
                    ...
                    child.ownerRenderComponent=  !!(skinObject && parent.ownerRenderComponent);
...
function getComponentConstructParams(component , context, topScope, isSkin , attrString , childScope )
{
    ...
    //非显示对象组件
    if( !component.isDisplayElement && !component.isSkinComponent )
    {
        var type = paramTypes[0] || componentType;
        if( type ==="*" ){
            type = elements.length > 1 ? "Array" : "Object";
            if( typeof elements[0] === "string" )
            {
               type = /^([\-\+]?[\d\.]+)$/.test( elements[0] ) ? "Number" : /^(true|false)$/i.test( elements[0] ) ? "Boolean" : "String";
            }
        }
        params.push( getValueByType( elements, type, context, topScope , !!(!paramTypes[0] && componentType) ) );
        return params;

    }else
    {
        //Skin Container Display 组件需要的构造参数
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    if( isCommand )
    {
        ...
        if( skin.isForeachCommand )
        {
             // ((skin).childrenPropertiesRefs||[]).forEach(function(item){
             //    if( item.declare ){
             //        code.unshift( "var "+item.target+":Array=[];" );
             //     }
             //     code.push( Context+"."+item.name+"="+item.target+";");
             // });
        }
    ...
    //一个组件或者是元素
        ...
        var isPublic = skin.id && skin.isPrivate !==true;
         //元素定义的属性
        var properties = getComponentDefinedProperties( skin, context , topScope, function(childCommand){
            if( skin !== context )
            {
                var content = makeRenderScript(childCommand,context,parent,topScope,forLevel,keyPrefix,container);
                if( content ){
        ...
        else if( skin.isComponent )
        {
            ...
            var getOrSetElement = skin.isDisplayElement ? DOMRender+".getElement("+[uukey,uniqueKey].join(",")+")||"+DOMRender+".setElement("+[uukey,uniqueKey,newValue].join(",")+")" : newValue;
            ...
            if( skin.isDisplayElement )
            {
                ...
                {
                    //从属性中引用过来 
                    if( isCanDeclareToConstructMethod(skin,isPublic) )
                    {
                        if( skin.ownerForeachCommand )
                        {
                            ...
                            {
                                getOrSetElement = skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+getOrSetElement+":null";
                            }
                        ...
                        }else
                        {
                            ...
                            if( skin.definedIncludeState )
                            {
                                _expression = "("+skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+getOrSetElement+":null) as "+classType;
                            }
                            topScope.declaration.push( "var "+createUniqueRefId(skin)+":"+classType+"="+_expression+";" );
                        }
                    }
                    //有定义状态机
                    else if( skin.definedIncludeState )
                    {
                        getOrSetElement = skin.definedIncludeState.logic+topScope.StateHandle+"."+skin.definedIncludeState.expre+"?"+getOrSetElement+":null";
                    }
                ...
                if( hasChildren || childrenReference.length > 0 )
                {
                    childContent.push( DOMRender+".updateChildren("+[ 
        ...
        //创建一个元素
            ...
            if( hasReference )
            {
                ...
                //元素对象的动态属性
                if( isCanDeclareToConstructMethod(skin,isPublic) && dynamicAttr.length > 0 )
                {
        ...
        if( isPublic && skin !== context )
        {
             ...
             if( skin.ownerForeachCommand )
             {
                 ...
                 //var __parent =skin.ownerForeachCommand;
                 ...
                 //childrenPropertiesRefs.push({name:skinId,target:forContainer,declare:false});
...
//是否可以把公开的元素声明在构造函数中表达式
function isCanDeclareToConstructMethod(skin, isPublic)
{
    return !!(isPublic /*&& !skin.definedIncludeState*/ /*&& !skin.ownerForeachCommand*/);
}
...
function makeRenderFactoryScript(skinObject, context, topStack, DOMRender,Context)
{
    ...
    body.push( "var __forKey__:*=null;" );
    body.push( "var __forItem__:*=null;" );
    ...
    if( context.__hasUseState__ /*skinObject.defineStates && skinObject.description && !Utils.isEmpty( skinObject.defineStates )*/ )
    {
