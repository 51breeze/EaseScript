...
function parseTextBinding(text,context)
{
    ...
    while ( match=regexp.exec(text) )
    {
        ...
        {
           //提取对数据集的引用名
           var matchRefs = name.replace(/([\'\"])[^\1]*\1/g,"").match(/([a-zA-Z]+\w+\.?)+/g);
           if( matchRefs ){
               matchRefs.forEach(function(name){
                    if( name.indexOf(".") < 0 )
                    {
                        refs.push( name );
                    }
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    //一个组件或者是元素
        ...
        //此元素是否在其它地方有被引用到
        var hasReference = isPublic || properties.length > 0 || skin.hasChildCommand || (propertyTwoWayBinding.content.length > 0);
        //获取静态属性
        ...
        var rootElement =  skin === context || (context.isMakeView && skin.name.toLowerCase()==="body" && skin.parent === context);
        ...
        if( rootElement )
        {
            ...
            {
                childContent.push( DOMRender+".watch("+Context+".element,["+propertyTwoWayBinding.content.join(",")+"]);" );
            }
        }
        ...
        if( skin === context )
        {
            ...
            if( attrRefName )
            {
                topScope.topStack.push( "this.render.attributes(this.element,"+attrRefName+");");
            }
            //元素对象的动态属性
            ...
            {
                childContent.push( DOMRender+".bindEvent("+Context+".element,{"+bindEvents.join(",")+"};");
            }
        ...
        else if( context.isMakeView && skin.name.toLowerCase()==="body" && skin.parent === context )
        {
            ...
            if( attrRefName )
            {
                childContent.push( DOMRender+".attributes("+bodyId+".element,"+attrRefName+");");
            }
            //元素对象的动态属性
            if( dynamicAttr.length > 0 )
            {
            ...
            else if( skin.isDisplayElement )
            {
                ...
                if( skin.isSkin )
                {
                    if( !skin.hasDefinedRenderProperty )
                    {
                        ...
                        var childSkinFactory = __toMakeSkinFactory(skin.children, context, topScope, skin, innerHtml);
                        ...
                        {
                            componentContent.push( createUniqueRefId(skin)+".render.factory="+childSkinFactoryId+";" ); 
                        }
                        childContent=[];
                    }
                ...
                {
                    componentContent.push( DOMRender+".attributes("+createUniqueRefId(skin)+".element,"+attrRefName+");");
                }
