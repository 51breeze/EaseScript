...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    for(var p in attrs)
    {
        ...
        if( hash !== true )
        {
            ...
            if( !skinObject.isCommand )
            {
                ...
                if( bind.result===true )
                {
                    var oneWayBinding = skinObject.oneWayBinding || (skinObject.oneWayBinding=[]);
                    oneWayBinding.push({name:attrName,value:attrValue,bind:bind});
                    attrValue = null;
    ...
    else if( elem.namespace() )
    {
        ...
        if( !skinObject.isMetadata )
        {
            ...
            else
            {
                ...
                {
                    throw new ReferenceError("Property is not defined or accessor for '"+name+"'");
                } 
    ...
    if (nodes.length > 0)
    {
        var childIndex = skinObject.isCommand ? childIndexAt : 0;
        for (var i in nodes) 
        {
           ...
           if( child && typeof child === "string" && !skinObject.isMetadata )
           {
                if( !skinObject.isCommand )
                {
                    ...
                    if( nodes.length === 1 )
                    {
                        ...
                        {
                            var oneWayBinding = skinObject.oneWayBinding || (skinObject.oneWayBinding=[]);
                            oneWayBinding.push({name:"innerHtml",value:child,bind:bind});
                            child = null;
                        }
                        if( child )
                        {
                            skinObject.attrdata.innerHtml = child;
                        }
                        child = null;
                    ...
                    }else
                    {
                        ...
                        if( bind.result===true )
                        {
                            child.oneWayBinding=[{name:"content",value:child.attr.content,bind:bind}];
                            child.attrdata={};
                            child.attr={};
                        }
                    }
...
function makeClientScript( classModule , skinObject , isMakeView )
{
    ...
    var classProperties = {};
    var topStack = [];
    var renderFactory = makeRenderFactoryScript(skinObject, classModule, topStack, classProperties);
    ...
    body.push("this.render.factory="+renderFactory+";\n");


    var declarationReference = [];
    var applicationClass = "es.core.Application";
    var virtualElementClass = "es.core.VirtualElement";
    var iVirtualElementInterface = "es.interfaces.IVirtualElement";

    if( imports.indexOf(virtualElementClass)<0 )
    {
        imports.push( virtualElementClass );
    } 

    var iRenderClass = "es.interfaces.IRender";

    if( imports.indexOf(iRenderClass)<0 )
    {
        imports.push( iRenderClass );
    }

    if( imports.indexOf(iVirtualElementInterface)<0 )
    {
        imports.push( iVirtualElementInterface );
    }
    ...
    if( isMakeView===true )
    {
        ...
        if( imports.indexOf("es.core.Skin")<0 )
        {
            imports.push( "es.core.Skin" );
        }
...
* 获取指定组件属性的引用
*/
function getComponentPropertyDescriptor( propertyObject, value,component, context)
{
    ...
    if( target.items.length > 0 )
    {
        ...
        if( value )
        {
            target.value= getValueByType( value, desc.paramType[0]||"*" , context );
        }
    }
    return target;
}
...
* 获取组件已经定义的属性
*/
function getComponentDefinedProperties( component , context )
{
    ...
    if( component.isComponent )
    {
        (component.children||[]).forEach(function(child){
            ...
            if( child.isProperty )
            {
                var target = getComponentPropertyDescriptor( child, child.children ,component, context);
            ...
            if( !(isSkin && name ==="name") )
            {
                ...
                if( desc && desc.isAccessor && desc.value && desc.value.set )
                {
                    properties.push({
                        ...
                        desc:desc,
                        value:getValueByType( value, desc.paramType[0] , context )
                    });
...
//生成动态渲染皮肤的脚本
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, parentKeyPrefix )
{
    var DOMRender = "DOMRender";
    var pid = parent && parent.id && parent !== context ? parent.id : "this";
    var key = uid();
    var childIndex = skin.childIndex||0;
    topScope = topScope || {};
    if( typeof skin === "string" )
    {
        var param = [childIndex, (keyPrefix||0), key, '"text"', 'null' ,'"'+skin+'"' ]
    ...
    var afterContent = "";
    forLevel = forLevel||0;
    createUniqueRefId( skin , "__var"+key+"__" );
   
    if( isCommand )
    {
        var lowerName = skin.name.toLowerCase();
        if( (lowerName===":foreach" || lowerName===":for") )
        {
            keyPrefix = "__FOR"+(context.forLevelCount||0)+"__";
           // keyPrefixContent = "var "+keyPrefix+"="+(parentKeyPrefix? parentKeyPrefix : "0")+";";
            keyPrefixContent = "var "+keyPrefix+":int=0;";
            afterContent= keyPrefix+"++;";
            context.forLevelCount = (context.forLevelCount||0)+1;
            skin.isForeachCommand = true;
        }
    }else if(hasChildren)
    {
        childContent.push( "var "+skin.childrenListRefName+":Array=[];" );
    }
    ...
    if( hasChildren )
    {
        for (;c<len;c++)
        {
            ...
            childContent.push( makeRenderScript(child, context, isCommand ? parent : skin , topScope, forLevel, keyPrefix, isCommand ? keyPrefix : parentKeyPrefix ) );
        }
    }

    if( afterContent )
    {
        childContent.push( afterContent );
    }
    ...
    if( isCommand )
    {
        ...
        if( keyPrefixContent )
        {
            code.push( keyPrefixContent );
        }
        code.push( make_skin_factory[ tagName ](attr, childContent.join("\n") , children.length > 0 ) );
        ...
        { 
            if( (skin.name===":foreach" || skin.name===":for")  )
            {
                code.unshift( pid+".removeVirtualElementAll();");
                code.push( pid+".display();");
            }
            var bindFnContent = 'function('+bindCommand.join(':*')+":*"+","+pid+":IVirtualElement){\n"+code.join("\n")+'\n};';
            topScope.declaration.push( "var "+skin.id+":Function = "+bindFnContent );
            var bindParam = ['["'+bindCommand.join('","')+'"]', skin.id, pid ];
            return 'this.binding('+bindParam.join(",")+');';
        } 
        return code.join("\n");
        ...
        var bindProperties = makeElementOneWayBindingContent( skin, context);
        var properties = getComponentDefinedProperties( skin, context );

        //只有在循环下面的元素才计算循环增量值
        if( keyPrefix )
        {
            var _parent = skin.parent;
            while( !_parent.isForeachCommand && _parent.isCommand && (_parent=_parent.parent) );
            if( !_parent.isForeachCommand )
            {
                 keyPrefix = null;
            }
        }
        ...
        var _childIndex = [ childIndex, keyPrefix ].filter(function(val){
             return !!val;
        });
            
        if( _childIndex.length <1 )
        {
            _childIndex.push( childIndex );
        }

        if( skin.isComponent && !skin.isSkin)
        {
            var classname = skin.fullclassname || skin.classname;
            var classType = ":"+(skin.classType ? skin.classType :classname);
            var componentContent = [];
            ...
            {
                componentContent.push( makeComponentPropertiesMothod(skin, properties).join("\n") );
            }
            var code="function(){\n"+componentContent.join("\n")+"\n}";
            var param=[childIndex,(keyPrefix||0),key,code, pid];
            childContent.push( pid+".createComponent("+param.join(",")+");" );

        }else{

            var param=[_childIndex.join("+") ,key,'"'+tag+'"', hasChildren ? skin.childrenListRefName : "null","null","null"];
            var indexAt = 3;
            if( attributes.length > 0 )
            {
                indexAt = 4;
            ...
            {
                 indexAt=5;
                 param.splice(indexAt,1,"{"+bindProperties.attrs+"}");
            }

            param.splice(indexAt+1,2);

            if( hasChildren || (bindProperties && bindProperties.content) || properties.length > 0 ){
                childContent.push( "var "+skin.id+":IDisplay="+DOMRender+".createElement("+param.join(",")+");" );
                childContent.push( parent.childrenListRefName+".push("+skin.id+");"); 
            }else{
                childContent.push( parent.childrenListRefName+".push("+DOMRender+".createElement("+param.join(",")+"));"); 
            }

            if( properties.length > 0 )
            {
                childContent.push( makeComponentPropertiesMothod(skin, properties).join("\n") );
            }
        }
        ...
        if( skin.id && skin.isPrivate !== true )
        {
             topScope.properties[ skin.id ] = skin.classType || skin.fullclassname || skin.classname || "Skin";
             if( skin.ownerForeachCommand ){
                 topScope.properties[ skin.id ] = "Array";
                 childContent.push( "this."+skin.id+".splice("+keyPrefix+",1,"+skin.id+");" );
             }else{
                 childContent.push( "this."+skin.id+"="+skin.id+";" );
             }
        }
    }
...
* 生成一个渲染皮肤的工厂函数
*/
function makeRenderFactory(skinObject, context, beforeStack,classProperties )
{
   var content = skinObject.children.map( function(chid){
         return makeRenderScript(chid, context, skinObject,{declaration:beforeStack,properties:classProperties} );
    });
...
function createUniqueRefId( object , key)
{
    if( !object.id )
    {
        key = key || getUniqueVar();
        object.id = key || getUniqueVar();
        object.isPrivate = true;
    }

    if( !object.childrenListRefName )
    {
        key = uid();
        object.childrenListRefName = "__var$"+key+"__";
    }
    return object.id;
}
...
* 生成指定皮肤对象的单向绑定属性
*/
function makeElementOneWayBindingContent(skinObject,context)
{
    ...
    Utils.forEach(skinObject.oneWayBinding,function(item)
    {
        ...
        if( item.bind.binding.length < 1 ){
             bindAttrs.push( '"'+item.name+'":'+item.value );
        }else{
            //绑定需要变动的属性对象
    ...
    if( binding.length > 0 )
    {
        createUniqueRefId( skinObject );
        var bindTarget = skinObject.id;
        var funName = getUniqueVar(context);
        var stateName = getUniqueVar(context);
        var refsParam = [stateName+":Object"];
        if( skinObject.isComponent )
        {
            refsParam.push( bindTarget+":"+skinObject.fullclassname );
        }else{
        ...
        binding.forEach(function(item)
        {
             if( skinObject.isComponent )
             {
                 bindingContent.push("if("+stateName+"."+item.name+"){\n");
        ...
        var bindingParam = [propsName,funName];
        if( skinObject.isComponent ){
           bindingParam.push( skinObject.id );
        }else{
           bindingParam.push( skinObject.id+".element" );
        }
        bindContent="this.binding("+bindingParam.concat(  bindingRefs ).join(",")+");"; 
    }
    return {content:bindContent,attrs:bindAttrs.join(","), declaration:declarations.join("\n")};
}

/**
* 生成每一个皮肤元素
*/
function makeEverySkinElement(skinObject, parentObject, uniqueKey, context, topStack, beforeStack, contentStack, classProperties, childReference )
{
      //将命令元素转换成对应的脚本
     if( skinObject.isCommand )
     {
         return contentStack.push( makeRenderScript(
                    skinObject, 
                    context, 
                    parentObject,
                    {declaration:beforeStack,properties:classProperties} 
                ));

     }

     var name = skinObject.name.toLowerCase();
      ...
      switch( name )
      {
            ...
            case "meta" :
            case "script" :
                context.gloableElementList.head.children.push( skinObject );
                return;
            case "head" :
                return; 
            break;
     ...
     if( children.length > 0 && (skinObject.isSkin || !skinObject.isComponent) )
     { 
         children.forEach(function(child)
         {
            if( !child.isProperty )
            {
                makeEverySkinElement(child, skinObject, uniqueKey+''+hasChildren, context, topStack, contentStack, contentStack, classProperties, reference );
                if( !child.isCommand )hasChildren++;
            }
         });
     }

    var parentId  = !parentObject ? "RootContainer" : parentObject.childrenListRefName;

    //获取在一个组件上已经定义的属性
    var properties = getComponentDefinedProperties( skinObject, context );
    var bindProperties = makeElementOneWayBindingContent(skinObject,context);
    var isPublic = skinObject.id && skinObject.isPrivate !== true;
    var instanceStack = isPublic ? topStack : beforeStack;
    var isSingle = properties.length < 1 && !bindProperties && !isPublic;
    var value=null;

     //body 是一个全局对象，所以只需要引用
     if( name==="body" )
     {
        if( context.isMakeView )
        {
            instanceStack.push( 'var ' +(skinObject.id||context.gloableElementList.body.id)+ ':Skin=this;');
        }else{
            instanceStack.push( 'var ' +(skinObject.id||context.gloableElementList.body.id)+ ':Skin=new Skin(document.body);'); 
        }
     }
     //是一个 Skin or Component
     else if( skinObject.isComponent )
     {
        var params = "";
        var classname = skinObject.fullclassname || skinObject.classname;
        var classType = ":"+(skinObject.classType ? skinObject.classType :classname);

        if( skinObject.isSkinComponent )
        {
            params  = context.isMakeView ? '"'+Utils.uid()+'"' : "hostComponent.getComponentId('"+Utils.uid()+"')";
        }else{
            params = getComponentConstructParams(skinObject , context, skinObject.isSkin, bindProperties && bindProperties.attrs ).join(","); 
        }

        if( isSingle )
        {
            value=' new ' + classname + '(' + (params||"") + ')';

        }else
        {
            instanceStack.push( 'var ' + createUniqueRefId( skinObject ) + classType + '= new ' + classname + '(' + (params||"") + ');' );
            if( properties.length > 0 )
            {
                contentStack.push( makeComponentPropertiesMothod(skinObject, properties ).join("\n") );
            }
            value = createUniqueRefId( skinObject );
        }
     }
     //默认皮肤元素
     else{

        var params = getComponentConstructParams(skinObject , context, true, bindProperties && bindProperties.attrs );
        if( reference && reference.length > 0 )
        {
            params.splice(1,0,'['+reference.join(",")+']');
        }else{
            params.splice(1,0,'null');
        }
        params.unshift(uniqueKey);

        if( isSingle )
        {
            value='DOMRender.createElement(' +params.join(",")+')';
        }else
        {
            contentStack.push( 'var ' +createUniqueRefId( skinObject )+ ':IDisplay=DOMRender.createElement(' + params.join(",") + ');' );
            value=createUniqueRefId( skinObject );  
        }
     }
     ...
     if( parentObject )
     {
        childReference.push( value );

     }else if( reference.length > 0 )
     {
         contentStack.push( "RootContainer.push(" + reference.join(");\nRootContainer.push(")+");" )
     }
     ...
     if( value )
     {
         ...
         {
            contentStack.push("RootContainer.push("+value+");");
         }
     }
    ...
    if( isPublic )
    {
        classProperties[ skinObject.id ] = skinObject.classType ? skinObject.classType : (skinObject.fullclassname || skinObject.classname || "Skin");
        if( skinObject.ownerForeachCommand ){
            classProperties[ skinObject.id ] = "Array";
        }
        instanceStack.push( 'this.'+skinObject.id+'='+skinObject.id+';');
    }

     //添加绑定的内容
     if( bindProperties && bindProperties.content )
     {
         beforeStack.push( bindProperties.declaration );
         contentStack.push( bindProperties.content );
     }
}
...
* 生成一个皮肤对象的代码
*/
function makeRenderFactoryScript(skinObject, context, topStack, classProperties )
{
    var beforeStack = [];
    var contentStack = [];
    var children = [];
    (skinObject.children||[]).forEach(function(child,index){
         makeEverySkinElement(child, null, index+1, context, topStack, beforeStack, contentStack, classProperties ,children);
    });
    return "function(context:"+skinObject.fullclassname+", DOMRender:IRender, RootContainer:Array){\n" + beforeStack.concat(contentStack).join("\n") +"\n}";
}
