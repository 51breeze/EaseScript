...
function makeSkinView(skin, level)
{
    ...
    if( !endTag ) {
        for (var i in skin.children) {
            str = makeSkinView(skin.children[i], level + 1);
            content.push(str);
        }
    }

    if( skin.children[0] && typeof skin.children[0] !== "string")
    {
        content.push(dept);
    }
    if( !endTag ) {
...
function parseSkin(elem, context, module , parent, async )
{
    ...
    async = async===true ? true : attrdata.async === 'true' && context.syntax !=='javascript';
        ...
        //尝试使用组件
        {
            ...
            else
            {
                ...
                if (viewport_desc /*&& viewport_desc.id==='function'*/)
                {
                    ...
                    {
                        var cid = parent.id;
                        if( !parent.id || parent.isPrivate)
                        {
                            parent.attr.id = cid = uid();
                        }
                        if( clientScriptContents.requirements.indexOf('Container')<0)
                        {
                            clientScriptContents.requirements.push( 'es.core.Container' );
                        }
                        attrdata['viewport'] = '@new Container( Element("#'+cid+'") )';
    ...
    if( skinObject.isProperty===true )
    {
        ...
        if( type )
        {
            if( !skinObject.isBindable )
            {
                ...
                setPropertyMethod(name, param, skinObject, context, null, async );
            }
        }
...
function parseView(elem, context, module , parent, async )
{
    var name = elem.name();
    if( name==null )
    {
    ...
    if( nodes.length > 0 )
    {
        ...
        for (var i in nodes)
        {
            child= parseView( nodes[i], context, module , skinObject , async );
            //如果是属性稍后会当参数处理
            if( child && child.isProperty !== true  && child.nonChild !==true )
            {
                if( child.id && !skinObject.id )
...
function setPropertyMethod( name, value, skinObject, context , desc , async )
{
    ...
    if( desc )
    {
        ...
        if( async )
        {
            id = skinObject.module.id;
            ref = clientScriptContents.script[ id ] || (clientScriptContents.script[ id ]=[]);
        }
        if (isset)
        {
...
var moduleContents = [];
var styleContents = [];
var clientScriptContents = {requirements:[],script:{}};
...
function start(view , config, project, syntax, loadModuleDescription )
{
    ...
    if( context.isMakeView ){
        ...
        var skinObject = parseView(element, context);
        mergeViewElement( skinObject , context);
        var view = makeSkinView( skinObject );
        var view_path = PATH.resolve( utils.getBuildPath(context.config, 'build.view'), context.config.bootstrap + '.html' );
        view = view.replace(/^([\r\n]+)|([\r\n]+)$/g,"");
        utils.setContents( view_path , view )
        console.log( view );

    }else{
        parseSkin(element, context);
    }
...
function mergeViewElement( viewObject , context )
{
    ...
    var js_path = PATH.relative( view_path, PATH.resolve( js_path, context.config.bootstrap + '.js') ).replace(/\\/g,"/");
    var css_path = utils.getBuildPath(context.config, 'build.css');
    var css_path = PATH.relative( view_path, PATH.resolve( css_path, context.config.bootstrap + '.css') ).replace(/\\/g,"/");
    script.attr={"type":"text/javascript","src":js_path};
    link.attr={"rel":"stylesheet","href":css_path};
    head.children=[meta,link,script,title];
    ...
    if( !objBody )
    {
        ...
        for(;index<len;index++)
        {
            ...
            if( ["html","head","body"].indexOf( child.name ) < 0 )
            {
                body.children.push( child )
                var i = viewObject.children.indexOf( child )
                viewObject.children.splice(i,1);
            }
        }
        viewObject.children.push( body );
    }
}
...
function makeScript( classModule )
{
    ...
    for (var classname in components )
    {
        ...
        if( component.async )
        {
            if( clientScriptContents.requirements.indexOf( value ) < 0 )
            {
                clientScriptContents.requirements.push( value );
            }
            clientScriptContents.script[ component.id ].unshift( 'var ' + component.id + ':' + value + '= new ' + value + '(' + param + ')' );
            clientScriptContents.script[ component.id ].push( component.id+'.display()' );
    ...
    styleContents.push( {style:styleContent,ownerModule:classModule.fullclassname} );
...
function makeSkin(view , config, project, syntax, loadModuleDescription )
{
    ...
    styleContents=[];
    //脚本内容(分离到前端运行的脚本)
    clientScriptContents={requirements:[],script:{}};

    //开始解析皮肤
    start(view , config , project, syntax, loadModuleDescription );

    var classModule = null;

    //在客户端运行的脚本内容
    if( clientScriptContents.requirements.length > 0 )
    {
        classModule = createModulObject('Object','client.Index','client.Index');
        var script = ['package client{\n',
        ...
        for(var i in clientScriptContents.script )
        {
            script.push( clientScriptContents.script[i].join(';\n') + ';');
        }
        script.push( '\n});\n' );
        script.push( '\n}\n}\n}' );
        classModule.script = script.join("");
        //console.log(  classModule.script );
    ...
    return {
        ...
        clientScriptContents:classModule,
