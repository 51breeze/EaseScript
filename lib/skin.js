...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    for(var p in attrs)
    {
        ...
        if( hash !== true )
        {
            ...
            if( !skinObject.isCommand )
            {
                ...
                if( bind.result===true )
                {
                    oneWayBindingAttrs.push({name:attrName,value:attrValue,bind:bind});
                    attrValue = null;
    ...
    skinObject.ownerProperty = parent && parent.isProperty ? true : skinObject.isProperty;
           ...
           if( child && typeof child === "string" && !skinObject.isMetadata )
           {
                ...
                if( nodes.length === 1 )
                {
                    if( bind.result===true )
                    {
                        ...
                        {
                            (bindContext.oneWayBinding || (bindContext.oneWayBinding=[])).push({name:skinObject.name,value:child,bind:bind});
                            skinObject.hasBinddingValue=true;
                        }else{
                            (bindContext.oneWayBinding || (bindContext.oneWayBinding=[])).push({name:"innerHTML",value:child,bind:bind});
                        }
                        child = null;
                ...
                }else
                {
                    ...
                    if( bind.result===true )
                    {
                        ...
                        if( skinObject.isProperty )
                        {
                            (_bindContext.oneWayBinding || (_bindContext.oneWayBinding=[])).push( {name:skinObject.name,value:child.attr.content,bind:bind} );
                            skinObject.hasBinddingValue=true;

                        }else{
                            (_bindContext.oneWayBinding || (_bindContext.oneWayBinding=[])).push( {name:"content",value:child.attr.content,bind:bind} );
                        }
                        child.attrdata={};
                        child.attr={};
                    }
...
function getComponentPropertyDescriptor( propertyObject, value,component, context)
{
    ...
    //默认会把文本字符存放到 attrdata.innerHTML 属性中
    if( lastPropertyObject.isProperty && lastPropertyObject.attrdata.innerHTML && value instanceof Array && value.length=== 0 )
    {
        value = lastPropertyObject.attrdata.innerHTML;
    }
...
//生成动态渲染皮肤的脚本
function makeRenderScript(skin, context, parent, uniqueKey, topScope, forLevel, keyPrefix, container )
{
    var DOMRender = "DOMRender";
    var childIndex = skin.childIndex||0;
    if( typeof skin === "string" )
    {
        var param = [keyPrefix ? uniqueKey+keyPrefix : uniqueKey, '"text"','"'+skin+'"' ];
    ...
    for (;c<len;c++)
    {
        ...
        if( child.isProperty !== true )
        {
            ...
            var str = makeRenderScript(
                 ...
                 child.isCommand ? uniqueKey : uniqueKey+''+childrenCount,
                 topScope, 
                 forLevel,
                 keyPrefix, 
                 childrenReference
    ...
    if( isCommand )
    {
        ...
        if( bindCommand )
        {
            ...
            code.push( "DOMRender.updateChildren("+createUniqueRefId(parent||skin)+","+skin.childIndex+","+createUniqueArrayRef(parent||skin)+");");

            //生成一个绑定器 
            var bindFnContent = 'function('+bindCommand.join(':*')+":*"+","+createUniqueRefId(parent)+":IVirtualElement){\n"+code.join("\n")+'\n};';
            ...
            topScope.afterStack.push( 'Context.binding('+bindParam.join(",")+');' ); 
            return null;
        } 
        return code.join("\n");
    }
    //一个组件或者是元素
        ...
        {
            if( p==="innerHTML" || (skin.name==="text" && p==="content") ){
        ...
        //如果是一个根元素并且存在子级元素命令
        var needPushToArray = ( skin.parent && (skin.parent.isCommand || skin.parent.hasChildCommand) ) || ( !parent && skin===context && skin.hasChildCommand);
        //元素的唯一key
        var uukey = keyPrefix ? uniqueKey+'+'+keyPrefix : uniqueKey;

        //针对视图处理, body 是一个全局对象，所以只需要引用
        if( context.isMakeView && skin.name.toLowerCase()==="body" )
        {
            if( childrenReference.length > 0 )
            {
                container.splice.apply(container, [0,0].concat(childrenReference) );
            }
            if( context.isMakeView )
            {
                topScope.declaration.push( 'var ' +(skin.id||context.gloableElementList.body.id)+ ':Skin=this;');
            }else{
                topScope.declaration.push( 'var ' +(skin.id||context.gloableElementList.body.id)+ ':Skin=new Skin(document.body);'); 
            }
        }
         //创建一个组件
        else if( skin.isComponent )
        {
            ...
            //元素对象的动态属性
            if( skin.isSkin && bindProperties && bindProperties.attrs.length > 0 )
            {
                componentContent.push( createUniqueRefId(skin)+".element.properties({"+bindProperties.attrs.join(",")+"});");
            }

            componentContent.push("return "+createUniqueRefId(skin)+";");

            var code="function("+createUniqueRefId(skin)+":"+classType+",uniqueKey:*){\n"+componentContent.join("\n")+"\n}";
            var param=[uukey,'"'+classname+'"',code];

            //元素在其它地方有被引用到
            if( hasReference )
            {
                childContent.push( "var "+createUniqueRefId(skin)+":IDisplay="+DOMRender+".createComponent("+param.join(",")+");" );
                if( needPushToArray )
                {
        ...
        //创建一个元素
            ...
            //元素对象的动态属性
            if( bindProperties && bindProperties.attrs.length > 0 )
            {
                 indexAt=4;
                 param.splice(indexAt,1,"{"+bindProperties.attrs.join(",")+"}");
            }
            //去掉为null的参数
            ...
            if( hasReference )
            {
                childContent.push( "var "+createUniqueRefId(skin)+":IDisplay="+DOMRender+".createElement("+param.join(",")+");" );
                if( needPushToArray )
                {
                    childContent.push( createUniqueArrayRef(parent||skin)+".push("+createUniqueRefId(skin)+");");
        ...
        if( isPublic && skin !== context )
        {
             ...
             topScope.properties[ skinId ] = skin.classType || skin.fullclassname || skin.classname || "Skin";
             if( skin.ownerForeachCommand )
             {
                 topScope.properties[ skinId ] = "Array";
                 childContent.push( "Context."+skinId+".splice("+keyPrefix+",1,"+skinId+");" );

             }else
             {
                 childContent.push( "Context."+skinId+"="+skinId+";" );
             }
        }
    }
...
function makeElementOneWayBindingContent(skinObject,context)
{
    ...
    Utils.forEach(skinObject.oneWayBinding,function(item)
    {
        ...
        if( item.bind.binding.length < 1 ){
             if( item.name==="innerHTML" || (item.name==="content" && skinObject.name==="text") ){
    ...
    if( binding.length > 0 )
    {
        var bindTarget = createUniqueRefId( skinObject );
       ...
       if( skinObject.ownerForeachCommand )
       {
        ...
        bindContent="Context.binding("+bindingParam.concat( bindingRefs ).join(",")+");"; 
    }
...
* 生成每一个皮肤元素
*/
function makeEverySkinElement(skinObject, parentObject, uniqueKey, context, topStack, beforeStack, contentStack, classProperties, container )
{
      //将命令元素转换成对应的脚本
     if( skinObject.isCommand )
     {
    ...
    var value=null;
     ...
     { 
         children.forEach(function(child)
         {
            if( !child.isProperty )
            {
                makeEverySkinElement(child, skinObject, uniqueKey+''+hasChildren, context, topStack, contentStack, contentStack, classProperties, childrenContainer );
                if( !child.isCommand ){
                    hasChildren++;
                }else{
                   isSingle = false; 
     ...
     if( name==="body" )
     {
        ...
        {
            container.splice.apply(container, [0,0].concat(childrenContainer) );
        }
     ...
     //默认皮肤元素
        ...
        params.splice(1,0, childrenContainer.length > 0 ? '['+childrenContainer.join(",")+']' : 'null');
        ...
        {
            contentStack.unshift( 'var ' +createUniqueRefId( skinObject )+ ':IDisplay=DOMRender.createElement(' + params.join(",") + ');' );
            value=createUniqueRefId( skinObject );  
        }
     }
...
function makeRenderFactoryScript(skinObject, context, topStack, classProperties )
{
    ...
    var afterStack = [];
    var hasChildren = false;
    var top ={declaration:beforeStack,properties:classProperties,afterStack:afterStack};
    (skinObject.children||[]).forEach(function(child,index)
    {
        var str = makeRenderScript(child,context,skinObject, index+1 , top ,0, '', container );
        if( str )
        {
            hasChildren = true;
            contentStack.push( str );
        }
    });

    if( skinObject.hasChildCommand && hasChildren )
    {
        contentStack.unshift( "var "+createUniqueArrayRef(skinObject)+":Array=[];" );
    }

    var body=[];
    if( beforeStack.length > 0){
        body.push( beforeStack.join("\n") );
    }
    if( contentStack.length > 0){
        body.push( contentStack.join("\n") );
    } 
    if( afterStack.length > 0){
        body.push( afterStack.join("\n") );
    }

    if( skinObject.hasChildCommand && hasChildren )
    {
         body.push( 'return '+createUniqueArrayRef(skinObject)+';' );
     }else{
         body.push( 'return [\n'+container.join(",\n")+'\n];' );
     }

    return "function(Context:"+skinObject.fullclassname+", DOMRender:IRender){\n" + body.join("\n") +"\n}";
}
