...
{
    return context.syntax !== "javascript" && elem.isComponent;
}

function makeSkinView(skin, context, level)
{
    level = level || 1;
    if( typeof skin === "string")return skin;
    var content=[];
    ...
    if( !endTag )
    {
        for (var i in skin.children)
        {
            ...
            {
                content.push(makeSkinView(skin.children[i], context, level + 1));

            }else if( isSerComponent(context, skin.children[i]) )
            {
                content.push( makeViewComponentOfPhp(skin.children[i], context, level + 1) );
            }
        }
    }
    //缩进，如果有子级并且不是文本
...
function makeViewComponentOfPhp(skin, context, level)
{
    var depth = new Array(level).join("\t")
    ...
    //var code = context.config.$makeFragmentScript(context.syntax, str.join("\n"), context.config, context.project);
    str.push(depth+"?>\n");
    return str.join("\n");
...
function checkInstanceOf(context, classModule, classType, isInterfaceType )
{
    ...
    var descriptions = function (full) {
        return getDescriptionAndGlobals(context.syntax, full);
    }
...
function start(view , config, project, syntax, loadModuleDescription )
{
    ...
    //如果是已经加载过
    if( config.loadedSkins[syntax][ filename ] === true )
    {
        return;
    }
...
function createApplicationView(viewObject , context)
{
    ...
    viewObject.attr={};
    var view ="<!DOCTYPE html>\r\n"+makeSkinView( viewObject, context );
    var suffix = ".html";
    if( context.syntax ==="php"){
        suffix = ".php";
    }
    var view_path = PATH.resolve( utils.getBuildPath(context.config, 'build.view'), context.classname + suffix );
...
function setViewHeadAndBodyElement(viewObject , context)
{
    ...
    var filename = context.fullclassname.toLowerCase();
...
function makeScript( classModule , skinObject )
{
    ...
    if( classModule.bindable )
    {
        ...
        {
            if( classModule.isMakeView===true ){
                body.push( bindableContent.join("") );
            }else{
                body.push(
   ...
   if( classModule.classname==="person") {
...
function makeSkin(view , config, project, syntax, loadModuleDescription )
{
    ...
    //保存之前的
    var oldContents = moduleContents;
    //初始化皮肤对象
    moduleContents=[];
    //开始解析皮肤
    start(view , config , project, syntax, loadModuleDescription );
    var results = moduleContents;
    moduleContents = oldContents;
    return results;
}
