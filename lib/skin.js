...
//private
function __toMakeChildrenFactory(skin, notContainer , parent, separation, makeSyntax )
{
    makeSyntax = makeSyntax || skin.context.syntax;
    var tag = skin.name;
    var children = skin.children || [];
...
function parseBinable(attrValue, attrName, skinObject, context )
{
    if(attrValue && attrValue.slice(0,2) === '{{' && attrValue.slice(-2) === '}}' )
    {
        ...
        var binding = ownContext.bindable[ refobj ] || (ownContext.bindable[ refobj ]={name:[],bind:[]});
...
function parseTextBinding(context,parent,text, childIndex )
{
    ...
    if( has  )
    {
        ...
        var ownContext =  parent.ownDataRenderContext || context;
        if( parent.isTitleElement ){
            ownContext.initProperties.push("document.title="+content+" as String");
        }else
        {
            setViewportComponent( parent );
            ownContext.initProperties.push(getSkinReferenceId(parent, context) + ".element.addChildAt( Element.createElement( " + content + " ), " + childIndex + ")");
        }
...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    //DataRender 将所有的子级组件存放到专有的作用域
    //如果是一个新的DataRender类则初始化
    if( skinObject.fullclassname==="es.core.DataRender" )
    {
        skinObject.ownDataRenderContext =skinObject
        skinObject.components =[];
        skinObject.initProperties = [];
        skinObject.initSelfProperty = [];
        skinObject.stateProperties = [];
        skinObject.defineStates = {};
        skinObject.bindable=[];
    }
    //否则存在就引用父级的作用域
    else if( parent && parent.ownDataRenderContext )
    {
        skinObject.ownDataRenderContext = parent.ownDataRenderContext;
    }

    var ownerProperty = !!skinObject.isProperty;
    if( parent && !ownerProperty ) {
    ...
    //默认使用皮肤来创建显示元素
    if( !ownerProperty && !skinObject.isComponent && !skinObject.isGloableElement && !isHeadElementChildOf(skinObject,context) )
    {
        skinObject.isComponent=true;
        skinObject.isSkin = true;
        skinObject.isDefaultSkin = true;
    }
    ...
    if( gloableElementReference )
    {
        ...
        if( gloableElement )
        {
            ...
            var ownContext =  skinObject.ownDataRenderContext || skinObject.context;
            if (skinObject.id && skinObject.isPrivate !== true)
            {
                eid = skinObject.id;
                ownContext.declarationReference.push('var ' + (skinObject.id) + ":" + skinObject.classType + " = " + gloableElement.id);
            }

            skinObject.attr = Utils.merge(gloableElement.attr||(gloableElement.attr={}), skinObject.attr )
            var attrStr = skinAttrToString(skinObject);
            if (attrStr.length > 0)
            {
                ownContext.initProperties.push(eid + ".element.setProperties({" + attrStr.join(",") + "})");
            }
            var child = getSkinChildToString(skinObject);
            if (child) {
                ownContext.initProperties.push(eid + ".element.addChildAt( Element.createElement('" + child + "')," + skinObject.childIndex + ")");
            }
        }
    }
    ...
    if( nodes.length > 0 )
    {
        ...
        for (var i in nodes)
        {
            ...
            if( child )
            {
                ...
                //创建子级时的位置索引
                if( !(child.isProperty === true || skinObject.ownerProperty) )
                {
                    childIndex++;
                }
            }
    ...
    if( skinObject.name==="DataRender" )
    {

        console.log( makeChildrenFactory( skinObject, context ) )
        process.exit();
    }
    ...
    if( skinObject.isComponent ===true && skinObject.isProperty !==true )
    {
        ...
        {
            if( skinObject.ownDataRenderContext && skinObject.ownDataRenderContext !== skinObject )
            {
                skinObject.ownDataRenderContext.components.push( skinObject );
            }else{
    ...
    }else if( skinObject.isComponent==true && skinObject !== context  )
    {
        ...
        }else
        {
            ...
            /*if( context.fullclassname==="view.Index")
            {
                console.log( param, skinObject.name , context.fullclassname )
            }*/
        }
...
function makeChildrenFactory(skinObject, context)
{
    ...
    for (var classname in components )
    {
        ...
        if( component.isDisplayElement && !component.elementAdded )
        {
            ...
            {
                var parentContainer = getSkinReferenceId(component.parent, context ) || "this";
    ...
    {
        body.push( newInstances.join(";\n")+";\n" )
    }

    if( initComponentProperties.length >0 )
    {
        body.push( initComponentProperties.join(";\n")+";\n" )
    }
    ...
    var skinEvent = "es.events.SkinEvent";
    if( skinObject.bindable && imports.indexOf(skinEvent)<0  )
    {
        imports.push( skinEvent );
    }

    //绑定状态属性
    var stateContent = getStateProperties( skinObject );
    if( stateContent )
    {
        body.push(
            'this.addEventListener( SkinEvent.INTERNAL_UPDATE_STATE ,function(event:SkinEvent){\r\n' +
    ...
    //元素属性绑定
    var bindableContent = getBinddingContent( skinObject );
    if( bindableContent )
    {
        body.push('System.getGlobalEvent().addEventListener( Event.INITIALIZE_COMPLETED ,function(){\n' + bindableContent + '},false,-500,this);\n');
    }
