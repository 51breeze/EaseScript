...
function makeServerViewComponent(skin, level, context, properties )
{
    ...
    instance.push( "var "+skin.id+"= new "+skin.fullclassname+"("+(skin.param||"")+");\n" );
    str.push( depth1+skin.initProperties.join(";\n"+depth1 )+";" );
    if( skin.isDisplayElement ) {
...
function createServerView(viewObject , context, properties )
{
    var attr = viewObject.attr;
    setViewHeadAndBodyElement(viewObject , context);
    viewObject.attr={};
    properties = properties || {};
    var view ="<!DOCTYPE html>"+makeServerView( viewObject, context , 1, properties );
    var code = [ properties.newInstance.join("") ];
    code.push( properties.member.join("") );
    code.push("this.addEventListener('INTERNAL_BEFORE_CHILDREN', function(event){");
    code.push( Utils.templateMake( view ) );
    code.push( "var __this__ = ("+context.fullclassname+")this;" );
...
function makeScript( classModule , skinObject )
{
    ...
    if (classModule.isMakeView === true)
    {
        if( classModule.syntax === "javascript")
        {
            ...
            body.push(  createServerView(skinObject, skinObject.context ) );
