           ...
           if( child && typeof child === "string" && !skinObject.isMetadata )
           {
                ...
                if( nodes.length === 1 )
                {
                    if( bind.result===true )
                    {
                        ...
                        {
                            (bindContext.oneWayBinding || (bindContext.oneWayBinding=[])).push({name:"innerHTML",value:child,bind:bind,target:skinObject,isAttr:false});
                        }
                        skinObject.hasBinddingValue=true;
                        child = null;
...
function makeClientScript( classModule , skinObject , isMakeView )
{
    ...
    if( renderFactory.beforeStack.length > 0 )
    {
         body.push( renderFactory.beforeStack.join("\n") );
    }
    ...
    var iVirtualElementInterface = "es.interfaces.IVirtualElement";
     imports.push( "es.interfaces.IDisplay" );
...
function makeRenderScript(skin, context, parent, uniqueKey, topScope, forLevel, keyPrefix, container )
{
    ...
    //一个组件或者是元素
        ...
        //并且是中视图的子级中
        if( context.isMakeView && skin.name.toLowerCase()==="body" && skin.parent === context )
        {
        ...
        else if( skin.isComponent )
        {
            ...
            var param=[uukey,uniqueKey,'"'+classname+'"',code];
...
* 生成指定皮肤对象的单向绑定属性
*/
function makeElementOneWayBindingContent(skinObject,context, refTarget )
{
    ...
    if( binding.length > 0 )
    {
       ...
       if( skinObject.ownerForeachCommand )
       {
        ...
        if( isDisplayElement )
        {
            bindingParam.push( targetRef.join(".")+".element" );
        }else{
...
function makeRenderFactoryScript(skinObject, context, topStack, classProperties )
{
    ...
    var properties = getComponentDefinedProperties( skinObject, context, _topScope );
    var innerHTML = bindProperties ? bindProperties.innerHTML : null;
    //生成元素的属性对象
    for( var p in skinObject.attributes )
    {
        if( p==="innerHTML" || (skinObject.name==="text" && p==="content") ){
            innerHTML = '"'+skinObject.attributes[p]+'"';
        }else{
           attributes.push('"'+p+'":'+'"'+skinObject.attributes[p]+'"');
        }
    }
    ...
    if( properties.length > 0 )
    {
        var componentProps = makeComponentPropertiesMothod(skinObject, properties);
        if( componentProps && componentProps.length > 0 )
        {
            classContent.push( componentProps.join("\n") );
        }
    }
    ...
    if( bindProperties && bindProperties.attrs.length > 0 )
    {
        classContent.push( "this.element.properties({"+bindProperties.attrs.join(",")+"});");
    }
    ...
    (skinObject.children||[]).forEach(function(child,index)
    {
        if( child.isProperty !==true )
        {
            ...
            if( str || container.length > 0 )
            {
                hasChildren = true;
                str && contentStack.push( str );
            }
        }
    });
    ...
    {
         body.push( 'return "'+innerHTML+'";' );
    }

    return {
        beforeStack:_topScope.declaration.concat( _topScope.contentStack , _topScope.afterStack, classContent ),
        attributes:attributes,
        content:"function(Context:"+context.fullclassname+", DOMRender:IRender){\n" + body.join("\n") +"\n}"
    }
}
