...
function parseBinable(attrValue, attrName, skinObject, context )
{
    if(attrValue && attrValue.slice(0,2) === '{{' && attrValue.slice(-2) === '}}' )
    {
        ...
        if( !context.isMakeView && !skinObject.isComponent && !skinObject.isProperty )
        {
...
function parseTextBinding(context,parent,text, childIndex )
{
    ...
    if( has  )
    {
        ...
        if( context.isMakeView )
        {
            ...
            context.initProperties.push( parent.id+".addChildAt( Element.createElement( " + content + " ), " + childIndex + " )");
...
function parseSkin(elem, context, module , parent, async )
{
    ...
    }else if( skinObject.isComponent==true && skinObject !== context  )
    {
        ...
        }else
        {
            ...
            {
                skinObject.notNewInstance = true;
                skinObject.classType = "Element";
                skinObject.elementAdded = true;
                skinObject.isGloableElement = true;
                param = 'new Element('+gloableElementReference+')';
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    Utils.forEach(components, function ( component )
    {
       ...
       if( component.async )
       {
           ...
           getComponentRinningContents(classModule, false, component, imports, loaded,
               clientScript.body, clientScript.property,
               clientScript.newInstances, clientScript.body);

       }else
       {
           getComponentRinningContents(classModule, isMakeView , component, imports, loaded, body, property, newInstances, invokeContents );
       }
    ...
    content.push( '\n}\n}' );
    if( classModule.fullclassname==="DataGridSkin") {
