...
function makeSkinParam(skin, flag)
{
    ...
    if( flag !== false )
    {
        ...
        if (child)
        {
            (skin.context.initProperties).push( (skin.id || "this") + ".element.addChildAt(Element.createElement('" + child + "'),"+skin.childIndex+")" );
        }
    }
...
function makeViewComponentOfPHP(skin, context, level, callback )
{
    ...
    if( context.imports )
    {
        str = "import "+context.imports.join(";\nimport ")+";\n"+str;
    }
    str = callback.call(skin, str )
    if( str )
    {
...
/**
 * 设置为一个视图容器
 * 只有一个 javascript 视图才需要做标志
 * @param skinObject
 * @param context
 */
function setViewportElementFlag(skinObject , context )
{
    if( context.isMakeView )
    {
        ...
        if( context.syntax === "javascript" && !skinObject.isViewportElementFlag && !(skinObject.isSkin || skinObject.isComponent || skinObject.isProperty) )
        {
            if( !skinObject.id )
            {
                skinObject.id = "__var" + uid() + "__";
            ...
            skinObject.isViewportElementFlag = true;
            context.initProperties.push("var " + skinObject.id + ":Element=new Element('#" + skinObject.attributeId + "')");
        }
    }
}
...
function parseSkin(elem, context, module , parent, async, childIndexAt )
{
    ...
    else if( elem.namespace() )
    {
        ...
        //尝试使用组件
        {
            ...
            if (getModuleType(context, skinObject.description, [context.baseSkinclass]))
            {
                ...
                {
                    setViewportElementFlag( parent, context );
                    skinObject.viewport = parent.id;
                ...
                if (isDisplayElement)
                {
                    ...
                    if( skinObject.isSkinComponent && skinObject.description.defineMetaTypeList && skinObject.description.defineMetaTypeList.Skin )
                    {
                        ...
                        if( typeof attrdata.skinClass === "undefined" )
                        {
                            ...
                            {
                                attrdata.skinClass =  skinMetatype.param.source;
                            }
                        }
                    }
    ...
    }else if( skinObject.isComponent==true && skinObject !== context  )
    {
        ...
        }else if( !skinObject.isGloableElement )
        {
...
function setViewHeadAndBodyElement(viewObject , context)
{
    ...
    objHead.children = objHead.children.concat( loadResourceFiles(context, view_path, css_build_path, js_build_path) );
...
function getServerComponentProperties(classModule, isMakeView, component, imports, loaded, body, property, newInstances, invokeContents )
{
    var value=component.fullclassname || component.classname;
    var param = component.param || '';
...
function makeServerScript( classModule , skinObject , isMakeView )
{
    ...
    Utils.forEach(components, function ( component )
    {
       ...
       {
           getServerComponentProperties(classModule, isMakeView , component, imports, loaded, body, property, newInstances, invokeContents );
       }
    ...
    if ( isMakeView === true )
    {
       // setViewHeadAndBodyElement(skinObject , skinObject.context)
        //console.log( createServerView(skinObject, skinObject.context ) )
        //process.exit();
        //invokeContents.push(  );
    }
    ...
    var constructor = [];
    var constructParam = "";
    var constructSuper = "super();\n";

    //如果是一个视图
    if ( isMakeView=== true)
    {
         constructSuper = "super(context);\n";
         body.unshift( constructSuper );
         if( hostComponent )body.unshift('this._hostComponent=context as '+hostComponent+';\n');
         constructParam = 'context:'+applicationClass;
         constructor.push("[Syntax('"+classModule.syntax+"')]");
         constructor.push('public function ' + classModule.classname + '('+constructParam+')' + '{\n' + body.join("") + '}');

         if( classModule.syntax !=="javascript" )
         {
            var clientBody = [ constructSuper, clientScript.newInstances.join("") , clientScript.body.join("") ];
            if( hostComponent )clientBody.unshift('this._hostComponent=context as '+hostComponent+';\n');
            constructor.push("[Syntax('javascript')]");
            constructor.push('public function ' + classModule.classname + '('+constructParam+')' + '{\n'+clientBody.join("")+'}');
         }

    } else
    {
        constructSuper = 'super(' + skinSuperParam+ ');\n';
        body.unshift( constructSuper );
        if (hostComponent){
            constructParam = 'hostComponent:' + hostComponent;
            if( hostComponent )body.unshift('this._hostComponent=hostComponent as '+hostComponent+';\n');
            constructor.push('public function ' + classModule.classname + '(' + constructParam + ')' + '{\n' + body.join("") + '}');
        } else {
            constructor.push('public function ' + classModule.classname + '()' + '{\n' + body.join("") + '}');
        }
    }
    ...
    try{
        ...
        if( context.isMakeView )
        {
            context.gloableElementList={
                "head":{id:getUniqueVar(context), isPrivate:true,gloableNodeElement:'document.head',classType:'Skin',hasUsed:false},
                //"title":{id:getUniqueVar(context), isPrivate:true,gloableNodeElement:'document.title',classType:'Skin',hasUsed:false},
                "body":{id:getUniqueVar(context), isPrivate:true,gloableNodeElement:'this',classType:'Skin',hasUsed:false,notNewInstance:true},
                "html":{id:getUniqueVar(context), isPrivate:true,gloableNodeElement:'document.documentElement',classType:'Skin',hasUsed:false},
            }
        }
