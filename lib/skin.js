    ...
    else if( elem.namespace() )
    {
        ...
        if( !skinObject.isMetadata )
        {
            ...
            else
            {
                ...
                if( !desc )
                {
                    throw new ReferenceError("Property is not defined for '"+name+"'");
                } 
...
function makeClientScript( classModule , skinObject , isMakeView )
{
    ...
    makeChildrenFactory(skinObject);
    process.exit();
   ...
   if( classModule.fullclassname==="es.skins.PaginationSkin") {
       // console.log(content.join(""));
       // process.exit();
    }
    return content.join("");
...
function makeSkin(view , config, syntax, ownerModule)
{
    ...
    try{
        ...
        if( syntax ==="javascript" )
        {
            ...
            makeSkinFactoryScript( skinObject,context,isMakeView );
...
{
    var target={items:[],type:null,value:null};
    ...
    var module = component.description;
    var typename =  null;
    do{
        ...
        if( !desc )
        {
            throw new ReferenceError("Property is not defined. for '"+(module.fullclassname||module.classname)+"::"+object.name+"'");
        }
        target.items.push({name:object.name,desc:desc});
        typename = getImportClassName(desc.type, module.import ) || desc.type;
        module = makeSkin.getLoaclAndGlobalModuleDescription( typename );
        if( !module )
        {
            throw new ReferenceError("Class '"+typename+"' is not exists."); 
        }
    ...
    if( value )
    {
        target.value= getValueByType( value, typename , context );
    }
    return target;
}
    ...
    if( component.isComponent )
    {
        ...
        Utils.forEach(component.attrdata,function(value,name)
        {
            if( !(isSkin && name ==="name") )
            {
                ...
                if( desc )
                {
                    var typename = getImportClassName(desc.type, component.description.import ) || desc.type;
                    properties.push({
                        items:[{name:name,desc:desc}],
                        type:typename,
                        value:getValueByType( value, typename , context )
                    });
...
function getComponentConstructParams(component , context, isSkin , attrString )
{
    ...
    if( component.fullclassname === "es.core.Render" )
    {
       params.push("this");
       params.push( component.parent.id );
       params.push( component.childIndex );
       params.push( makeRenderFactory(component , context) );
       return params;
    }
...
{
    var pid = parent ? parent.id : "this";
    var key = uid();
    var childIndex = skin.childIndex||"0";
    topScope = topScope || {};
    if( typeof skin === "string" )
    {
        return pid+".createVirtualElement("+childIndex+","+(keyPrefix||0)+","+key+",'text','"+skin+"');";
    }
    ...
    if( isCommand )
    {
        ...
        if( attributes.length > 0)
        {
            ...
            topScope.attributes.push("var "+attrRefName+"={"+attributes.join(",")+"};");
        }
        ...
        if( skin.isComponent )
        {
            ...
            componentContent.push('var ' + skin.id + classType + '= new ' + classname + '(' + (param||"") + ');');
            var code="function(){\n"+componentContent.join("\n")+"\n}";
            var param=[childIndex,(keyPrefix||0),key,code];
            childContent.push( "var "+skin.id+"="+pid+".createVirtualComponent("+param.join(",")+");" );

        }else{

            var param=[childIndex,(keyPrefix||0),key,'"'+tag+'"',"null","null"];
            var indexAt = 3;
            if( attributes.length > 0 )
            {
                indexAt = 4;
            ...
            //绑定一组属性值 
            if( skin.oneWayBinding && skin.oneWayBinding.length > 0)
            {
                var bind=[];
                Utils.forEach(skin.oneWayBinding,function(item){
                    //直接对属性的引用
                    if( item.bind.binding.length < 1 ){
                         bind.push( '"'+item.name+'":'+item.value );
                    }else{
                        //绑定需要变动的属性对象
                ...
                indexAt=5;
                param.splice(indexAt,1,"{"+bind.join(",")+"}");
                bindingFlag = binding.length > 0;
            }
            param.splice(indexAt+1,2);
            if( hasChildren || bindingFlag ){
                childContent.push( "var "+skin.id+" = "+pid+".createVirtualElement("+param.join(",")+");"  );
            }else{
               childContent.push( pid+".createVirtualElement("+param.join(",")+");"  ); 
            }
            if( bindingFlag )
            {
                  var bindTarget = getUniqueVar(context);
                  var funName = getUniqueVar(context);
                  var stateName = getUniqueVar(context);
                  var refsParam = [stateName+":Object",bindingProperties.join(":*,")+":*",bindTarget+":Element"];
                  ...
                  topScope.attributes.push("var "+propsName+":Array="+'["'+bindingProperties.join('","')+'"];');
                  topScope.attributes.push( "var "+funName+":Function="+ bindingContent.join("") );
                  var bindingParam = [propsName,funName,skin.id+".element"].concat(  bindingRefs );
                  childContent.push("this.binding("+bindingParam.join(",")+");"  ); 
            }
        }
    }
    ...
    if( isCommand )
    {
        ...
        var code =  keyPrefixContent+make_skin_factory[ tagName ](attr, childContent.join("") , children.length > 0 );
        if( bindCommand )
        {
           return [
            'this.binding(["'+bindCommand.join('","')+'"],function('+bindCommand.join(':*')+":*"+"){\n",
               code,
            '});'
           ].join("");
        } 
...
* 生成一个渲染皮肤的工厂函数
*/
function makeRenderFactory(skinObject, context)
{
   var content = skinObject.children.map( function(chid){
         return makeRenderScript(chid, context);
    });
...
* 生成每一个皮肤元素
*/
function makeEverySkinElement(skinObject, context, instances, methods)
{
     (skinObject.children||[]).forEach(function(child,index)
     {
         ...
         if( child.isCommand )
         {
            var topScope = {declaration:[],assign:{},binding:[],attributes:[]};
            var content = makeRenderScript(child, context, skinObject, topScope );
            instances.push( topScope.attributes.join("\n")+"\n"+content );

         }else
         {
            //获取在一个组件上已经定义的属性
            var properties = getComponentDefinedProperties( child, context );
            var bindAttrs=[];
            var bindContent = [];
           
            //绑定一组属性值 
            if( child.oneWayBinding && child.oneWayBinding.length > 0)
            {
                var binding=[];
                var bindingProperties=[];
                var bindingRefs=[];
                var bind = [];
                Utils.forEach(child.oneWayBinding,function(item)
                {
                    //直接对属性的引用
                    if( item.bind.binding.length < 1 ){
                ...
                if( binding.length > 0 )
                {
                    ...
                    var funName = getUniqueVar(context);
                    var stateName = getUniqueVar(context);
                    var refsParam = [stateName+":Object",bindingProperties.join(":*,")+":*"];

                    if( !child.id )
                    {
                        child.id = getUniqueVar();
                        child.isPrivate = true;
                    }

                    if( child.isComponent )
                    {
                        refsParam.push( bindTarget+":"+child.fullclassname );
                    }else{
                    ...
                    var propsName = getUniqueVar();
                    bindContent.push("var "+propsName+":Array="+'["'+bindingProperties.join('","')+'"];');
                    bindContent.push( "var "+funName+":Function="+ bindingContent.join("") );
                    var bindingParam = [propsName,funName];
                    if( child.isComponent ){
                     bindingParam.push( child.id );
                    }else{
                     bindingParam.push( child.id+".element" );
                    }
                    bindContent.push("this.binding("+bindingParam.concat(  bindingRefs ).join(",")+");"); 
               }
             ...
             if( name==="body" )
             {
                if( context.isMakeView )
                {
                    instances.push( 'var ' +(child.id||context.gloableElementList.body.id)+ ':Skin=this;');
                }else{
                    instances.push( 'var ' +(child.id||context.gloableElementList.body.id)+ ':Skin=new Skin(document.body);'); 
                }
             ...
             {
                var params = "";
                var classname = child.fullclassname || child.classname;
                var classType = ":"+(child.classType ? child.classType :classname);
                if( child.isSkinComponent )
                {
                    params  = context.isMakeView ? '"'+Utils.uid()+'"' : "hostComponent.getComponentId('"+Utils.uid()+"')";
                }else{
                    params = getComponentConstructParams(child , context).join(","); 
                }
                instances.push( 'var ' + child.id + classType + '= new ' + classname + '(' + (params||"") + ');' );
                methods.push( makeComponentPropertiesMothod(child, properties ).join("\n") );
                methods.push(  (skinObject.id||"this") + '.addChildAt('+child.id+','+index+');');
             ...
             //默认皮肤元素
                ...
                var params = getComponentConstructParams(child , context, true, bindAttrs.join(",") );
                instances.push( 'var ' + child.id + ':Skin= new Skin(' + params.join(",") + ');' );
                methods.push(  (skinObject.id||"this") + '.addChildAt('+child.id+','+index+');');  
             }
             ...
             {
                 methods.push( bindContent.join("\n") );
             }

         }

         if( (child.isSkin || !child.isComponent) && !child.isCommand )
         {
             makeEverySkinElement(child, context, instances, methods);
         }
...
* 生成一个皮肤对象的代码
*/
function makeSkinFactoryScript(skinObject, context)
{
    var instances = [];
    var methods = [];
    makeEverySkinElement(skinObject, context, instances, methods);
    console.log( instances.concat(methods).join("\n") );
    return instances.concat(methods).join("\n");
}
