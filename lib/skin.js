...
function parse(elem, context, module , parent, async )
{
    var name = elem.name();
    if( name==null )
    {
    ...
    if( nodes.length > 0 )
    {
        ...
        for (var i in nodes)
        {
            child= parse( nodes[i], context, module , skinObject , async );
            //如果是属性稍后会当参数处理
...
function makeScript( classModule )
{
    ...
    var allowStateProps=["class","style","width","height","left","top","right","bottom","opacity","position","scrollLeft","scrollTop"];
    for ( var s in  sProp )
    {
         var props = s.split(".")
         if( props.length === 2 )
         {
             ...
             {
                 var _id = sProp[s].target.id;
                 var refcontent = stateProperties[statename] || (stateProperties[statename] = []);
                 if( propname==="style"){
                     refcontent.push(_id + ".style(" + "'" + sProp[s].value.replace(",","','") + "')");
    ...
    //构造函数
    content.push( 'function '+classModule.classname+'(skinObject=null)'+'{'+body.join("")+'}' );
    //类中成员函数
