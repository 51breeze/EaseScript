...
function parseValueString(value, refer, flag)
{
    ...
    var okValue = is ? value : flag !== true && /^([\-\+]?[\d\.]+|true|false)$/i.test( value ) ? value : '"'+escapeValueString(value)+'"';
    if( /^([\-\+]?[\d\.]+)$/i.test( okValue ) )
    {
...
function getClassPropertyDesc(name, currentModule, context , isset, isget )
{
...
function makeScript( classModule , skinObject , isMakeView )
{
            ...
            if(System.env.HOT_UPDATA){
                System.env.HOT_UPDATA( this, function(obj:${classModule.classname}, newClass:Class ){
                    if( obj instanceof es.core.View ){
                        (obj.hostComponent as es.core.Application).render( new newClass(obj.hostComponent) as es.core.View );
                        return true;
    ...
    if( classModule.fullclassname==="view.TestView")
    {
...
function makeRenderScript(skin, context, parent, topScope, forLevel, keyPrefix, container)
{
    ...
    //一个组件或者是元素
        ...
        if( skin === context )
        {
            ...
            {
                if( isClientRunning ){
                    childContent.push( DOMRender+".bindEvent("+[uukey,uniqueKey,"this","{"+bindEvents.join(",")+"}"].join(",")+");");
                }else{
        ...
        else if( skin.isComponent )
        {
            ...
            if( skin.isDisplayElement )
            {
                ...
                if( !isPublic )
                {
                    ...
                    {
                        if( isClientRunning ){
                           param.splice(7,1, "{"+bindEvents.join(",")+"}");
                        }else{
        ...
        //创建一个元素
            ...
            if( hasReference )
            {
                ...
                if( isPublic )
                {
                    ...
                    if( bindEvents.length > 0 || propertyTwoWayBinding.content.length > 0 )
                    {
                ...
                pushDeclareExpressionToContent( 
                    ...
                    bindEvents.length > 0 || propertyTwoWayBinding.content.length > 0
                );
                ...
                if( isPublic && !skin.ownerForeachCommand )
                {
                    ...
                    if( bindEvents.length > 0 )
                    {
                        childContent.push( DOMRender+".bindEvent("+[uukey,uniqueKey,refNodeTarget,"{"+bindEvents.join(",")+"}"].join(",")+");");
                    }
