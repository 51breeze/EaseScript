...
var syntaxBuilder={
    ...
    "buildClassStructure":function( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
    {
        ...
        {
           str += 'var constructor=null;\n';
        }

        if( classModule.id ==='class' && !classModule.isStatic  )
        {
            str+= 'var '+classname+'=require.define("'+classModule.fullclassname+'",module,function(){constructor.apply(this,arguments);},'+typeMap[classModule.id]+'),\n';
        }
        else if( classModule.id==="namespace" )
        {
            str+= 'var '+classname+'=require.define("'+classModule.fullclassname+'",module,function(prefix,uri){Namespace.call(this,prefix,uri);},'+typeMap[classModule.id]+'),\n'; 
        }
        else 
        {
            str+= 'var '+classname+'=require.define("'+classModule.fullclassname+'",module,function(){throw new TypeError("\\"'+classModule.fullclassname+'\\" is not constructor.");},'+typeMap[classModule.id]+'),\n';
        }
        
        Utils.forEach(mapHash,function(name,key){
            str+=name+"=require(\""+key+"\"),\n";
        });
        str += "_private=module.privateSymbol;\n";

        //构造函数
        if( classModule.id ==='class' && !classModule.isStatic  )
        {
            str += "constructor = " +classModule.constructor.value+";\n";
        }
