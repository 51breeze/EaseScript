...
function makeExportModuleString( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
{
    ...
    var excludeFile = Utils.EXCLUDE_GLOBAL | Utils.EXCLUDE_ERROR | Utils.EXCLUDE_CLIENT;
    if( config.runtime_error_debug  )
    {
        excludeFile ^= Utils.EXCLUDE_ERROR;
    }
    ...
    for(var n in importMap )
    {
        if( classModule.fullclassname !== importMap[n].fullclassname && !importMap[n].isInternal )
        {
            ...
            {
                idClass = Utils.getRequireIdentifier( config, importMap[n], 'javascript');
            ...
            }else if( importMap[n].type )
            {
                ...
                if( importMap[n].notLoadFile || Utils.excludeLoadSystemFile(importMap[n].type , excludeFile) )
                {
                    continue;
                }
    ...
    {
        if( require === true && !Utils.excludeLoadSystemFile( type, excludeFile ) )
        {
            ...
            idClass = Utils.getRequireIdentifier( config, config.globals[ _type ] || {type:_type} , 'javascript');
    ...
    if( !classModule.dependentModules )
    {
        classModule.dependentModules = {};
    }

    classModule.dependentModules["javascript"] = mapHash;
    ...
    //var identifier = Utils.getRequireIdentifier(config, classModule, '.es');
...
var syntaxBuilder={
    ...
    "definePropertiesClassOf":function (config, classProperties, classModule )
    {
        ...
        var iterator =  classModule.implements.filter( (name)=>{
            var classname = classModule.import[name] || name;
            var module = syntaxBuilder.getLoaclAndGlobalModuleDescription(classname);
            if( module && module.fullclassname==="es.interfaces.IListIterator")
            {
                return true;
            }
            return false
        ...
        var iteratorName = classModule.fullclassname.replace(/\.([a-zA-Z])/,function(a,b){
            return b.toUpperCase();
        });
        iterator = iterator.length > 0 ? `\tif(Symbol.iterator){
        Object.defineProperty(this,Symbol.iterator.valueOf(),{"value":function get${iteratorName}Iterator(){
            var self=this;
            this.rewind();
            return {
                "next":function next()
                {
                    if( self.valid() )
                    {
                        var obj = {value:self.current(),key:self.key(),done:false};
                        self.next();
                        return obj;
                    }else{
                        return {done:true};
    ...
    "buildPropertiesAndMethod":function ( describe, config , classModule , makeSyntax)
    {
        ...
        for( var p in describe )
        {
            ...
            if( !compile.isConformRunPlatform(desc, makeSyntax) )
            {
                continue;
            }
    ...
    "buildExpressionSetOf": function(globalsConfig, classmodule, thisvrg, desc, props ,info, value, operator,ns,useNs )
    {
        ...
        if( !checkRunning )
        {
            ...
            //对象属性
            if( ( !owner || owner.nonglobal !== true ) && !(desc.descriptor && (desc.descriptor.isAccessor || desc.descriptor.isFunAccessor) ) )
            {
                map = map.concat( props[0] );
                return Utils.joinProperty( map )+operator+_value;
            }
    ...
    "buildExpressionNewOf":function(globalsConfig, classmodule, express, desc, info , newParam, instanceType )
    {
        if( express ==="Function")
        {
            ...
            args = args ? args.split(",").map( value=> `'${value.replace(/[\r\n]+/g,'')}'` ).join(",") : '';
            funBody = `'${funBody.replace(/[\r\n]+/g,'')}'`;
            newParam = [ funBody ];
            if( args ){
                newParam.unshift(args);
            }
            return `new Function(${newParam.join(",")})`;
        }
    ...
    "getForIn":function( value, name, internalName , classmodule, isIterator , isArrayOrObject )
    {
        ...
        return internalName+' = ListIterator('+value+');'+internalName+'.valid();'+internalName+'.next()';
    }
    ...
    "getForInOf":function( value, name, internalName , classmodule, isIterator )
    {
        ...
        return 'var '+internalName+' = ListIterator('+value+');'+internalName+'.valid();'+internalName+'.next()';
