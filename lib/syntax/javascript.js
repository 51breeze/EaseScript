...
function makeModuleString( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
{
    var keys = [ classname ];
    ...
    {
        if( importMap[n].nonglobal && classModule.fullclassname !==  importMap[n].fullclassname && !importMap[n].isInternal )
        {
    ...
    var str="";
    str += 'function(module,require){\n';
    str+= 'var '+classname+'=require.define("'+classModule.fullclassname+'",module,'+moduleStatement+','+typeMap[classModule.id]+');\n';
    Utils.forEach(mapHash,function(name,key)
    {
        str+="var "+name+"=require(\""+key+"\");\n";
    });

    str +='var '+config.context.private+"=module.privateSymbol;\n";

    //构造函数
    if( classModule.id ==='class' && !classModule.isStatic  )
    {
        str += "var constructor = " +classModule.constructor.value+";\n";
    }
    
    str += propertiesAndMethod||"";

    var descriptor = makeModuleDescriptorString( config,classModule, inheritModule, uri );
    str+="Object.merge(module,{\n\t"+descriptor.join(',\n\t')+"\n});\n";
    ...
    if( isInternal )
    {
        str="("+str+"(Internal.createModule(\""+(classModule.package ? classModule.package+"."+classModule.fullclassname : classModule.fullclassname )+"\"),require))";
    }
    return str;
}
...
function makeExportModuleString( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
{
    ...
    var requirements = {"Symbol":true,"Object":true,"System":true,"Internal":true};
    for(var n in importMap )
    {
        if( classModule.fullclassname !== importMap[n].fullclassname && !importMap[n].isInternal )
        {
            ...
            {
                idClass = importMap[n].fullclassname;
                var suffix = importMap[n].isSkinModule ? '.html' : config.suffix;

                if( idClass.substr(0, config.system_lib_path_name.length+1) === config.system_lib_path_name+'.' )
                {
                    idClass = path.resolve( config.system_lib_path, idClass.replace(/\./g,'/') ).replace(/\\/g,'/')+suffix;
                   
                }else
                {
                    idClass = path.resolve( workspace, idClass.replace(/\./g,'/') ).replace(/\\/g,'/')+suffix;
                }
            ...
            }else if( importMap[n].type )
            {
                ...
                idClass = path.resolve( config.system_lib_path, 'javascript/system', (Utils.getGlobalTypeMap( importMap[n].type ))+'.js' ).replace(/\\/g,'/');
                if( !Utils.isFileExists( idClass ) )
                {
                    continue;
                }
                //console.log( idClass )
    ...
    Utils.forEach(requirements,function(require,type)
    {
        if( require === true )
        {
            var idClass = path.resolve( config.system_lib_path, 'javascript/system', type+'.js').replace(/\\/g,'/');
            mapHash[ idClass  ] = type;
        }
    });
    ...
    var str="";
    var moduleStatement = makeModuleStatement( classModule );
    str+= 'var '+classname+'='+moduleStatement+';\n';
    ...
    var descriptor = makeModuleDescriptorString( config, classModule, inheritModule, uri );
    str+='Internal.defineClass("'+classModule.fullclassname+'",'+classname+',{\n\t'+descriptor.join(',\n\t')+'\n},'+typeMap[ classModule.id ]+');\n';
...
function makeModuleStatement(classModule)
{
    ...
    if( classModule.id ==='class' && !classModule.isStatic   )
    {
        moduleStatement= 'function '+classModule.fullclassname.replace(/\./g,'_')+'(){\nconstructor.apply(this,arguments);\n}';
    }
    else if( classModule.id==="namespace" )
    {
        moduleStatement= 'function '+classModule.fullclassname.replace(/\./g,'_')+'(prefix,uri){\nNamespace.call(this,prefix,uri);\n}';
    }
    else 
    {
...
var syntaxBuilder={
    ...
    "bulidNamespaceClassValueOf": function(classname, fullclassname, classModule, importMap, value , config )
    {
        ...
        var str="";
        str += 'function(module,require){\n';
        str+= 'var '+classname+'=require.define("'+classModule.fullclassname+'",module,function(prefix,uri){\nNamespace.call(this,prefix,uri);\n},'+typeMap[classModule.id]+');\n'; 
    ...
    "buildClassStructure":function( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
    {
        ...
        return makeModuleString(classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri);
    },
    ...
    {
        return 'if('+name+' === undefined ){'+value+';}';
    },
    ...
    "getForIn":function( value, name, internalName , module, isIterator , isArrayOrObject )
    {
        if( isIterator )
        {
