...
var syntax={
    ...
    "buildPropertiesAndMethod":function ( describe, config , classModule , makeSyntax)
    {
        ...
        for( var p in describe )
        {
            ...
            if( desc.defineMetaTypeList && desc.defineMetaTypeList.Syntax )
            {
                ...
                if( syntaxMetatype.syntax && syntaxMetatype.syntax[ makeSyntax ]  )
                {
                    desc = syntaxMetatype.syntax[ makeSyntax ];
            ...
            if (typeof desc.value === "object")
            {
                if (desc.value.get)
                {
                    item=['"value":'+ (desc.varToBindable && typeof desc.value.get === "string"  ?  desc.value.get : desc.value.get.value) ];
                    code.push('"Get_' + nsuri+p + '":{' + item.join(',') + '}');
                }

                if (desc.value.set)
                {
                    item=['"value":'+(desc.varToBindable && typeof desc.value.set === "string" ? desc.value.set : desc.value.set.value) ];
    ...
    "buildClassPropertiesAndMethod":function ( classModule, inheritModule, config, makeSyntax )
    {
        ...
        {
            //静态成员
            var s = syntax.buildPropertiesAndMethod( classModule.static, config , classModule, makeSyntax );
            ...
            {
                //实例成员
                s = syntax.buildPropertiesAndMethod( classModule.proto, config , classModule , makeSyntax);
    ...
    "buildClassStructure":function( classname, classModule, inheritModule, importMap, propertiesAndMethod, externalCode, config, uri )
    {
        ...
        var inherit = inheritModule ? inheritModule.fullclassname : null;
        for(var n in importMap )
        {
            if( importMap[n].nonglobal && classModule.fullclassname !==  importMap[n].fullclassname )
            {
                if( inherit ===importMap[n] )
                {
                    keys.splice(1,0, n.replace(/\./g,"_") );
                    values.splice(1,0, importMap[n].fullclassname );
    ...
    "buildExpressionGetOf":function (globalsConfig,classmodule, thisvrg, desc, props ,info, before,ns,useNs)
    {
        ...
        if( !checkRunning )
        {
            ...
            {
                owner = Maker.getLoaclAndGlobalModuleDescription( desc.descriptor.owner );
                if( !owner && classmodule.isInternal===true && desc.descriptor.owner === classmodule.classname  )
                {
                    owner = classmodule;
                }
            }
    ...
    "getCheckMethodParamTypeValue":function ( globalsConfig, value, type, nullable, typeClass, filename, line )
    {
        if( globalsConfig.mode !== 3 )
        {
            ...
            if( numberType.indexOf(type) >=0 )
            {
                ...
                if( type ==="uint" )
                {
                    return 'if(' + nullable+'!('+value+'>=0)'+')throw new TypeError("type does not match. must be ' + type + '","' + filename + '",' + line + ');\n';
                }
            }
            if ( type.indexOf(".") > 0 )
            {
                return 'if('+nullable+'!'+syntax.isOf( value, syntax.getDefinitionByName( typeClass ) )+')throw new TypeError("type does not match. must be ' + type + '","' +filename + '",' + line + ');\n';
            } else {
                return 'if('+nullable+'!' +syntax.isOf( value, typeClass ) + ')throw new TypeError("type does not match. must be ' + type + '","' +filename + '",' +line + ');\n';
            }
        }
    },
