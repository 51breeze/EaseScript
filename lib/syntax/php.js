...
var syntax={
    ...
    "bulidNamespaceClassValueOf": function(classname, fullclassname, module, importMap, value , config)
    {
        ...
        v += 'class '+classname+' extends Namespaces\n{\n';
    ...
    "definePropertiesClassOf":function (config, classProperties, classModule )
    {
        ...
        for( var p in classProperties )
        {
            ...
            if( classProperties[p].id !=="const" && !item.isStatic && !item.value.match(/^(\"|\'|\d+|null|false|true|NaN)/i) )
            {
    ...
    "buildPropertiesAndMethod":function ( describe, config , classModule , makeSyntax )
    {
        ...
        for( var p in describe )
        {
            ...
            if( desc.defineMetaTypeList && desc.defineMetaTypeList.RunPlatform )
            {
                continue;
            }
            ...
            }else
            {
                ...
                if (typeof desc.value === "object" && desc.id !== "namespace")
                {
                    if (desc.value.get)
                    {
                        value = (desc.varToBindable ? desc.value.get : desc.value.get.value);
                    ...
                    if (desc.value.set)
                    {
                        value = (desc.varToBindable ? desc.value.set : desc.value.set.value);
    ...
    "newInstanceClassOf":function (className, classModule, param)
    {
        ...
        {
            if( param.length > 2 ) {
                return 'new \\Object(' + param.replace(/\{/g, "[").replace(/\}/g, "]").replace(/\:/g, "=>") +')';
            }else
            {
                return "new \\Object()";
            }
    ...
    "buildExpressionGetOf":function (globalsConfig,classmodule, thisvrg, desc, props ,info, before,ns,useNs, isNew )
    {
        ...
        if( !checkRunning )
        {
            ns = desc.hasCustomNs || desc.descriptor.privilege==="internal" ? ns : "";
    ...
    "buildExpressionCallOf": function(globalsConfig, classmodule, superModule, thisvrg, desc, props ,info, ns , useNs )
    {
        ...
        if( !checkRunning )
        {
            ...
            //var thisname = (thisvrg.charAt(0)==="$" ? thisvrg.slice(1) : thisvrg);
    ...
    "getCheckMethodParamTypeValue":function ( globalsConfig, value, type, typeClass, filename, line )
    {
        if( type ==="*" || type=="void" )return "";
        if( !isReferenceType(type) )
        {
            return 'if( !' +syntax.isOf( value, syntax.getClassModuleName( type, typeClass ) ) + ')throw new TypeError("type mismatch the \\"\\'+value+'\\" parameter must be ' + type +'");\n';
        }
    },
    "getMethodParamDefualtValue":function ( globalsConfig, name, value , type)
    {
        if( value.match(/^([\'\"])/) || value.match(/^(\w+|\[\s?\])$/) ) {
            return '';
        }
    ...
    },"getJointOperator":function ( operator, left, right )
    {
        ...
        if( leftType !== "Number" && ( left.thisArg.match(/([\+\-\*\/\%\^\&\|]{1})([^>])/) || right.thisArg.match(/>>|<<|>>>|<<</) ) )
        {
            leftType = "Number";
        }

        if( rightType !== "Number" && ( right.thisArg.match(/([\+\-\*\/\%\^\&\|]{1})([^>])/) || right.thisArg.match(/>>|<<|>>>|<<</) ) )
        {
            rightType = "Number";
        }
...
function isReferenceType( type )
{
    ...
    return ["Object","Number","String","Boolean","Class","Function"].indexOf( type ) < 0;
}
