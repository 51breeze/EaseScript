...
var syntaxBuilder={
    ...
    "buildPropertiesAndMethod":function ( describe, config , classModule , makeSyntax )
    {
        ...
        for( var p in describe )
        {
            ...
            {
                var _value = syntaxBuilder.buildPropertiesAndMethod(desc,config,classModule );
                            ...
                            }else
                            {
                               if( isStatic && desc.value )
                               {
                                   ...
                                   // var propname = fullClassName+"::$"+nsuri+p;
                                    var reflectSet = "\\Reflect::set('"+fullClassName+"','"+fullClassName+"','"+p+"',"+desc.value+",null,"+(nsuri||null)+");\n";
    ...
    "buildClassStructure":function( classname, module, inheritClass, importMap, propertiesAndMethod , externalCode, config, uri, ns)
    {
        ...
        for(var n in importMap )
        {
            ...
            {
                importClass.push( (classNameMap[ _cname ] || _cname).replace(/\./g, "\\") );
            }
        }
        ...
        if( module.useExternalClassModules && module.useExternalClassModules.length >0 && module.package )
        {
             for(var us in  module.useExternalClassModules )
             {
                 ...
                 {
                     useClass[ _cname ] = true;
                     var realname = module.useExternalClassModules[us];
                     importClass.push( (classNameMap[ realname ] || realname).replace(/\./g,"\\") );
                 }
             }
        }
    ...
    "newInstanceClassOf":function (className, classModule, param)
    {
        ...
        {
            if( param.length > 2 ) {
                return 'new \\BaseObject(' + param.replace(/\{/g, "[").replace(/\}/g, "]")+')';
            }else
            {
    ...
    "getCheckMethodParamTypeValue":function ( globalsConfig, value, type, nullable, typeClass, filename, line )
    {
        ...
        if( !isReferenceType(type) )
        {
            ...
            return 'if('+nullable+'!' +syntaxBuilder.isOf( value, syntaxBuilder.getClassModuleName( type, typeClass ) ) + ')throw new \\TypeError("type mismatch the \\"\\'+value+'\\" parameter must be ' + type +'");\n';
        }
    },
