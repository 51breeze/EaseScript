...
const fun_map={
    ...
    "Function":function (thisarg, name, value, isGetFun) {
        ...
        switch ( name ) {
            ...
            case "apply" :
                return "\\es\\system\\Reflect::apply("+ [thisarg].concat( value ).join(",") + ")";
            case "bind" :
...
for(var p in classNameMap )
{
    if( fun_map[ p ] )
    {
        fun_map[ classNameMap[p] ] = fun_map[ p ];
    }
}
...
var syntaxBuilder={
    ...
    "buildExpressionCallOf": function(globalsConfig, classmodule, superModule, thisvrg, desc, props ,info, ns , useNs )
    {
        ...
        if( !checkRunning )
        {
            ...
            if( props.length===1 && fun_map.hasOwnProperty(thisvrg ) )
            {
    ...
    {
       switch ( classType.slice(1,-1).toLowerCase() ) {
           case "function":
    ...
    "defineMethodParameter":function ( globalsConfig, paramMap, classModule )
    {
        ...
        for (var n in paramMap)
        {
            ...
            if( item.value )
            {
                ...
                n ="$"+n;
                if( item.nullable || item.originValue )
                {
                    n+="=null";
                }
                //console.log(n)
    ...
    "makeArrowFunction":function(classmodule,globalsConfig, paramMaps, body){
        ...
        const names = params.map( item => {
            if( item.type !=="*" && item.type !=="void" && !isReferenceType(item.type) ){
                ...
                if( !(lowerType =="object" || lowerType==="array") )
                {
        ...
        if( restParam.length > 0 ){
            rest = restParam.map( item=> `\$${item.name}=new \\es\\system\\ArrayList( array_slice( func_get_args() , ${params.length}) );\n`).join("");
        }
