...
const thisMap={
    ...
    'window':"\\es\\system\\System::window()",
    "Object":"BaseObject",
}
...
var syntaxBuilder={
            ...
            Utils.forEach( module.useExternalClassModules, function (classname) {
                if( !useClass[ classname ] && !(excludeUseMap[ classname ] || module.fullclassname === classname) )
                {
                    ...
                    if( o.classname !== module.classname )
                    {
                        if (o.nonglobal === true) {
                            if (o.package) {
                                importClass.push( classNameMap[classname] || syntaxBuilder.formatImportClassPath(classname) );
                            }
                        } else {
                            importClass.push(syntaxBuilder.formatImportClassPath( classNameMap[classname] || ('es.system.'+classname) ) );
                        }
                    }
                }
            });
    ...
    "getDefinitionByName":function (classname , desc, classmodule, isNew )
    {
        ...
        }else if( !isglobal && isDef && !module.package && classname.indexOf(".") < 0 )
        {
            classname = "\\"+classname;
        }
    ...
    "getWhenValue":function ( value ) {
        return "\\es\\system\\System::when("+value.replace(/(\|\|)/g,",")+")";
    },
