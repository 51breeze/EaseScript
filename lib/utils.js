...
function parseMetaEmbed(metatype, config, context )
{
    ...
    return copyfileToBuildDir(source, utils.getBuildPath(config, 'build.assets'), config );
}
...
utils.excludeLoadSystemFile=function excludeLoadSystemFile( name )
{
    return !!_notLoadSystemFiles[ name.toLowerCase() ];
}
...
utils.getRequireIdentifier=function getRequireIdentifier( config, module, suffix )
{
    ...
    {
        return 'system/'+filepath+suffix;
    }
}

function makeFileToBuildPath()
{
    var b = Utils.resolvePath( attrValue, [PATH.dirname(context.filepath )] );
    var dest = Utils.copyfileToBuildDir( b, Utils.getBuildPath(context.config, 'build.assets'), context.config );
    attrValue = PATH.relative( Utils.getBuildPath(context.config, 'build.webroot'), dest).replace(/\\/g, '/');
}
