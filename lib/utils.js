...
var metaType = ['Bindable','Embed','Style','Event','Syntax','Skin','ArrayElementType','View','HostComponent','Remove','RunPlatform','Injection'];
function executeMetaType( metatype )
{
    ...
    metatype = trim( metatype ).replace(/^[\'\"]|[\'\"]$/g,'')
...
function parseMetaEmbed(metatype, config, relative )
{
    ...
    return copyfileToBuildDir(source, utils.getBuildPath(config, 'build.img'), config );
}
...
{
    var item=[];
    utils.forEach(object,function (item,key) {
        item.push(key);
    })
    return item;
}
