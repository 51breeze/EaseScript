...
var metaType = ['Bindable','Embed','Import','Style','Event','Syntax','Skin','ArrayElementType','View','HostComponent','Remove','RunPlatform','Injection','Router','Env','Configure'];
...
function copyfileToBuildDir(source, builddir, config)
{
    ...
    if( fs.existsSync(filepath) )
    {
        ...
        if( config.assets_hash )
        {
            path =  getBuildPath(config, 'build.assets');
            dest = PATH.join( path, utils.MD5( filepath, 32)+PATH.extname( filepath ) );
        }
...
{
    if( name.toLowerCase().slice(0,3) ==="on:")
    {
        return name.toLowerCase().slice(3);
    }
...
const EXCLUDE_GLOBAL = 1;
const EXCLUDE_ERROR = 2;
const EXCLUDE_EVENT = 4;
const EXCLUDE_SYSTEM = 8;
const EXCLUDE_CLIENT = 16;
excludeGroupFiles[EXCLUDE_GLOBAL]={
    "string":true,
    "String":true,
    "number":true,
    "Number":true,
...
excludeGroupFiles[EXCLUDE_ERROR]={
    "Error":true,
    "SyntaxError":true,
    "ReferenceError":true,
    "URIError":true,
...
excludeGroupFiles[EXCLUDE_EVENT]={
    "MouseEvent":true,
    "TouchEvent":true,
...
excludeGroupFiles[EXCLUDE_SYSTEM]={
    "arguments":true,
    "Arguments":true,
    "console":true,
    "Console":true,
...
excludeGroupFiles[EXCLUDE_CLIENT]={
    "Node":true,
    "NodeList":true,
    "Document":true,
    "document":true,
 ...
 * @return array
 */
utils.EXCLUDE_GLOBAL = EXCLUDE_GLOBAL;
utils.EXCLUDE_ERROR = EXCLUDE_ERROR;
utils.EXCLUDE_EVENT = EXCLUDE_EVENT;
utils.EXCLUDE_SYSTEM = EXCLUDE_SYSTEM;
utils.EXCLUDE_CLIENT = EXCLUDE_CLIENT;
utils.EXCLUDE_ALL = 0;

utils.excludeLoadSystemFile=function excludeLoadSystemFile(name,excludeFileMap,mode)
{
    if( !excludeFileMap && mode === void 0 )
    {
        excludeFileMap=excludeGroupFiles[ EXCLUDE_GLOBAL ];
    }

    if( typeof excludeFileMap ==="number" )
    {
        mode = excludeFileMap;
        excludeFileMap = null;
    }

    if( excludeFileMap && excludeFileMap[name] === true )
    {
         return true;
    }
    ...
    if( mode === 0 )
    {
        mode = Object.entries( excludeGroupFiles ).map( (value)=>value[0] ).reduce( (prev,curr)=>prev | curr);
    }
    ...
    if( mode > 0 )
    {
        for(var i = 0, n = 0; i<mode; n++)
        {
            i = Math.pow(2, n);
            var key = i & mode;
            if ( key>0 && utils.excludeLoadSystemFile(name, excludeGroupFiles[ key ]) )
            {
                return true;
            }
        }
...
utils.setExcludeLoadSystemFile=function(name, mode)
{
    var mode = mode || utils.getExcludeFileMaxMode();
    var obj = excludeGroupFiles[ mode ] || (excludeGroupFiles[ mode ]={});
    obj[ mode ][ name ] =true;
    return mode;
}

utils.getExcludeFileMaxMode=function()
{
    return Math.pow(2, Object.entries( excludeGroupFiles ).length-1 );
}
...
utils.getRequireIdentifier=function getRequireIdentifier( config, module, syntax , context )
{
    ...
    if( module.nonglobal && syntax === "javascript" )
    {
    ...
    }else
    {
        if( module.path && module.isImportMetatype )
        {
           ...
           {
               identifier = PATH.relative( config.project.path, module.path  ).replace(/\\/g,'/')
           }else{
        ...
        if( context )
        {
           ...
           identifier = PATH.relative( context, system_path ).replace(/\\/g,'/');
        }else{
            identifier = 'system/'+filepath+(suffix||".js");
        }
    }

    if( identifier.charAt(0) !=="." && identifier.charAt(0) !=="/" && syntax ==="node" )
    {
...
function makeAssetsFile(config,formPath,file)
{
    ...
    if( info.dir )
    {
        if( file.substr(0,root_path.length) === root_path )
        {
            to = PATH.join( to,  PATH.dirname( file.substr(root_path.length, file.length ) ) );
